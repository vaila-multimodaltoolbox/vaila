...
- **Crop Face (`vaila/crop_faces_atletas.py`)**: integrated at **Frame C -> Video and Image -> C_B_r1_c2**. The GUI flow must remain input directory first, output directory second, then optional `face_detector.task` selection when the model is not found. Creator metadata remains `Abel Gonçalves Chinaglia`; help docs are `vaila/help/crop_faces_atletas.md` and `.html`.
