...
def emg_analysis(emg_file, fs, start_index, end_index, no_plot, selected_path):
    ...
    if start_index >= len(emg_signal):
        messagebox.showerror("Invalid Start Index", "Start index is out of bounds. Exiting.")
        return
    ...
    if len(emg_signal_cut) == 0:
        messagebox.showerror("Empty Signal", "The selected range results in an empty signal. Exiting.")
        return
    ...
    if len(emg_abs) == 0:
        messagebox.showerror("Empty Signal After Rectification", "The rectified signal is empty. Exiting.")
        return
    ...
    freq_array = np.array(median_freq_values + [0] * (max_len - len(median_freq_values)))
    ...
    data_matrix = np.vstack((time_array, rms_array, freq_array, linear_envelope_values, freq_max_array, psd_max_array)).T

    with open(output_file, 'w') as f:
        f.write("Sample,RMS_µVolts,MedianFrequency_Hz,Linear_envelope_µVolts,Freq_Max,PSD_Max\n")
...
def run_emg_gui():
    ...
    emg_file = filedialog.askopenfilename(title="Select EMG File", filetypes=[("CSV files", "*.csv"), ("Text files", "*.txt")])
    if not emg_file:
        messagebox.showerror("No File Selected", "No file selected. Exiting.")
        return

    fs = simpledialog.askinteger("Input", "Enter Sampling Rate (Hz):", initialvalue=1000, minvalue=1)
    if fs is None:
    ...
    input_dialog = simpledialog.askstring("Input", "Enter Start Line, End Line, Plot Data (y/n) separated by commas:", initialvalue="0,None,n")
    if not input_dialog:
    ...
    if len(inputs) != 3:
        messagebox.showerror("Invalid Input", "Invalid input format. Provide three values separated by commas. Exiting.")
        return
