...
def imu_orientations(accelerometer, gyroscope, time, sample_rate, sensor_name):
    ...
    tilt_deg = np.stack((np.degrees(tilt_x_rad), np.degrees(tilt_y_rad), np.degrees(tilt_z_rad)), axis=-1)
    ...
    for index in range(len(time)):
        ahrs.update_no_magnetometer(gyroscope[index], accelerometer[index], (1 / sample_rate))  # Assuming Hz sampling rate
...
def plot_and_save_graphs(time, data, labels, title, xlabel, ylabel, sensor_name, save_path):
...
def save_results_to_csv(base_dir, time, gyroscope, accelerometer, euler, tilt_deg, quaternions, sensor_name):
    results = {
        'Time': time,
        'Gyro_X': gyroscope[:, 0],
        'Gyro_Y': gyroscope[:, 1],
...
def plot_and_save_sensor_data(time, gyroscope, accelerometer, euler, tilt_deg, quaternions, sensor_name, base_dir_figures):
    plot_and_save_graphs(time, accelerometer, ["Acc X", "Acc Y", "Acc Z"], "Accelerometer Data", 'Time (s)', 'Acceleration (g)', sensor_name, f'{base_dir_figures}/{sensor_name}_Accelerometer_Data.png')
    plot_and_save_graphs(time, gyroscope, ["Gyro X", "Gyro Y", "Gyro Z"], "Gyroscope Data", 'Time (s)', 'Gyroscope (°/s)', sensor_name, f'{base_dir_figures}/{sensor_name}_Gyroscope_Data.png')
    plot_and_save_graphs(time, tilt_deg, ["Tilt X", "Tilt Y", "Tilt Z"], "Tilt Angles", 'Time (s)', 'Tilt (°)', sensor_name, f'{base_dir_figures}/{sensor_name}_Tilt_Angles.png')
    plot_and_save_graphs(time, euler, ["Euler X", "Euler Y", "Euler Z"], "Euler Angles", 'Time (s)', 'Euler (°)', sensor_name, f'{base_dir_figures}/{sensor_name}_Euler_Angles.png')
...
def importc3d(dat):
    ...
    return markers, marker_labels, marker_freq, analogs, points_residuals, analog_labels, analog_freq
...
def analyze_imu_data():
    ...
    base_output_dir = os.path.join(output_directory, "IMU_multimodal_results")
    os.makedirs(base_output_dir, exist_ok=True)

    # Ask the user if they want to select headers for all files
    select_headers_for_all = messagebox.askyesno("Header Selection", "Select headers for all files?")
    
    selected_headers = None
    if select_headers_for_all:
        # Select a file to choose headers from
        selected_file = filedialog.askopenfilename(title="Pick file to select headers", filetypes=[("CSV files", "*.csv"), ("C3D files", "*.c3d")])
        ...
        try:
            if file_type == 'csv' and file_name.endswith('.csv'):
                ...
                if selected_headers:
                    ...
                    data = show_csv(file_path)  # Assuming show_csv returns the selected data
                if data is not None:
                    dataf = apply_filter(data, sample_rate, method=filter_method)
                    accelerometer_1 = (dataf[:, [0, 2, 1]] * np.array([1, 1, -1])) / gravity
                    gyroscope_1 = (dataf[:, [3, 5, 4]] * np.array([1, 1, -1])) * (180 / np.pi)
                    accelerometer_2 = (dataf[:, [9, 11, 10]] * np.array([1, 1, -1])) / gravity
                    gyroscope_2 = (dataf[:, [12, 14, 13]] * np.array([1, 1, -1])) * (180 / np.pi)
                ...
                #analogs_2d = analogs.reshape(analogs.shape[0] * analogs.shape[1], analogs.shape[2])
                #df_analogs = pd.DataFrame(analogs_2d.T, columns=analog_labels)  # Convert to DataFrame for easier selection
                analogs_selected = df_analogs[selected_headers].values  # Select columns from DataFrame
                dataf = apply_filter(analogs_selected, sample_rate, method=filter_method)
                accelerometer_1 = (dataf[:, [0, 2, 1]] * np.array([1, 1, -1])) / gravity
                gyroscope_1 = (dataf[:, [3, 5, 4]] * np.array([1, 1, -1])) * (180 / np.pi)
                accelerometer_2 = (dataf[:, [9, 11, 10]] * np.array([1, 1, -1])) / gravity
                gyroscope_2 = (dataf[:, [12, 14, 13]] * np.array([1, 1, -1])) * (180 / np.pi)

            time = np.linspace(0, len(accelerometer_1) / sample_rate, len(accelerometer_1))

            tilt_1, euler_1, quaternions_1 = imu_orientations(accelerometer_1, gyroscope_1, time, sample_rate, "Sensor Trunk")
            tilt_2, euler_2, quaternions_2 = imu_orientations(accelerometer_2, gyroscope_2, time, sample_rate, "Sensor Pelvis")
            ...
            base_dir_figures = os.path.join(base_output_dir, f'figure/IMU_{file_name}_{current_time}')
            base_dir_processed_data = os.path.join(base_output_dir, f'processed_data/IMU_{file_name}_{current_time}')
            ...
            plot_and_save_sensor_data(time, gyroscope_1, accelerometer_1, euler_1, tilt_1, quaternions_1, "Sensor Trunk", base_dir_figures)
            plot_and_save_sensor_data(time, gyroscope_2, accelerometer_2, euler_2, tilt_2, quaternions_2, "Sensor Pelvis", base_dir_figures)

            # Save processed data for each sensor
            save_results_to_csv(base_dir_processed_data, time, gyroscope_1, accelerometer_1, euler_1, tilt_1, quaternions_1, "Sensor Trunk")
            save_results_to_csv(base_dir_processed_data, time, gyroscope_2, accelerometer_2, euler_2, tilt_2, quaternions_2, "Sensor Pelvis")
