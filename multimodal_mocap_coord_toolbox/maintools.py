...
def get_kinematics_c3d(file_path):
    ...
    try:
        ...
        all_points = {label: {'x': [], 'y': [], 'z': []} for label in c3d['parameters']['POINT']['LABELS']['value']}
...
def get_joint_df(kinematic_dict: Dict[str, Dict[str, list]], joint: Optional[str] = None) -> pd.DataFrame:
...
def calculate_coupling_angle(joint1_array: np.ndarray, joint2_array: np.ndarray):
    ...
    CtgVar_vc_DG = np.select(
        condlist=[(coupangle >= 0) & (coupangle < 22.5), 
    ...
    group_phase = [round((np.count_nonzero(CtgVar_vc_DG == i) / len(CtgVar_vc_DG)) * 100, 2) for i in range(1, 5)]

    return group_phase, coupangle

def create_coupling_angle_figure(group_percent, coupangle, array_joint1, array_joint2, joint1_name="Joint1", joint2_name="Joint2", axis_title="X-Axis", size=15):
    letter_size = size - 5
    mark_size = size / 2
    alpha_value = 0.5
    ...
    ax[0].set_title(f'Joint Angles | {joint1_name} - {joint2_name} | Axis: {axis_title}', size=letter_size, weight='bold')
    ax[0].plot(array_joint1, marker='o', linestyle='-', color='b', markersize=mark_size, alpha=alpha_value, label=joint1_name)
    ax[0].plot(array_joint2, marker='o', linestyle='-', color='r', markersize=mark_size, alpha=alpha_value, label=joint2_name)
    ...
    ax[1].set_title(f'Coupling Angle | {joint1_name} - {joint2_name} | Axis: {axis_title}', size=letter_size, weight='bold')
    ax[1].plot(coupangle, color='k', marker='o', markersize=mark_size, linestyle=':', alpha=alpha_value, label='Coupling Angle')
    ...
    ax2 = ax[1].twinx()
    ax2.set_yticks([22.5-11.25, 67.5-22.5, 112.5-22.5, 157.5-22.5, 202.5-22.5, 247.5-22.5, 292.5-22.5, 337.5-22.5, 360],
                    [f'{joint1_name}','In-Phase',f'{joint2_name}','Anti-Phase',f'{joint1_name}','In-Phase',f'{joint2_name}','Anti-Phase',f'{joint1_name}'], weight='bold')
    
    labels = ['Anti-Phase', 'In-Phase', f'{joint1_name} Phase', f'{joint2_name} Phase']
    ax[2].set_title(f'Categorization of Coordination Patterns | {joint1_name} - {joint2_name} | Axis: {axis_title}', size=letter_size, weight='bold')
    ax[2].set_ylabel('Percentage (%)', fontsize=letter_size)
    #bars = ax[2].bar(labels, group_percent, color=gray_colors, alpha=0.7)
    
    patches = [mpatches.Patch(color=color, label=f'{label}: {perc:.0f}%') for color, label, perc in zip(gray_colors, labels, group_percent)]
