...
def create_main_output_directory(filename):
    ...
    main_output_dir = os.path.join(os.getcwd(), f"Results_force_cube_analysis_{today_date}", f"Results_{base_filename}")
...
def makefig1(data, output_dir, filename):
    ...
    ax1.plot(data)
    ax1.set_title("Space + Left Click to select, Right Click to remove, press 'Enter' to finish")
...
def makefig2(data, indices, threshold):
    ...
    for i in range(min(num_segments, 9)):  # Limit to 9 segments for the 3x3 matrix
        ...
        start_active, end_active = find_active_indices(section_data, start_median, end_median, threshold)
        ...
        ax2[i].grid(True)
        ax2[i].legend(fontsize='xx-small', loc='upper right', handlelength=1, handletextpad=0.2, borderaxespad=0.2, framealpha=0.6)
        plt.tight_layout()
...
def build_headers():
    # Define CSV headers
    headers = [
        "FileName", "TimeStamp", "Trial", "BW_kg", "SideFoot_RL", "Dominance_RL", "Quality",
        "Num_Samples", "Index_40ms", "Index_100ms", "Index_ITransient", "Index_VIP", "Index_Max",
        "Test_Duration_s","CumSum_Times_s", "Contact_Time_s", "Time_40ms_s", "Time_100ms_s", "Time_ITransient_s", "Time_VIP_s", "Time_Peak_VMax_s",
        "VPeak_40ms_BW", "VPeak_100ms_BW", "Peak_VITransient_BW",  "Peak_VIP_BW", "Peak_VMax_BW",
        "Total_Imp_BW.s", "Imp_40ms_BW.s", "Imp_100ms_BW.s", "Imp_ITransient_BW.s", "Imp_Brake_VMax_BW.s", "Imp_Propulsion_BW.s",
        "RFD_40ms_BW.s-1", "RFD_100ms_BW.s-1", "RFD_ITransient_BW.s-1", "RFD_Brake_VMax_BW.s-1", "RFD_Propulsion_BW.s-1",
        "Simple_stiffness_constant", "High_stiffness", "Low_stiffness", "Transition_time", "Average_loading_rate"
        ]
...
def calculate_loading_rates(vgrf, time_data):
    ...
    def find_vip_index(vgrf, time_data):
        ...
        if peaks.size > 0:
            vip_index = peaks[np.argmax(vgrf[peaks])]  # VIP is the highest peak within the first 40%
        else:
    ...
    print("\n")
    print("VIP Index:", vip_index, "Start index:", start_index_poi20_80, "End index:", end_index_poi20_80)
    ...
    return valr, vip_value, vip_index, vgrf_poi20_80, time_poi20_80, start_index_poi20_80, end_index_poi20_80
...
def fit_stiffness_models(vgrf_poi20_80, time_poi20_80):
    ...
    try:
        ...
        popt, _ = curve_fit(logistic_ogive, time_poi20_80, vgrf_poi20_80, bounds=(0, [np.inf, np.inf, np.max(time_poi20_80), np.inf]))
        kl, kh, tT, m = popt
    except RuntimeError as e:
        print("Curve fitting failed:", e)
        kl, kh, tT, m = np.nan, np.nan, np.nan, np.nan  # Default to NaN if fitting fails
...
def calculate_cube_values(signal, Fs):
    ...
    * Reference to Impact Transient
    ...
    valr, vip_value, vip_index, vgrf_poi20_80, time_poi20_80, start_index_poi20_80, end_index_poi20_80 = calculate_loading_rates(signal, time_interval)
    ...
    area_impact_transient = np.trapz(signal[:index_impact_transient], time_interval[:index_impact_transient])
    rfd_impact_transient = vpeak_impact_transient / time_impact_transient if time_impact_transient != 0 else np.nan
    ...
    area_propulsion = np.trapz(signal[index_peakmax:], time_interval[index_peakmax:] - time_interval[index_peakmax])
    rfd_propulsion = (signal[-1] - vpeakmax) / (total_time - time_peakmax) if time_peakmax and (total_time - time_peakmax) != 0 else float('inf')

    return (num_samples, index_40ms, index_100ms, index_impact_transient, index_peakmax,
            total_time, time_interval, t40ms, t100ms, time_impact_transient, time_peakmax,
            vpeak_40ms, vpeak_100ms, vpeak_impact_transient, vpeakmax,
            total_area, area_until_40ms, area_until_100ms, area_impact_transient, area_peakmax, area_propulsion,
            rfd_40ms, rfd_100ms, rfd_impact_transient, rfd_peakmax, rfd_propulsion, index_poi, vpeak_poi, time_poi)

## FIGURE 3
def makefig3(databw, active_ranges, body_weight_kg, output_dir, filename, sidefoot, dominance, quality, simple_filename, timestamp, indices, Fs):
    ...
    result_plot_filename = os.path.join(output_dir, filename_without_extension + "_result.png")
    ...
    with open(result_csv_filename, 'w', newline='') as csvfile:
        ...
        for i, (start, end) in enumerate(active_ranges[:9]):
            ...
            (num_samples,
            index_40ms, index_100ms, index_impact_transient, index_peakmax,
            total_time, time_interval, t40ms, t100ms, time_impact_transient, time_peakmax,
            vpeak_40ms, vpeak_100ms, vpeak_impact_transient, vpeakmax,
            total_area, area_until_40ms, area_until_100ms, area_impact_transient, area_peakmax, area_propulsion,
            rfd_40ms, rfd_100ms, rfd_impact_transient, rfd_peakmax, rfd_propulsion, index_poi, vpeak_poi, time_poi) = calculate_cube_values(active_segment_data, Fs)
            ...
            valr, vip_value, vip_index, vgrf_poi20_80, time_poi20_80, start_index_poi20_80, end_index_poi20_80 = calculate_loading_rates(active_segment_data, np.arange(len(active_segment_data)) / Fs)
            index_impact_transient = end_index_poi20_80
            time_impact_transient = time_poi20_80[-1]
            ...
            current_ticks = np.append(ax3[i].get_xticks(), 0.04)  # Include 0.04s in the existing ticks
            ...
            y_fill_impact_transient = (vpeak_impact_transient / time_impact_transient) * x_fill_impact_transient
            # Fill the area below the line in red with transparency
            ax3[i].fill_between(x_fill_impact_transient, 0, y_fill_impact_transient, color='red', alpha=0.3)
            ...
            ax3[i].plot(time_impact_transient, vpeak_impact_transient, 'r*', markersize=12, label=f'Transient: {time_impact_transient:.3f}s:{vpeak_impact_transient:.3f}BW')
            ax3[i].plot(time_peakmax, vpeakmax, 'kv', markersize=8, label=f'Peak Max: {time_peakmax:.3f}s:{vpeakmax:.3f}BW')
            ax3[i].axvline(x=t40ms, color='red', label=f'Peak in 40ms: {vpeak_40ms:.3f}BW', linestyle=':', linewidth=1)
            ax3[i].axvline(x=t100ms, color='olive', label=f'Peak in 100ms: {vpeak_100ms:.3f}BW', linestyle=':', linewidth=1)
            ax3[i].plot(time_poi, vpeak_poi, 'rv', markersize=8, label=f'VIP_Transient: {time_peakmax:.3f}s:{vpeakmax:.3f}BW')

            # Linhas para RFD
            # Plotando linhas inclinadas para RFD Impact Transient
            ax3[i].plot([0, time_impact_transient], [active_segment_data[0], vpeak_impact_transient], 'r--', linewidth=2.5, label=f'RFD Impact Transient: {rfd_impact_transient:.3f}BW\u00B7s\u207B\u00B9')
            # Se rfd_peakmax tem o mesmo intervalo de tempo que rfd_impact_transient
            ax3[i].plot([0, time_peakmax], [active_segment_data[0], vpeakmax], color='orange', linestyle='--', linewidth=2.5, label=f'RFD Brake: {rfd_peakmax:.3f}BW\u00B7s\u207B\u00B9')
            # Para RFD Propulsion, que começa n o pico máximo e vai até o fim do tempo total
            ax3[i].plot([time_peakmax, total_time], [vpeakmax, active_segment_data[-1]], 'g--', linewidth=2.5, label=f'RFD Propulsion: {rfd_propulsion:.3f}BW\u00B7s\u207B\u00B9')
            ax3[i].plot([], [], 's', color='red', alpha=0.3, label=f'Impulse Impact Transient: {area_impact_transient:.3f}BW\u00B7s')
            ax3[i].plot([], [], 's', color='orange', alpha=0.3, label=f'Impulse Brake: {area_peakmax:.3f}BW\u00B7s')
            ax3[i].plot([], [], 's', color='green', alpha=0.3, label=f'Impulse Propulsion: {area_propulsion:.3f}BW\u00B7s')
            ax3[i].plot([], [], 's', color='gray', alpha=0.2, label=f'Total Impulse: {total_area:.3f}BW\u00B7s')
            ax3[i].plot([], [], ' ', label=f'Contact Time: {total_time:.3f}s')  # Invisible line for Contact Time
            # Add legend with small font size
            ax3[i].legend(fontsize='small', loc='upper right', handlelength=1, handletextpad=0.2, borderaxespad=0.2, framealpha=0.6)
            plt.tight_layout()

            result = [f"{simple_filename}", f"{timestamp}", f"{i+1}", f"{body_weight_kg:.3f}", f"{sidefoot}", f"{dominance}", f"{quality}",
                    f"{num_samples}", f"{index_40ms}", f"{index_100ms}", f"{index_impact_transient}", f"{index_poi}", f"{index_peakmax}",
                    f"{test_duration[i]:.3f}", f"{cumulative_time:.3f}", f"{total_time:.3f}", f"{t40ms:.3f}", f"{t100ms:.3f}", f"{time_impact_transient:.3f}", f"{time_poi:.3f}", f"{time_peakmax:.3f}",
                    f"{vpeak_40ms:.3f}", f"{vpeak_100ms:.3f}", f"{vpeak_impact_transient:.3f}", f"{vpeak_poi:.3f}", f"{vpeakmax:.3f}",
                    f"{total_area:.3f}", f"{area_until_40ms:.3f}", f"{area_until_100ms:.3f}", f"{area_impact_transient:.3f}", f"{area_peakmax:.3f}", f"{area_propulsion:.3f}",
                    f"{rfd_40ms:.3f}", f"{rfd_100ms:.3f}", f"{rfd_impact_transient:.3f}", f"{rfd_peakmax:.3f}", f"{rfd_propulsion:.3f}",
                    f"{kc:.3f}", f"{kh:.3f}", f"{kl:.3f}", f"{tT:.3f}", f"{valr:.3f}"]
            writer.writerow(result)

            matresults = np.asarray([
                simple_filename, timestamp, i+1, body_weight_kg, sidefoot, dominance, quality,
                num_samples, index_40ms, index_100ms, index_impact_transient, index_poi, index_peakmax,
                test_duration[i], cumulative_time, total_time, t40ms, t100ms, time_impact_transient, time_poi, time_peakmax,
                vpeak_40ms, vpeak_100ms, vpeak_impact_transient, vpeak_poi, vpeakmax,
                total_area, area_until_40ms, area_until_100ms, area_impact_transient, area_peakmax, area_propulsion,
                rfd_40ms, rfd_100ms, rfd_impact_transient, rfd_peakmax, rfd_propulsion,
                kc, kh, kl, tT, valr
                ], dtype=object)  # Note que mudamos para 'dtype=object' para acomodar strings e floats.
...
def run_statistics(data2stats, filename, output_dir, timestamp):
    result_stat_filename = os.path.join(output_dir, os.path.splitext(os.path.basename(filename))[0] + "_stats.csv")
    result_profile_filename = os.path.join(output_dir, os.path.splitext(os.path.basename(filename))[0] + "_profile.html")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:  # Check if filename is provided
        print("Usage: python script.py <filename> [sidefoot] [dominance] [quality] [threshold] [columns] [index_file] [bw_kg]")
    ...
    if index_file:
        indices = np.loadtxt(index_file).astype(int)  # Load and convert indices from file if provided
    else:
        indices = makefig1(databw_norm, main_output_dir, filename)  # Pass output_dir and filename to makefig1

    active_ranges = makefig2(databw_norm, indices, threshold)
    databw = butterworthfilt(databw_norm, 59, Fs)
    results = makefig3(databw, active_ranges, body_weight_kg, main_output_dir, filename, sidefoot, dominance, quality, simple_filename, timestamp, indices, Fs)
    result_stats, result_profile = run_statistics(results, filename, main_output_dir, timestamp)
