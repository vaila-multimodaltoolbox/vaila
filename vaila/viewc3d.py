...
def ask_user_units_c3d():
    ...
    mm_radio = tk.Radiobutton(
        main_frame,
        text="Millimeters (mm) - typical values like 1234.56",
        variable=choice_var,
        value="mm",
        font=("Arial", 10),
    )
    ...
    m_radio = tk.Radiobutton(
        main_frame,
        text="Meters (m) - typical values like 1.23",
        variable=choice_var,
        value="m",
        font=("Arial", 10),
    )
...
def check_opengl_support():
    ...
    try:
        ...
        success = test_vis.create_window(
            window_name="OpenGL Test", width=100, height=100, visible=False
        )
        ...
        if platform.system() == "Darwin" and "glfw" in error_msg:
            return (
                False,
                f"macOS GLFW error (common on Apple Silicon M-series): {str(e)}",
            )
...
def run_viewc3d():
    ...
    if len(valid_data_points) > 0:
        data_x_min, data_x_max = (
            valid_data_points[:, 0].min(),
            valid_data_points[:, 0].max(),
        )
        data_y_min, data_y_max = (
            valid_data_points[:, 1].min(),
            valid_data_points[:, 1].max(),
        )
        data_z_min, data_z_max = (
            valid_data_points[:, 2].min(),
            valid_data_points[:, 2].max(),
        )
    ...
    def load_skeleton_from_json(_vis_obj):
        ...
        if not skeleton_linesets:
            messagebox.showwarning(
                "Skeleton",
                "No skeleton segments were created. Check marker names in the JSON vs selected markers.",
            )
        vis.update_renderer()
        return False
    ...
    def measure_distance_between_two_markers(_vis_obj):
        ...
        if a not in name_to_idx or b not in name_to_idx:
            messagebox.showwarning(
                "Warning", "One or both marker names not found among selected markers."
            )
            return False
    ...
    def apply_swaps_from_file(_vis_obj):
        ...
        swap_file = filedialog.askopenfilename(
            ...
            filetypes=[("CSV Files", "*.csv"), ("TOML Files", "*.toml"), ("All Files", "*.*")],
        )
        root.destroy()
        ...
        try:
            ...
            for swap in swaps_list:
                ...
                c3d_data[:, idx_a, frame_start : frame_end + 1] = c3d_data[
                    :, idx_b, frame_start : frame_end + 1
                ]
    ...
    def manual_swap_markers(_vis_obj):
        ...
        def create_swap_dialog():
            ...
            def show_dialog():
                ...
                title_label = tk.Label(
                    main_frame, text="Create Swap CSV Entry", font=("Arial", 14, "bold")
                )
                title_label.pack(pady=(0, 10))
                ...
                selected_a_label = tk.Label(
                    selected_frame, text="Marker A: None", font=("Arial", 10), fg="blue"
                )
                selected_a_label.pack(anchor=tk.W)
                selected_b_label = tk.Label(
                    selected_frame, text="Marker B: None", font=("Arial", 10), fg="red"
                )
                    ...
                    if selected_markers[0] is None:
                        ...
                        if selected_name == selected_markers[0]:
                            tk.messagebox.showwarning(
                                "Warning", "Cannot select the same marker twice!", parent=root
                            )
                            listbox.selection_clear(0, tk.END)
                            return
                ...
                tk.Label(range_frame, text="Swap range:", font=("Arial", 10, "bold")).pack(
                    anchor=tk.W
                )
                tk.Radiobutton(
                    range_frame,
                ...
                def on_swap():
                    if selected_markers[0] is None or selected_markers[1] is None:
                        tk.messagebox.showwarning(
                            "Warning", "Please select two markers!", parent=root
                        )
                        return
        ...
        # Validate marker names (ensure they exist in the C3D file)
        label_to_idx = {label: idx for idx, label in enumerate(all_marker_labels)}
        if marker_a not in label_to_idx or marker_b not in label_to_idx:
            print(f"\n[red]Error: One or both markers not found: {marker_a}, {marker_b}[/red]")
            return False
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                if not all(
                    col in df_existing.columns
                    for col in ["marker_a", "marker_b", "start_frame", "end_frame"]
                ):
            ...
            range_str = (
                f"frame {frame_start + 1}"
                if frame_start == frame_end
                else f"frames {frame_start + 1}-{frame_end + 1}"
            )
            print(
                f"\n[green]Swap CSV entry created: {marker_a} <-> {marker_b} at {range_str}[/green]"
            )
            print(f"[green]CSV saved to: {csv_path}[/green]")
            print("[cyan]Use 'N' key to apply swaps from this CSV file[/cyan]")
            ...
            tk.messagebox.showerror(
                "Error", f"Failed to create swap CSV:\n{str(e)}", parent=error_root
            )
            error_root.destroy()
    ...
    def save_c3d_file(_vis_obj):
        ...
        try:
            ...
            if "ANALOG:UNITS" in error_msg:
                ...
                print(
                    "[yellow]This is a known issue with some C3D files from certain motion capture systems[/yellow]"
                )
                print("[cyan]Trying alternative save method...[/cyan]")
                ...
                try:
                    ...
                    # Copy header
                    new_c3d["header"]["points"]["frame_rate"] = edited_c3d_object["header"][
                        "points"
                    ]["frame_rate"]
                    new_c3d["header"]["points"]["first_frame"] = edited_c3d_object["header"][
                        "points"
                    ]["first_frame"]
                    new_c3d["header"]["points"]["last_frame"] = edited_c3d_object["header"][
                        "points"
                    ]["last_frame"]
                    ...
                    print(
                        "[yellow]Note: Some parameters (like ANALOG data) were not included due to compatibility issues[/yellow]"
                    )
    ...
    def save_screenshot(_vis_obj):
        ...
        out_path = os.path.join(
            out_dir, f"viewc3d_frame{current_frame:05d}_{screenshot_serial[0]:03d}.png"
        )
        try:
    ...
    def save_skeleton_template_json(_vis_obj):
        ...
        try:
            ...
            print(
                f"  Available markers: {len(selected_marker_names)} (showing first 30 in template)"
            )
    ...
    def print_labels_to_terminal():
        ...
        for marker_name, marker_pos in zip(selected_marker_names, frame_data, strict=True):
            if not np.isnan(marker_pos).any():
                ...
                print(
                    f"  {marker_name:20s} | Pos: [{marker_pos[0]:8.4f}, {marker_pos[1]:8.4f}, {marker_pos[2]:8.4f}] | Color: {color_hex}"
                )
            else:
...
O - Manual swap markers (visual editing)
    ...
    def show_quality_stats(_vis_obj):
        ...
        if events:
            ...
            for i, (time, label, context) in enumerate(
                zip(events["times"], events["labels"], events["contexts"], strict=True)
            ):
    ...
    def create_full_soccer_field(vis, df):
        ...
        radius = np.linalg.norm(
            np.array(p["center_circle_top_intersection"]) - np.array(p["center_field"])
        )
        ...
        geometries.append(
            draw_line_3d(
                vis,
                p["left_penalty_area_top_right"],
                p["left_penalty_area_bottom_right"],
            )
        )
        geometries.append(
            draw_line_3d(
                vis,
                p["left_penalty_area_bottom_right"],
                p["left_penalty_area_bottom_left"],
            )
        )
        ...
        geometries.append(
            draw_line_3d(
                vis,
                p["right_penalty_area_bottom_left"],
                p["right_penalty_area_top_left"],
            )
        )
        geometries.append(
        ...
        geometries.append(
            draw_line_3d(
                vis,
                p["right_penalty_area_top_right"],
                p["right_penalty_area_bottom_right"],
            )
        )
        geometries.append(
            draw_line_3d(
...
def load_field_lines_from_csv():
    ...
    try:
        ...
        if "point_name" in df.columns and all(
            p in df["point_name"].values for p in required_points
        ):
            ...
            if not lines_indices:
                messagebox.showwarning(
                    "Warning",
                    "No lines could be generated from the CSV. Ensure it contains at least two points per line segment.",
                )
                return None
            ...
            print(
                f"Data bounds: X[{x_min:.1f}, {x_max:.1f}] Y[{y_min:.1f}, {y_max:.1f}] Z[{z_min:.1f}, {z_max:.1f}]"
            )
            print(
                f"Center: [{center_x:.1f}, {center_y:.1f}, {center_z:.1f}], Optimal zoom: {optimal_zoom:.4f}"
            )
