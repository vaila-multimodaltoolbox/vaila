...
def load_c3d_file():
    ...
    try:
        ...
        if is_mm:
            ...
            print(f"[bold green]✓ Applied conversion: MILLIMETERS → METERS[/bold green]")
            print(f"  Method: {detection_method}")
        else:
            print(f"[bold green]✓ No conversion applied: Data already in METERS[/bold green]")
            print(f"  Method: {detection_method}")
        ...
        print(f"[bold green]Successfully loaded and processed C3D file![/bold green]")
...
def run_viewc3d():
    ...
    def set_view_limits(vis_obj):
        ...
        try:
            ...
            ctr.set_zoom(0.5)  # Adjust zoom based on the range
...
H - Show this help"""
    ...
    def override_units(_vis_obj):
        ...
        if user_choice == 'mm':
            ...
            print(f"[yellow]Converting data assuming current data is in millimeters...[/yellow]")
            points = points * 0.001  # Assuming current is mm, convert to meters
            
        elif user_choice == 'm':
            # Convert from mm to meters (multiply by 1000 then by 0.001)
            print(f"[yellow]Converting data assuming current data is in meters...[/yellow]")
        ...
        print(f"[bold green]Unit conversion override applied![/bold green]")
        return False
...
def load_field_lines_from_csv():
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols):
            messagebox.showerror("Error", "CSV file must contain columns: x, y, z")
            return None
        
        # Criar linhas baseadas no arquivo soccer field
        lines_points = []
        lines_indices = []
        ...
        for i, row in df.iterrows():
            ...
            lines_points.append([x, y, z])
        
        # Criar linhas específicas para campo de futebol
        # Perímetro do campo (pontos 1-4)
        perimeter_indices = [0, 1, 3, 2, 0]  # bottom_left -> top_left -> top_right -> bottom_right -> bottom_left
        for i in range(len(perimeter_indices) - 1):
            lines_indices.append([perimeter_indices[i], perimeter_indices[i + 1]])
        
        # Linha do meio (pontos 5-6: midfield_left -> midfield_right)
        if len(lines_points) > 5:
            lines_indices.append([4, 5])  # midfield_left to midfield_right
        
        # Círculo central (pontos 7-9)
        if len(lines_points) > 8:
            # Criar aproximação do círculo central
            center_points = [6, 7, 8]  # center_field, circle_top, circle_bottom
            for i in range(len(center_points) - 1):
                lines_indices.append([center_points[i], center_points[i + 1]])
        
        # Gols e áreas (se existirem no CSV)
        if len(lines_points) > 12:
            # Gol esquerdo
            lines_indices.append([9, 10])  # left_goal_bottom to left_goal_top
            # Gol direito  
            lines_indices.append([11, 12])  # right_goal_bottom to right_goal_top
            
            # Área de penalidade esquerda
            if len(lines_points) > 16:
                penalty_left = [13, 14, 16, 15, 13]  # retângulo da área
                for i in range(len(penalty_left) - 1):
                    lines_indices.append([penalty_left[i], penalty_left[i + 1]])
                
                # Área de penalidade direita
                if len(lines_points) > 28:
                    penalty_right = [25, 26, 28, 27, 25]
                    for i in range(len(penalty_right) - 1):
                        lines_indices.append([penalty_right[i], penalty_right[i + 1]])
