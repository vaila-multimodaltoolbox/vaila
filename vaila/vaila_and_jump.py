...
def process_jump_data(input_file, output_dir, use_time_of_flight):
    ...
    try:
        ...
        for index, row in data.iterrows():
            ...
            liftoff_force = calculate_liftoff_force(mass, velocity, contact_time) if contact_time else None
            average_power = calculate_average_power(potential_energy, contact_time) if contact_time else None
            relative_power = (average_power / mass) if average_power else None
            jump_performance_index = (average_power * time_of_flight) if average_power and time_of_flight else None
            total_time = time_of_flight + contact_time if time_of_flight and contact_time else None

            # Append the results for each row
            results.append({
                'height_m': round(height, 3),
                'liftoff_force_N': round(liftoff_force, 3) if liftoff_force else None,
                'velocity_m/s': round(velocity, 3),
                'potential_energy_J': round(potential_energy, 3),
                'kinetic_energy_J': round(kinetic_energy, 3),
                'average_power_W': round(average_power, 3) if average_power else None,
                'relative_power_W/kg': round(relative_power, 3) if relative_power else None,
                'jump_performance_index': round(jump_performance_index, 3) if jump_performance_index else None,
