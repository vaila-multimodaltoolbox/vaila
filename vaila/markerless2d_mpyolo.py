        ...
        while cap.isOpened():
            ...
            for object_id in tracking_data[0]:
                ...
                if frame_data:
                    ...
                    pose_estimator, estimator_idx = pose_manager.get_pose_estimator(
                        bbox, object_id
                    )
        ...
        while cap.isOpened():
            ...
            for class_id, objects in tracking_data.items():
                for object_id, detections in objects.items():
                    ...
                    if (
                        class_id == 0
                        and object_id in pose_data
                        and frame_idx in pose_data[object_id]
                    ):
                        ...
                        for lm in landmarks:
                            if lm is not None:
                                cv2.circle(
                                    frame, (int(lm[0]), int(lm[1])), 2, (0, 0, 255), -1
                                )
...
def run_tracker_in_thread(model_path, video_source, tracker_config, save_output):
    ...
    results = model.track(
        source=video_source, tracker=tracker_config, save=save_output, stream=True
    )
    for _ in results:
        pass
    ...
    for class_id, objects in tracking_data.items():
        ...
        for object_id, detections in objects.items():
            csv_path = os.path.join(
                output_dir, f"{class_name}_{object_id}_detections.csv"
            )
