...
Then run the markerless_2d_analysis_nvidia.py script:
python markerless_2d_analysis_nvidia.py -i input_directory -o output_directory -c config.toml
...
MediaPipe GPU delegate, providing 2-5x performance improvement over CPU processing.
...
- MediaPipe (`pip install mediapipe`)
  * **Linux (NVIDIA)**: Ensure CUDA Toolkit and cuDNN are installed/configured.
...
def _debug_log(hypothesis_id, location, message, data=None):
    try:
        ...
        try:
            ...
            _log_path = _debug_log_dir / "debug.log"
...
def test_mediapipe_gpu():
    """
    Test if MediaPipe can use GPU delegate.
    ...
    try:
        ...
        try:
            ...
            if "GPU" in error_msg or "delegate" in error_msg.lower() or "CUDA" in error_msg:
                return False, f"GPU delegate test failed: {error_msg}"
            else:
        ...
        return False, f"Unexpected error testing GPU: {str(e)}"
...
def process_video(video_path, output_dir, pose_config, use_gpu=True):
    """
    ...
    if use_gpu:
        try:
            ...
            print("Using GPU (NVIDIA CUDA) for processing")
        except Exception as e:
            print(f"Warning: Could not use GPU delegate: {e}")
...
class DeviceSelectionDialog(tk.simpledialog.Dialog):
    ...
    def apply(self):
        ...
        if self.gpu_available and selected == "gpu":
            self.result = "gpu"
        else:
            self.result = "cpu"
...
def process_videos_in_directory(existing_root=None):
    ...
    if gpu_available:
        ...
        if gpu_test_result[0]:
            print("✓ MediaPipe GPU delegate test passed")
        else:
            print(f"⚠ MediaPipe GPU delegate test failed: {gpu_test_result[1]}")
            gpu_available = False  # Disable GPU option if test fails
    else:
    ...
    for i, video_file in enumerate(video_files, 1):
        ...
        try:
            process_video(video_file, output_dir, pose_config, use_gpu=use_gpu)
        except Exception as e:
