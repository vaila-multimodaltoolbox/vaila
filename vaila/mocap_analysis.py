...
def analyze_mocap_fullbody_data():
    ...
    base_dir_processed_data = os.path.join(save_directory, f'MocapFull_{current_time}/processed_data')
    ...
    if use_anatomical:
        ...
        if not anatomical_dir:
            messagebox.showerror("No Directory Selected", "No directory selected for anatomical files. Exiting.")
            root.destroy()
            return
    ...
    messagebox.showinfo("Select Headers", "Please select 24 headers in the following order:\n"
    ...
    for idx, file_name in enumerate(file_names):
        ...
        axes[0].plot(time, trunk_euler_angles[:, 0], label='Trunk X [Ext. backward (+) Flex. forward (-)]', color='red')
        axes[0].plot(time, pelvis_euler_angles[:, 0], label='Pelvis X [Ext. backward (+) Flex. forward (-)]', linestyle='--', color='red')
        if use_anatomical and file_name in anatomical_data:
            axes[0].axhline(trunk_euler_angles_anat[0], color='gray', linestyle='-', label='Anatomical Trunk X')
            axes[0].axhline(pelvis_euler_angles_anat[0], color='gray', linestyle='--', label='Anatomical Pelvis X')
        axes[0].set_xlabel('Time (s)')
        axes[0].set_ylabel('X: Ext(+)/Flex(-) (degrees)')
        axes[0].legend()
        axes[0].set_title(f'Euler Angles - {file_name} (X-axis)')

        axes[1].plot(time, trunk_euler_angles[:, 1], label='Trunk Y [Side Bending Right (+) Side Bending Left (-)]', color='green')
        axes[1].plot(time, pelvis_euler_angles[:, 1], label='Pelvis Y [Side Bending Right (+) Side Bending Left (-)]', linestyle='--', color='green')
        if use_anatomical and file_name in anatomical_data:
            axes[1].axhline(trunk_euler_angles_anat[1], color='gray', linestyle='-', label='Anatomical Trunk Y')
            axes[1].axhline(pelvis_euler_angles_anat[1], color='gray', linestyle='--', label='Anatomical Pelvis Y')
        axes[1].set_xlabel('Time (s)')
        axes[1].set_ylabel('Y: Side Bending R(+)/L(-) (degrees)')
        axes[1].legend()
        axes[1].set_title(f'Euler Angles - {file_name} (Y-axis)')

        axes[2].plot(time, trunk_euler_angles[:, 2], label='Trunk Z [Axial Rot. Right (+) Axial Rot. Left (-)]', color='blue')
        axes[2].plot(time, pelvis_euler_angles[:, 2], label='Pelvis Z [Axial Rot. Right (+) Axial Rot. Left (-)]', linestyle='--', color='blue')
        if use_anatomical and file_name in anatomical_data:
            axes[2].axhline(trunk_euler_angles_anat[2], color='gray', linestyle='-', label='Anatomical Trunk Z')
            axes[2].axhline(pelvis_euler_angles_anat[2], color='gray', linestyle='--', label='Anatomical Pelvis Z')
        ...
        save_results_to_csv(base_dir_processed_data, time, trunk_euler_angles, pelvis_euler_angles, base_name)
