...
# Function to detect scientific notation in the data
def detect_scientific_notation(file_path):
    try:
        df = pd.read_csv(file_path, nrows=5)
        for col in df.columns:
            if df[col].dtype == object:
                if df[col].str.contains('e|E').any():
                    return True
    except Exception as e:
        print(f"Error detecting scientific notation in {file_path}: {e}")
    return False

# Function to save the DataFrame, preserving scientific notation if present
def save_dataframe(df, file_path, columns, scientific_notation):
    float_format = '%.12e' if scientific_notation else None
...
def reshapedata(file_path, new_order, save_directory, suffix):
    try:
        print(f"Starting reshapedata for {file_path}")
        headers = get_headers(file_path)
        scientific_notation = detect_scientific_notation(file_path)
        
        # Read CSV with Dask to handle large data, specifying dtype
        dtypes = {col: 'float64' for col in headers}
        ...
        ddf_reordered = ddf.iloc[:, new_order_indices]

        print("First 5 rows of the reordered DataFrame:")
        print(ddf_reordered.head())
        ...
        save_dataframe(ddf_reordered.compute(), new_file_path, new_order, scientific_notation)
...
class ColumnReorderGUI(tk.Tk):
    def __init__(self, original_headers, file_names, directory_path):
        ...
        # Detect scientific notation in the first file
        scientific_notation = detect_scientific_notation(os.path.join(directory_path, file_names[0]))

        # Read CSV with Dask to handle large data, specifying dtype
        dtypes = {col: 'float64' for col in original_headers}
        self.df = dd.read_csv(os.path.join(directory_path, file_names[0]), dtype=dtypes).compute()

        self.title("Reorder CSV Columns")
        ...
        self.instructions = tk.Label(scrollable_frame, text="Click to select a Column and press Enter to reorder. Select and press 'd' to delete.\nPress 'm' to manually select range. Press 'l' to edit rows. Press Ctrl+S to save. Press Ctrl+Z to undo.\nPress Esc to save and exit.", font=("Arial", 14))
        ...
        self.header_frame.grid(row=1, column=0, columnspan=3, pady=10, padx=10, sticky="nsew")

        self.number_label = tk.Label(self.header_frame, text="Number", font=("Arial", 12, "bold"))
        self.number_label.grid(row=0, column=0, padx=(10, 5), pady=(10, 0))

        self.name_label = tk.Label(self.header_frame, text="Name", font=("Arial", 12, "bold"))
        self.name_label.grid(row=0, column=1, padx=(5, 10), pady=(10, 0))

        self.shape_label = tk.Label(self.header_frame, text=f"Shape: {self.df.shape}", font=("Arial", 12))
        ...
        self.convert_button = tk.Button(button_frame, text="Convert Units", command=self.convert_units)
        self.convert_button.grid(row=0, column=0, padx=5, pady=5, sticky="n")

        self.modify_labref_button = tk.Button(button_frame, text="Modify Lab Ref System", command=self.modify_labref)
    ...
    def swap_columns(self, event):
        ...
        if selected_idx:
            ...
            new_position = simpledialog.askinteger("Swap Column", f"Enter the new starting position for '{self.current_order[selected_idx[0]]}' (1-{len(self.current_order)}):", minvalue=1, maxvalue=len(self.current_order))
    ...
    def manual_selection(self, event=None):
        selection_range = simpledialog.askstring("Manual Selection", "Enter the selection range in the format 'start:end' (1-based indexing):")
        if selection_range:
            try:
                self.save_state()
                ...
                new_position = simpledialog.askinteger("Insert Position", f"Enter the position to insert the selected columns (1-{len(self.current_order)}):", minvalue=1, maxvalue=len(self.current_order))
                if new_position is not None:
                    new_position -= 1  # Adjust for 0-based indexing
                ...
                messagebox.showerror("Error", "Invalid format. Please enter the range as 'start:end'.")

    def edit_rows(self, event):
        row_range = simpledialog.askstring("Edit Rows", "Enter the row range to edit in the format 'start:end' (1-based indexing):")
        if row_range:
            try:
                start, end = map(int, row_range.split(':'))
                start -= 1  # Adjust for 0-based indexing
                end -= 1    # Adjust for 0-based indexing
                row_shape = (end - start + 1, len(self.current_order))  # Assuming all rows are equal length

                row_edit_window = tk.Toplevel(self)
                row_edit_window.title("Edit Rows")
                row_edit_window.geometry("600x400")

                shape_label = tk.Label(row_edit_window, text=f"Shape: {row_shape}", font=("Arial", 12))
                shape_label.pack(pady=10)

                button_frame = tk.Frame(row_edit_window)
                button_frame.pack(pady=10)

                save_button = tk.Button(button_frame, text="Save range", command=lambda: self.save_row_range(row_edit_window, start, end))
                save_button.grid(row=0, column=0, padx=5)

                delete_button = tk.Button(button_frame, text="Delete range", command=lambda: self.delete_row_range(row_edit_window, start, end))
                delete_button.grid(row=0, column=1, padx=5)

                close_button = tk.Button(button_frame, text="Close", command=row_edit_window.destroy)
    ...
    def save_row_range(self, window, start, end):
        ...
        scientific_notation = detect_scientific_notation(os.path.join(self.directory_path, self.file_names[0]))
        save_dataframe(row_df, new_file_path, row_df.columns, scientific_notation)
    ...
    def delete_row_range(self, window, start, end):
        ...
        scientific_notation = detect_scientific_notation(os.path.join(self.directory_path, self.file_names[0]))
        save_dataframe(deleted_df, new_file_path, deleted_df.columns, scientific_notation)
    ...
    def save_intermediate(self, event=None):
        try:
            ...
            for file_name in self.file_names:
                ...
                reshapedata(file_path, self.current_order, self.rearranged_path, "")
            messagebox.showinfo("Success", f"Intermediate save completed. Files are saved in {self.rearranged_path} with a timestamp.")
        except Exception as e:
    ...
    def save_and_exit(self, event=None):
        try:
            ...
            for file_name in self.file_names:
                file_path = os.path.join(self.directory_path, file_name)
                reshapedata(file_path, self.current_order, self.rearranged_path, "_final")
            messagebox.showinfo("Success", f"Reordering completed for all files. Final files are saved in {self.rearranged_path} with '_final' in the name.")
            self.destroy()
        except Exception as e:
    ...
    def convert_units(self):
        ...
        self.current_unit_listbox = tk.Listbox(convert_window, selectmode=tk.SINGLE, width=20, height=27, exportselection=False)  # Adjusted height
        ...
        self.target_unit_listbox = tk.Listbox(convert_window, selectmode=tk.SINGLE, width=20, height=27, exportselection=False)  # Adjusted height
        ...
        time_column_check = tk.Checkbutton(convert_window, variable=self.time_column_var)
        time_column_check.grid(row=3, column=0, columnspan=3, pady=5)

        # Convert button
        convert_button = tk.Button(convert_window, text="Convert", command=lambda: self.apply_conversion(convert_window))
    ...
    def apply_conversion(self, convert_window):
        ...
        if current_unit == target_unit:
            messagebox.showinfo("Information", "Selected units are the same. No conversion is needed.")
            return
        ...
        for file_name in self.file_names:
            file_path = os.path.join(self.directory_path, file_name)
            dtypes = {col: 'float64' for col in self.current_order}
            ddf = dd.read_csv(file_path, dtype=dtypes)
            if time_column:
                ddf.iloc[:, 1:] = ddf.iloc[:, 1:] * conversion_factor
            else:
                ddf = ddf * conversion_factor
            ...
            scientific_notation = detect_scientific_notation(file_path)
            save_dataframe(ddf.compute(), new_file_path, ddf.columns, scientific_notation)

        messagebox.showinfo("Success", f"Conversion from {current_unit} to {target_unit} applied successfully to all files. Files saved in {self.rearranged_path}.")
    ...
    def modify_labref(self):
        ...
        self.current_option_listbox = tk.Listbox(modify_window, selectmode=tk.SINGLE, width=70, height=4, exportselection=False)  # Increasing listbox width
        ...
        # Modify button
        modify_button = tk.Button(modify_window, text="Modify", command=lambda: self.apply_modify(modify_window))
    ...
    def apply_modify(self, modify_window):
        ...
        if selected_text.startswith("Custom"):
            custom_angles = simpledialog.askstring("Input", "Enter rotation angles in degrees for x, y, and z axes (e.g., [20, -90, 100]):")
            if custom_angles:
        ...
        if not selected_text:
            messagebox.showinfo("Information", "No option selected. Please select an option.")
            return
