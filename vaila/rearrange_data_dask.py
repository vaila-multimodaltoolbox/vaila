...
def reshapedata(file_path, new_order, save_directory, suffix):
    try:
        ...
        save_dataframe(
            ddf_reordered.compute(), new_file_path, new_order, scientific_notation
        )
...
class ColumnReorderGUI(tk.Tk):
    def __init__(self, original_headers, file_names, directory_path):
        ...
        scientific_notation = detect_scientific_notation(
            os.path.join(directory_path, file_names[0])
        )

        # Read CSV with Dask to handle large data, specifying dtype
        dtypes = {col: "float64" for col in original_headers}
        self.df = dd.read_csv(
            os.path.join(directory_path, file_names[0]), dtype=dtypes
        ).compute()
    ...
    def save_row_range(self, window, start, end):
        ...
        scientific_notation = detect_scientific_notation(
            os.path.join(self.directory_path, self.file_names[0])
        )
    ...
    def delete_row_range(self, window, start, end):
        ...
        save_dataframe(
            deleted_df, new_file_path, deleted_df.columns, scientific_notation
        )
    ...
    def save_and_exit(self, event=None):
        try:
            ...
            for file_name in self.file_names:
                file_path = os.path.join(self.directory_path, file_name)
                reshapedata(
                    file_path, self.current_order, self.rearranged_path, "_final"
                )
            messagebox.showinfo(
                "Success",
    ...
    def apply_conversion(self, convert_window):
        ...
        for file_name in self.file_names:
            ...
            scientific_notation = detect_scientific_notation(file_path)
            save_dataframe(
                ddf.compute(), new_file_path, ddf.columns, scientific_notation
            )
