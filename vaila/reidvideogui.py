...
if hasattr(sys, 'frozen'):
    os.environ['QT_PLUGIN_PATH'] = os.path.join(os.path.dirname(sys.executable), 'plugins', 'platforms')
elif 'VIRTUAL_ENV' in os.environ:
    os.environ['QT_QPA_PLATFORM_PLUGIN_PATH'] = os.path.join(os.environ['VIRTUAL_ENV'], 'Lib', 'site-packages', 'PySide6', 'plugins', 'platforms')
...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QGroupBox, QGridLayout, QSplitter, QFrame, QMessageBox)
from PySide6.QtCore import Qt, QTimer
from PySide6.QtGui import QImage, QPixmap

# Import functions from both source files
from reid_markers import (
    load_markers_file, save_markers_file, detect_markers, get_marker_coords,
    fill_gaps, merge_markers, swap_markers, detect_gaps
)
...
class CombinedMarkerGUI(QMainWindow):
        ...
        if self.df is not None:
            ...
            for marker_id in self.selected_markers:
                ...
                if x_values is not None and y_values is not None:
                    ...
                    self.canvas.ax_x.plot(frames, x_values, label=f"Marker {marker_id}", color=normalized_color)
                    self.canvas.ax_y.plot(frames, y_values, label=f"Marker {marker_id}", color=normalized_color)
            ...
            if x_col in self.df.columns and y_col in self.df.columns:
                if frame_num < len(self.df):
                    ...
                    if not pd.isna(x) and not pd.isna(y):
                        ...
                        cv2.putText(frame, str(marker_id), (int(x)+10, int(y)-10), 
    ...
    def fill_gaps(self):
        ...
        if not self.selected_markers:
            QMessageBox.information(self, "Selection Needed", "Please select at least one marker to fill gaps.")
            return
        ...
        QMessageBox.information(self, "Success", f"Filled gaps for {len(self.selected_markers)} markers.")
    
    def merge_markers(self):
        """Merge selected markers"""
        if len(self.selected_markers) < 2:
            QMessageBox.information(self, "Selection Needed", "Please select at least 2 markers to merge.")
            return
    ...
    def swap_markers(self):
        ...
        if len(self.selected_markers) != 2:
            QMessageBox.information(self, "Selection Needed", "Please select exactly 2 markers to swap.")
            return
