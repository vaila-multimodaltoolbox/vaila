...
"""
    ...
    def setup_ui(self):
        ...
        header = tk.Label(
            main_frame,
            text="Brainstorm: Voice to AI Creative Assistant",
            font=("Arial", 16, "bold"),
        )
        header.pack(pady=10)
        ...
        tk.Button(
            help_frame,
            text="Workflow Guide",
            command=self.show_workflow_guide,
            bg="#607D8B",
            fg="white",
            font=("Arial", 10),
        ).pack()
        ...
        tk.Button(
            ...
            command=self.load_transcription,
            bg="#795548",
            fg="white",
            font=("Arial", 10, "bold"),
        ).pack(side="left", padx=5)
        ...
        tk.Button(
            text_header,
            text="Save Text",
            command=self.save_transcription_edits,
            bg="#4CAF50",
            fg="white",
            font=("Arial", 9),
        ).pack(side="right", padx=5)
        ...
        self.text_display = scrolledtext.ScrolledText(
            text_frame, width=100, height=8, font=("Arial", 10)
        )
        ...
        tk.Button(
            gen_buttons,
            text="Batch Music Generation",
            command=self.batch_generate_music,
            bg="#00BCD4",
            fg="white",
            font=("Arial", 10, "bold"),
        ).pack(side="left", padx=5)
        ...
        tk.Button(
            music_buttons,
            text="Execute Code",
            command=self.execute_music_code,
            bg="#4CAF50",
            fg="white",
            font=("Arial", 9),
        ).pack(side="left", padx=5)
        ...
        self.music_display = scrolledtext.ScrolledText(
            music_frame, width=100, height=15, font=("Courier", 9)
        )
        ...
        tk.Button(ideas_buttons, text="Save Ideas", command=self.save_ideas).pack(
            side="left", padx=5
        )

        self.ideas_display = scrolledtext.ScrolledText(
            ideas_frame, width=100, height=15, font=("Arial", 10)
        )
    ...
    def choose_output_directory(self):
        ...
        if directory:
            ...
            self.show_message(
                "info",
                "Directory Selected",
                "Please click 'New Session' to start working.",
            )
        ...
        try:
            ...
            if self._convert_wav_to_mp3(wav_file, mp3_file):
                ...
                self.status_label.config(
                    text=f"Audio saved as MP3: {os.path.basename(mp3_file)}", fg="green"
                )
            else:
                print("[DEBUG] MP3 conversion failed, keeping WAV")
                self.status_label.config(
                    text=f"Audio saved as WAV: {os.path.basename(wav_file)}", fg="green"
                )
            ...
            if mp3_created:
                self.show_message(
                    "info",
                    "Success",
                    f"Audio recorded and saved!\nWAV: {wav_file}\nMP3: {mp3_file}",
                )
            else:
                self.show_message(
                    "info",
                    "Success",
                    f"Audio recorded and saved as WAV!\n{wav_file}\n\nNote: MP3 conversion requires FFmpeg",
                )
    ...
    def load_transcription(self):
        ...
        if file_path:
            try:
                ...
                self.status_label.config(
                    text=f"Loaded transcription: {os.path.basename(file_path)}",
                    fg="blue",
                )
    ...
    def text_to_audio(self):
        ...
        try:
            ...
            if tts_success:
                self.status_label.config(
                    text=f"TTS audio saved: {os.path.basename(audio_file)}", fg="green"
                )
                self.show_message(
                    "info",
                    "TTS Success",
                ...
                self.show_message(
                    "error",
                    "TTS Failed",
                    "Could not generate audio. Please install pyttsx3 or gtts:\npip install pyttsx3 gtts",
                )
        ...
        try:
            ...
            if os.path.exists(mp3_file):
                try:
                    ...
                    result = subprocess.run(
                        ["ffmpeg", "-i", mp3_file, audio_file, "-y"],
                        capture_output=True,
                        timeout=10,
                    )
                    if result.returncode == 0:
...
"""
            ...
            try:
                result = subprocess.run(
                    ["cscript", "//NoLogo", vbs_file], capture_output=True, timeout=30
                )
                os.unlink(vbs_file)
    ...
    def transcribe_audio(self):
        ...
        try:
            ...
            if text:
                ...
                self.status_label.config(
                    text=f"Transcription completed! Language: {detected_lang}",
                    fg="green",
                )
                self.show_message(
                    "info",
    ...
    def batch_transcribe(self):
        ...
        directory = filedialog.askdirectory(
            title="Select Directory with Audio Files", parent=self.root
        )
        ...
        try:
            ...
            generate_music = self.show_message(
                "yesno",
                "Generate Music",
                ...
                try:
                    ...
                    if text:
                        ...
                        results_summary.append(
                            f"SUCCESS: {filename} -> {base_name}_transcription.txt"
                        )
                        ...
                        if generate_music:
                            try:
                                ...
                                results_summary.append(
                                    f"   Music code generated: {base_name}_music.py"
                                )
                                ...
                                try:
                                    ...
                                    if result.returncode == 0:
                                        ...
                                        results_summary.append(
                                            f"   WARNING: MIDI generation failed: {result.stderr[:100]}"
                                        )
                                except Exception as midi_error:
                                    results_summary.append(
                                        f"   WARNING: Could not execute music code: {str(midi_error)[:100]}"
                                    )

                            except Exception as music_error:
                                results_summary.append(
                                    f"   ERROR: Music generation failed: {str(music_error)[:100]}"
                                )
                    else:
                        failed += 1
...
"""
            ...
            self.status_label.config(
                text=f"Batch completed: {successful} success, {failed} failed",
                fg="green",
            )
            ...
            self.show_message(
                ...
                f"Failed to process batch transcription:\n{str(e)}",
            )
    ...
    def generate_music(self):
        ...
        if not text:
            self.show_message(
                "warning",
                "Warning",
                "Please transcribe audio or enter some text first.",
            )
            return
...
"""
    ...
    def generate_mp3(self):
        ...
        if not self.session_dir:
            self.show_message(
                "error", "Error", "No session created. Please create a session first."
            )
            return
        ...
        try:
            ...
            if not os.path.exists(scripts_dir):
                self.show_message(
                    "error",
                    "Error",
                    "No scripts directory found. Please generate and execute music code first.",
                )
                return
            ...
            if not midi_files:
                self.show_message(
                    "error",
                    "Error",
                    "No MIDI files found. Please execute music code first to generate MIDI files.",
                )
                return
...
"""
    ...
    def _try_timidity_conversion(self, midi_file, mp3_file):
        ...
        try:
            ...
            result = subprocess.run(
                ["timidity", "--version"], capture_output=True, text=True, timeout=5
            )
            if result.returncode != 0:
                return False
    ...
    def open_image_url(self):
        ...
        self.show_message(
            "info",
            "Info",
            "Image URL opening functionality would be implemented with actual API integration.",
        )
    ...
    def batch_generate_music(self):
        ...
        directory = filedialog.askdirectory(
            title="Select Directory with Transcription Files (.txt)", parent=self.root
        )
        ...
        try:
            ...
            if not text_files:
                self.show_message(
                    "warning",
                    "No Text Files",
                    "No .txt files found in the selected directory.",
                )
                return
...
"""
            ...
            self.show_message(
                "error",
                "Batch Music Error",
                f"Failed to process batch music generation:\n{str(e)}",
            )
