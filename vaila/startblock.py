...
from tkinter import Tk, Toplevel, filedialog, messagebox, ttk, simpledialog
...
def spectral_analysis_baseline(signal_data, time_data, fs, last_second=1.0):
    ...
    if total_duration < last_second:
        ...
        print(f"  Signal duration ({total_duration:.2f} s) is shorter than requested ({last_second} s). Using all data.")
    else:
        # Extract last second
    ...
    if len(baseline_data) < 10:
        print(f"  Warning: Baseline segment too short ({len(baseline_data)} samples). Using default cutoff.")
        return {
    ...
    try:
        freqs, psd = welch(baseline_data_centered, fs=fs, nperseg=nperseg, 
    ...
    if dominant_frequency < 2.0:
        suggested_cutoff = 10.0
        print(f"  Low dominant frequency ({dominant_frequency:.2f} Hz) detected. May be drift. Using default cutoff.")
...
def process_single_file(csv_file, show_plot=True, output_dir=None, 
                        sampling_rate_hz=80.0, baseline_window=50, 
                        negative_threshold_offset=0.5):
    """
    ...
    y = y_raw / 1000.0
    print(f"Data converted from grams to kilograms (÷ 1000)")
    ...
    print(f"\n{'='*60}")
    print(f"Reaction Time Analysis:")
    print(f"{'='*60}")
    print(f"  Surge start (first negative): {surge_start_time:.2f} ms ({surge_start_time/1000:.3f} s)")
    ...
    print(f"  Reaction Time (End Surge → Peak): {reaction_time_end_surge_to_peak:.2f} ms ({reaction_time_end_surge_to_peak/1000:.3f} s)")
    print(f"{'='*60}")
    ...
    ax1.axvline(x=surge_start_time, color='brown', linestyle='--', linewidth=2, alpha=0.7, label='Surge start (max neg derivative)')
    ax1.axvline(x=minimum_time, color='orange', linestyle=':', linewidth=1.5, alpha=0.7, label='Minimum (bottom of U)')
    ax1.axvline(x=end_surge_time, color='red', linestyle='--', linewidth=2, alpha=0.7, label='End of surge (max pos derivative)')
    ax1.axvline(x=positive_peak_time, color='purple', linestyle='--', linewidth=2, alpha=0.7, label='Positive peak')
    # Add baseline line
    ax1.axhline(y=baseline_initial, color='green', linestyle=':', linewidth=1, alpha=0.5, label='Initial baseline')
    # Mark minimum point
    ...
    ax1.text(0.02, 0.02, reaction_time_text, 
    ...
    ax2.plot(x[start_idx:end_idx], y_data[start_idx:end_idx], 'b-', linewidth=2, label='Raw data (kg)')
    ax2.axhline(y=0, color='k', linestyle='--', linewidth=1, alpha=0.5, label='Zero')
    ax2.axhline(y=baseline_initial, color='green', linestyle=':', linewidth=1, alpha=0.5, label='Initial baseline')
    ax2.plot(surge_start_time, y_data[surge_start_idx], 's', color='brown', markersize=12, label='Surge start (max neg derivative)')
    ax2.plot(minimum_time, minimum_value, 'o', color='orange', markersize=12, label=f'Minimum ({minimum_value:.2f} kg)')
    ax2.plot(end_surge_time, y_data[end_surge_idx], 's', color='red', markersize=12, label='End of surge (max pos derivative)')
    ax2.plot(positive_peak_time, y_data[positive_peak_idx], 'o', color='purple', markersize=12, label=f'Positive peak ({positive_peak_value:.2f} kg)')
    ...
    ax2.set_title('Zoom: Surge Start → Minimum → End Surge → Positive Peak', fontsize=14, fontweight='bold')
    ...
    if len(positive_force_x_filtered) > 0:
        ax3.plot(positive_force_x_filtered, positive_force_y_filtered, 'g-', linewidth=2.5, label='Positive force')
        ax3.fill_between(positive_force_x_filtered, 0, positive_force_y_filtered, alpha=0.3, color='green', label='Area under curve')
        ax3.axhline(y=0, color='k', linestyle='-', linewidth=1, alpha=0.5)
        ax3.plot(positive_peak_time, positive_peak_value, 'o', color='purple', markersize=12, label=f'Positive peak ({positive_peak_value:.2f} kg)')
        ...
        positive_duration = 0
        ax3.text(0.5, 0.5, 'No positive force detected', 
                 transform=ax3.transAxes, ha='center', va='center', fontsize=12)
    
    ax3.set_xlabel(df.columns[0] if len(df.columns) > 0 else 'Time (ms)', fontsize=12)
    ax3.set_ylabel(df.columns[1] + ' (kg)' if len(df.columns) > 1 else 'Positive Force (kg)', fontsize=12)
...
def batch_process_directory(directory_path, sampling_rate_hz=80.0, baseline_window=50,
                            negative_threshold_offset=0.5, output_dir=None):
    """
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        try:
            ...
            result = process_single_file(csv_file, show_plot=False, 
...
def run_startblock_gui():
    ...
    config_path_label = ttk.Label(config_frame, text="No config loaded", foreground="gray", wraplength=350)
    ...
    ttk.Button(config_frame, text="Load Config File", command=load_config_file, width=20).grid(row=1, column=0, padx=5, pady=5)
    ttk.Button(config_frame, text="Create Config File", command=create_config, width=20).grid(row=1, column=1, padx=5, pady=5)
    ...
    input_path_label = ttk.Label(path_frame, text="No path selected", foreground="gray", wraplength=350)
    ...
    ttk.Button(path_frame, text="Select File", command=select_file, width=20).grid(row=1, column=0, padx=5, pady=5)
    ttk.Button(path_frame, text="Select Directory", command=select_directory, width=20).grid(row=1, column=1, padx=5, pady=5)
        ...
        try:
            ...
            if input_path_obj.is_file():
                ...
                process_single_file(
                    ...
                    negative_threshold_offset=config['detection'].get('negative_threshold_offset', 0.5),
    ...
    ttk.Button(button_frame, text="Run Analysis", command=run_analysis, width=25).grid(row=0, column=0, padx=5)
    ttk.Button(button_frame, text="Cancel", command=root.destroy, width=25).grid(row=0, column=1, padx=5)
    ...
    if len(sys.argv) > 1:
        ...
        if csv_file.exists():
            ...
            process_single_file(
                ...
                negative_threshold_offset=defaults['detection'].get('negative_threshold_offset', 0.5)
            )
        else:
            print(f"Error: File not found: {csv_file}")
            sys.exit(1)
