...
class VerticalGroundReactionForceGaitEventDetectorTest(unittest.TestCase):
    ...
    def test_Gait(self):
        ...
        vgrfged.SetAcquisitionInformation(
            reader.GetOutput().GetFirstFrame(), reader.GetOutput().GetPointFrequency(), ""
        )
        output = vgrfged.GetOutput()
        output.Update()
        ...
        self.assertEqual(
            ev.GetDetectionFlags(), btk.btkEvent.Automatic | btk.btkEvent.FromForcePlatform
        )
    ...
    def test_PluginC3D_ROI(self):
        ...
        vgrfged.SetRegionOfInterest(
            60 - reader.GetOutput().GetFirstFrame(), 120 - reader.GetOutput().GetFirstFrame()
        )
        output = vgrfged.GetOutput()
        output.Update()
