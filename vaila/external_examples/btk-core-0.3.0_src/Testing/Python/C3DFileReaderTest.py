...
class C3DFileReaderTest(unittest.TestCase):
    ...
    def test_Sample09_PlugInC3D(self):
        ...
        self.assertEqual(
            ...
            acq.GetMetaData().GetChild("POINT").GetChild("FRAMES").GetInfo().ToInt(0),
        )
    ...
    def test_Sample22_BKINtechnologies(self):
        ...
        self.assertEqual(
            acq.GetEvent(0).GetDescription(), "subject must wait at centre starting now"
        )
    ...
    def test_UTF8(self):
        ...
        if platform.system() != "Windows":
            reader = btk.btkAcquisitionFileReader()
            reader.SetFilename(
                _TDDConfigure.C3DFilePathIN + "others/Я могу есть стекло/оно мне не вредит.c3d"
            )
            reader.Update()
            acq = reader.GetOutput()
