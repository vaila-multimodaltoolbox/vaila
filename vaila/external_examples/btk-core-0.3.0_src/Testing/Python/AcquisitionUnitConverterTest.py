...
class AcquisitionUnitConverterTest(unittest.TestCase):
    ...
    def test_ConversionFromFile(self):
        ...
        for j in range(0, acq.GetPointNumber()):
            ...
            for i in range(1, 50):
                self.assertAlmostEqual(
                    acq.GetPoint(j).GetValues()[i, 0] * s, acq2.GetPoint(j).GetValues()[i, 0], 10
                )
                self.assertAlmostEqual(
                    acq.GetPoint(j).GetValues()[i, 1] * s, acq2.GetPoint(j).GetValues()[i, 1], 10
                )
                self.assertAlmostEqual(
                    acq.GetPoint(j).GetValues()[i, 2] * s, acq2.GetPoint(j).GetValues()[i, 2], 10
                )
        for j in range(0, acq.GetAnalogNumber()):
            ...
            for i in range(1, 50):
                self.assertAlmostEqual(
                    acq.GetAnalog(j).GetValues()[i] * s, acq2.GetAnalog(j).GetValues()[i], 10
                )

        self.assertAlmostEqual(
            acq.GetMetaData().GetChild("SEG").GetChild("MARKER_DIAMETER").GetInfo().ToDouble(0)
            * 0.001,
            acq2.GetMetaData().GetChild("SEG").GetChild("MARKER_DIAMETER").GetInfo().ToDouble(0),
            5,
        )
