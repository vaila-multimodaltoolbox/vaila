...
class MergeAcquisitionFilterTest(unittest.TestCase):
    ...
    def test_TwinsFromFile_Concat(self):
        ...
        for i in range(0, input.GetPointNumber()):
            ...
            self.assertEqual(
                output.GetPoint(i + input.GetPointNumber()).GetLabel(),
                input.GetPoint(i).GetLabel() + "_2",
            )
        for i in range(0, input.GetAnalogNumber()):
            self.assertEqual(output.GetAnalog(i).GetLabel(), input.GetAnalog(i).GetLabel())
            self.assertEqual(
                output.GetAnalog(i + input.GetAnalogNumber()).GetLabel(),
                input.GetAnalog(i).GetLabel() + "_2",
            )
        ...
        corners2Val = (
            input.GetMetaData().GetChild("FORCE_PLATFORM").GetChild("CORNERS").GetInfo().ToDouble()
        )
        for i in range(0, 24):
        ...
        channel2Val = (
            input.GetMetaData().GetChild("FORCE_PLATFORM").GetChild("CHANNEL").GetInfo().ToInt()
        )
        for i in range(0, 12):
        ...
        origin2Val = (
            input.GetMetaData().GetChild("FORCE_PLATFORM").GetChild("ORIGIN").GetInfo().ToDouble()
        )
        for i in range(0, 6):
