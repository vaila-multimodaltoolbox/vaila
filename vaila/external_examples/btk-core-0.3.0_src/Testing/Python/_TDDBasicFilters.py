...
def suite():
    ...
    suite.addTest(
        unittest.makeSuite(
            SeparateKnownVirtualMarkersFilterTest.SeparateKnownVirtualMarkersFilterTest
        )
    )
    suite.addTest(unittest.makeSuite(SubAcquisitionFilterTest.SubAcquisitionFilterTest))
    suite.addTest(
        unittest.makeSuite(
            VerticalGroundReactionForceGaitEventDetectorTest.VerticalGroundReactionForceGaitEventDetectorTest
        )
    )
    return suite
