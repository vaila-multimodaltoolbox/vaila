...
def recursive_batch_cut(target_root, dry_run=False):
    ...
    for sync_file in sync_files_found:
        ...
        if not is_sync or not sync_data:
            print(f"  [yellow]Sync file {sync_file.name} does not seem to follow the expected format. Skipping.[/]")
            continue
        ...
        if dry_run:
            for vid, info in sync_data.items():
                if (sync_file.parent / vid).exists():
                    print(f"    [white]- Would process: {vid} -> {info['new_name']} (Frames: {info['initial_frame']} to {info['final_frame']})[/]")
                else:
