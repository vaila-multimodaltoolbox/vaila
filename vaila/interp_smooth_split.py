...
class InterpolationConfigDialog:
    ...
    def create_dialog_content(self):
        ...
        tk.Label(
            self.header_frame, text="Interpolation & Smoothing Config", font=("Arial", 16, "bold")
        ).pack()
        tk.Label(
            self.header_frame,
        ...
        self.main_split = tk.PanedWindow(
            self.window, orient=tk.HORIZONTAL, sashrelief=tk.RAISED, sashwidth=4
        )
        ...
        tk.Button(
            self.buttons_frame, text="💾 Save Template", command=self.create_toml_template, width=15
        ).grid(row=0, column=0, padx=5, pady=5)
        tk.Button(
            self.buttons_frame, text="📂 Load TOML", command=self.load_toml_config, width=15
        ...
        tk.Button(
            ...
            bg="#a8df65",
            font=("Arial", 10, "bold"),
        ).grid(row=1, column=0, columnspan=2, sticky="ew", padx=5, pady=(15, 5))

        self.build_analysis_panel()

    def build_interp_tab(self):
        tk.Label(
            self.tab_interp, text="Select Interpolation Method:", font=("Arial", 10, "bold")
        ...
        self.interp_combo = ttk.Combobox(
            self.tab_interp, values=list(methods.values()), state="readonly"
        )
        ...
        tk.Label(self.tab_interp, text="Max Gap Size (frames):", font=("Arial", 10, "bold")).pack(
            anchor="w", pady=(15, 5)
        )
    ...
    def build_smooth_tab(self):
        tk.Label(self.tab_smooth, text="Select Smoothing Method:", font=("Arial", 10, "bold")).pack(
            anchor="w", pady=(10, 5)
        )
        ...
        self.smooth_combo = ttk.Combobox(
            self.tab_smooth, values=list(methods.values()), state="readonly"
        )
    ...
    def build_all_param_frames(self):
        ...
        tk.Label(f5, text="Tip: 4-10 Hz for biomechanics", fg="gray", font=("Arial", 9)).pack(
            anchor="w", pady=(0, 10)
        )
    ...
    def build_general_tab(self):
        tk.Label(self.tab_general, text="Padding (%):", font=("Arial", 10, "bold")).pack(
            anchor="w", pady=(10, 5)
        )
        tk.Entry(self.tab_general, textvariable=self.padding_var).pack(fill="x", pady=5)

        tk.Label(self.tab_general, text="Split Dataset:", font=("Arial", 10, "bold")).pack(
            anchor="w", pady=(15, 5)
        )
        tk.Checkbutton(self.tab_general, text="Enable Splitting", variable=self.split_var).pack(
            anchor="w", pady=5
        )
    ...
    def build_analysis_panel(self):
        ...
        tk.Button(self.analysis_top, text="📊 Load Test CSV", command=self.load_test_data).pack(
            side="left", padx=5
        )
        ...
        if file_path:
            try:
                ...
                self.col_combo = ttk.Combobox(
                    self.col_frame,
                    values=numeric_cols,
                    textvariable=self.test_col_var,
                    state="readonly",
                )
                self.col_combo.pack(side="left", padx=5)
                self.col_combo.bind(
                    "<<ComboboxSelected>>", lambda e: self.run_analysis(self.test_col_var.get())
                )
