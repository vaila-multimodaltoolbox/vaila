        ...
        if mode == 1:  # 1D mode
            ...
            for j in range(0, n_features, 2):
                ...
                initial_state_covariance = np.array([
                    [0.1, 0, 0, 0, 0, 0],    # x uncertainty
                    [0, 0.1, 0, 0, 0, 0],    # y uncertainty
                    [0, 0, 1.0, 0, 0, 0],    # vx uncertainty (high)
                    [0, 0, 0, 1.0, 0, 0],    # vy uncertainty (high)
                    [0, 0, 0, 0, 2.0, 0],    # ax uncertainty (very high)
                    [0, 0, 0, 0, 0, 2.0]     # ay uncertainty (very high)
                ])
                ...
                transition_covariance = np.array([
                    ...
                    [0, 0, 0.1, 0, 0, 0],    # vx process noise
                    [0, 0, 0, 0.1, 0, 0],    # vy process noise
                    [0, 0, 0, 0, 0.2, 0],    # ax process noise
                    [0, 0, 0, 0, 0, 0.2]     # ay process noise
                ])
                
                # Measurement noise (small for position measurements)
                observation_covariance = np.array([
                    [0.01, 0],         # x measurement noise
                    [0, 0.01]          # y measurement noise
                ])
                ...
                filtered_data[:, j+1] = smoothed_state_means[:, 1]  # y position
