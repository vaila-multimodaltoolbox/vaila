...
def run_rec3d_one_dlt3d():
    ...
    for file in pixel_files:
        ...
        df = pd.read_csv(file)
        print(
            f"[DEBUG] Pixel data from {os.path.basename(file)} loaded with shape:",
            df.shape,
        )
    ...
    for idx, df in enumerate(pixel_dfs, start=1):
        ...
        print(
            f"[DEBUG] Pixel file {idx} ({os.path.basename(pixel_files[idx-1])}) frames:",
            frames,
        )
        ...
        for marker in range(1, num_markers + 1):
            ...
            for df in pixel_dfs:
                ...
                try:
                    ...
                    print(
                        f"[DEBUG] Error parsing marker {marker} in frame {frame}: {e}"
                    )
                    valid_marker = False
                    break
            ...
            if not valid_marker or len(pixel_obs_list) != len(dlt_params_list):
                print(
                    f"[DEBUG] Marker {marker} in frame {frame} has invalid data. Appending NaNs."
                )
                row_results.extend([np.nan, np.nan, np.nan])
            else:
                point3d = rec3d_multicam(dlt_params_list, pixel_obs_list)
                print(
                    f"[DEBUG] Reconstructed 3D point for marker {marker} in frame {frame}:",
                    point3d,
                )
    ...
    if not output_dir:
        messagebox.showerror(
            "Erro", "Nenhum diretório selecionado. Operação cancelada."
        )
        return
    ...
    if point_rate is None:
        messagebox.showerror(
            "Error", "Point data rate is not defined. Operation cancelled."
        )
        return
