...
DEFAULT_SKELETON_JSON = (
    Path(__file__).resolve().parent.parent / "tests" / "tugturn" / "skeleton_pose_mediapipe.json"
)
...
def load_mediapipe_pose_connections(json_path: Path = DEFAULT_SKELETON_JSON):
    ...
    try:
        ...
        if parsed:
            return parsed
        print(
            f"Warning: skeleton JSON '{json_path}' has no valid connections. Using embedded default."
        )
    except Exception as e:
        print(f"Warning: could not load skeleton JSON '{json_path}': {e}. Using embedded default.")

    parsed_default = _parse_pose_connections(
        DEFAULT_SKELETON_CONNECTIONS_JSON.get("connections", [])
    )
    if parsed_default:
...
def calculate_absolute_inclination_3d(p_top, p_bottom, vertical_vector=None):
    ...
    v_seg_unit = np.divide(
        v_seg, v_seg_norm, out=np.zeros_like(v_seg, dtype=float), where=v_seg_norm != 0
    )
...
class TUGAnalyzer:
    ...
    def calculate_spatiotemporal_params(
        self, fps: float = 30.0, phases: dict | None = None
    ) -> dict:
        """
    ...
    def segment_tug_phases(self) -> dict:
        ...
        if abs(y_peak_val - y_turn) > y_tol * 2.0:
            print(
                f"Warning: Y peak ({y_peak_val:.2f} m) far from expected turn zone "
        ...
        while stop_search > sts_end and (
            mid_trunk_vy[stop_search] < speed_thresh and trunk_speed_xy[stop_search] < speed_thresh
        ):
            stop_search -= 1
...
def generate_plotly_report(
    ...
    fig3 = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("First gait sagittal events (Y-Z)", "Second gait sagittal events (Y-Z)"),
    )
    ...
    for vc_key, title_suffix in vc_titles:
        ...
        for lo, hi, col, lbl in [
            ...
            fig_time.add_hrect(
                ...
                fillcolor=col,
                line_width=0,
                annotation_text=lbl if lo < 180 else "",
...
def generate_phase_skeleton_gifs(
    ...
    for phase_name, val in ordered_phase_ranges(phases):
        ...
        phase_videos.append(
            {"Phase": canonical_phase_name(phase_name), "Video_Path": str(gif_path.resolve())}
        )
...
def generate_matplotlib_report(
    ...
    ax7.scatter(
        trunk_y[trunk_vy > 0], trunk_z[trunk_vy > 0], color="darkmagenta", s=15, label="Positive Vy"
    )
    ax7.scatter(
        trunk_y[trunk_vy <= 0],
        trunk_z[trunk_vy <= 0],
    ...
    for vc_key, title_suffix in vc_titles:
        ...
        ax_vc_timeline.scatter(
            movement_pct, gamma, c=point_colors, s=18, edgecolors="none", zorder=2
        )
...
def calculate_axial_vector_coding(
    analyzer: "TUGAnalyzer", fps: float, start_s: float, end_s: float
) -> dict:
    """
    ...
    counts = {
        p: patterns.count(p) for p in ("In_Phase", "Anti_Phase", "Proximal_Phase", "Distal_Phase")
    }
...
def calculate_limb_vector_coding_y(
...
def process_tug_file(
        ...
        for f in analyzer.gait_events.get(leg, {}).get("HS", []):
            ...
            if sts_s <= t < sts_e:
                ...
                if not valid_spatial:
                    print(
                        f"Notice: Filtered out {leg} HS at {t:.2f}s during First gait (Y={y_pos:.2f}m outside [{y_min_valid}, {y_max}])"
                    )
                    continue
                ...
                if not valid_spatial:
                    print(
                        f"Notice: Filtered out {leg} HS at {t:.2f}s during Second gait (Y={y_pos:.2f}m outside [{y_min_valid}, {y_max}])"
                    )
                    continue
            ...
            step_data = {"Time_s": t, "Y_m": round(y_pos, 3), "Side": leg, "Phase": phase_label}
            for k in [
                "Stance_Time_s",
                "Swing_Time_s",
                "Step_Length_m",
        ...
        if abs(s["Time_s"] - prev_s["Time_s"]) < 0.01:
            if s["Phase"] == "first_gait":
                if s["Y_m"] > prev_s["Y_m"]:
                    dedup_steps[-1] = s
                    print(
                        f"Notice: Removed simultaneous {prev_s['Side']} step at {prev_s['Time_s']:.3f}s (kept {s['Side']}, further ahead at Y={s['Y_m']:.2f})"
                    )
                else:
                    print(
                        f"Notice: Removed simultaneous {s['Side']} step at {s['Time_s']:.3f}s (kept {prev_s['Side']}, further ahead at Y={prev_s['Y_m']:.2f})"
                    )
            elif s["Phase"] == "second_gait":
                if s["Y_m"] < prev_s["Y_m"]:
                    dedup_steps[-1] = s
                    print(
                        f"Notice: Removed simultaneous {prev_s['Side']} step at {prev_s['Time_s']:.3f}s (kept {s['Side']}, further back at Y={s['Y_m']:.2f})"
                    )
                else:
                    print(
                        f"Notice: Removed simultaneous {s['Side']} step at {s['Time_s']:.3f}s (kept {prev_s['Side']}, further back at Y={prev_s['Y_m']:.2f})"
                    )
            else:
    ...
    def _append_vc_ts(res, metric_name):
        if res.get("gamma_deg"):
            for pct, g, pat in zip(
                res["Movement_Percent"], res["gamma_deg"], res["Coordination_Pattern"], strict=False
            ):
                vc_ts_data.append(
                    {
    ...
    if len(bck_seq) > 0:
        ...
        stats["Global"]["Second_1st_complete_step_inside"] = (
            bck_seq[1] if len(bck_seq) > 1 else None
        )
        ...
        results_data.update(
            {
                ...
                "XcoM_Dev_First_Gait_m": stats.get("Global", {}).get(
                    "XcoM_Deviation_First_Gait_m", 0
                ),
                "XcoM_Dev_Second_Gait_m": stats.get("Global", {}).get(
                    "XcoM_Deviation_Second_Gait_m", 0
                ),
                "First_1st_strike_inside_zone": stats.get("Global", {}).get(
                    "First_1st_strike_inside_zone", ""
                ),
                "First_1st_complete_step_inside": stats.get("Global", {}).get(
                    "First_1st_complete_step_inside", ""
                ),
                "First_Number_of_steps_inside": stats.get("Global", {}).get(
                    "First_Number_of_steps_inside", ""
                ),
                "First_Last_strike_inside_zone": stats.get("Global", {}).get(
                    "First_Last_strike_inside_zone", ""
                ),
                "First_Sequence_steps_inside": stats.get("Global", {}).get(
                    "First_Sequence_steps_inside", ""
                ),
                "Second_1st_strike_inside_zone": stats.get("Global", {}).get(
                    "Second_1st_strike_inside_zone", ""
                ),
                "Second_1st_complete_step_inside": stats.get("Global", {}).get(
                    "Second_1st_complete_step_inside", ""
                ),
                "Second_Number_of_steps_inside": stats.get("Global", {}).get(
                    "Second_Number_of_steps_inside", ""
                ),
                "Second_Last_strike_inside_zone": stats.get("Global", {}).get(
                    "Second_Last_strike_inside_zone", ""
                ),
                "Second_Sequence_steps_inside": stats.get("Global", {}).get(
                    "Second_Sequence_steps_inside", ""
                ),
            }
        )
        for prefix, res in [
...
def main():
    ...
    if not input_path:
        ...
        input_path = filedialog.askopenfilename(
            title="Select the TUGTURN CSV file to analyze", filetypes=[("CSV files", "*.csv")]
        )
        if not input_path:
    ...
    if output_path:
        ...
        try:
            if output_path_explicit.resolve() == input_dir.resolve():
                ...
                print(f"Output dir equals input dir; redirecting to: {output_path_explicit}")
        except Exception:
            pass
