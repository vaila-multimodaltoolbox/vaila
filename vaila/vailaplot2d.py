...
class PlotGUI:
    ...
    def _create_widgets(self):
        ...
        title_label = Label(
            main_frame, 
            text="vailá 2D Plotting Tool",
            font=("Arial", 16, "bold")
        )
        title_label.pack(pady=(0, 20))
        
        # Plot type selection frame
        plot_type_frame = LabelFrame(main_frame, text="Select Plot Type", padx=10, pady=10)
...
def select_and_plot():
    ...
    file_count_label = Label(status_frame, text=f"Files selected: {len(selected_files)}")
    file_count_label.pack(anchor="w")
    
    header_count_label = Label(status_frame, text=f"Headers selected: {len(selected_headers)}")
    ...
    Button(button_frame, text="Select File", command=on_select_file, width=12).pack(side="left", padx=5)
    Button(button_frame, text="Clear Selections", command=on_clear, width=12).pack(side="left", padx=5)
    Button(button_frame, text="Plot", command=on_plot, width=12).pack(side="left", padx=5)
    Button(button_frame, text="Add More", command=on_add_more, width=12).pack(side="left", padx=5)
...
def plot_angle_angle():
    ...
    for file_idx, file_path in enumerate(selected_files):
        ...
        if len(headers) % 2 != 0:
            messagebox.showwarning("Warning", "Please select pairs of headers for angle-angle plot.")
            return
...
def plot_confidence_interval():
    ...
    for file_idx, file_path in enumerate(selected_files):
        ...
        headers = [header for header in selected_headers if header in data.columns]
        if len(headers) < 1:
            messagebox.showwarning("Warning", "Please select at least one header for confidence interval plot.")
            return
...
def select_headers_gui(headers):
    ...
    Label(
        title_frame, 
        text="Select Headers for Analysis", 
        font=("Arial", 14, "bold")
    ).pack()
    ...
    for i, label in enumerate(headers):
        chk = Checkbutton(scrollable_frame, text=label, variable=header_vars[i], padx=5, pady=2)
    ...
    Button(btn_frame, text="Select All", command=select_all, width=15).pack(side="left", padx=5)
    Button(btn_frame, text="Unselect All", command=unselect_all, width=15).pack(side="left", padx=5)
    Button(btn_frame, text="Confirm", command=on_select, width=15).pack(side="right", padx=5)
