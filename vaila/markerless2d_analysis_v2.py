        ...
        if pose_config["use_yolo"] and yolo_model:
                ...
                if landmarks_norm:
                    ...
                    for i, (norm, px) in enumerate(zip(landmarks_norm, landmarks_px)):
                        ...
                        if len(landmarks_history[person_id][i]) > max_history:
                            landmarks_history[person_id][i] = landmarks_history[person_id][i][-max_history:]
                        ...
                        if pose_config.get("filter_type") == "kalman":
                            ...
                            landmarks_px[i] = [int(filtered[0] * width), 
                                              int(filtered[1] * height), 
                                              filtered[2],
                                              visibility]
                        elif pose_config.get("filter_type") == "savgol" and len(landmarks_history[person_id][i]) >= 5:
                            filtered = apply_savgol_filter(landmarks_history[person_id][i], norm)
                            landmarks_norm[i] = filtered
                            landmarks_px[i] = [int(filtered[0] * width), int(filtered[1] * height), filtered[2]]
                    ...
                    if pose_config.get("estimate_occluded", False):
                        ...
                        for i, norm in enumerate(landmarks_norm):
                            if norm is not None and not np.isnan(norm[0]):
                                landmarks_px[i] = [int(norm[0] * width), int(norm[1] * height), norm[2]]
            ...
            if frame_landmarks_norm is None:
                ...
                if results.pose_landmarks:
                    ...
                    for landmark in results.pose_landmarks.landmark:
                        ...
                        landmarks_px.append([int(landmark.x * width), int(landmark.y * height), landmark.z])
            ...
            if results.pose_landmarks:
                ...
                for landmark in results.pose_landmarks.landmark:
                    ...
                    frame_landmarks_px.append([int(landmark.x * width), int(landmark.y * height), landmark.z])
            else:
                frame_landmarks_norm = None
                frame_landmarks_px = None
...
def apply_kalman_filter(landmarks_history, current_landmarks=None):
    ...
    if current_landmarks is None:
        current_landmarks = [np.nan, np.nan, np.nan, 1.0] if len(landmarks_history[0]) > 3 else [np.nan, np.nan, np.nan]
    ...
    try:
        ...
        kf = KalmanFilter(
            initial_state_mean=history_array[0],
            n_dim_obs=n_dim
        )
...
def estimate_missing_landmarks(landmarks, visibility_threshold=0.5):
    ...
    for i, lm in enumerate(landmarks):
        if lm is None or lm[0] is None or np.isnan(lm[0]) or (len(lm) > 3 and lm[3] < visibility_threshold):
