...
# Pose mappings
# =====================
MEDIAPIPE_TO_COCO17 = {0:0, 2:1, 5:2, 7:3, 8:4, 11:5, 12:6, 13:7, 14:8, 15:9, 16:10, 23:11, 24:12, 25:13, 26:14, 27:15, 28:16}
COCO_IDX = {"nose":0, "left_eye":1, "right_eye":2, "left_ear":3, "right_ear":4, "left_shoulder":5, "right_shoulder":6, "left_elbow":7, "right_elbow":8, "left_wrist":9, "right_wrist":10, "left_hip":11, "right_hip":12, "left_knee":13, "right_knee":14, "left_ankle":15, "right_ankle":16}
COCO17_NAMES = ["nose","left_eye","right_eye","left_ear","right_ear","left_shoulder","right_shoulder","left_elbow","right_elbow","left_wrist","right_wrist","left_hip","right_hip","left_knee","right_knee","left_ankle","right_ankle"]
...
def pick_points_on_image(image: np.ndarray, n: int=4) -> np.ndarray:
    ...
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN and len(pts) < n:
            ...
            cv2.putText(img, f"P{len(pts)}", (x+6,y-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
...
def rotation_between(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    if c < -0.999999:
        ...
        if abs(a[0]) > 0.9: axis = np.array([0,1,0], np.float32)
        v = np.cross(a, axis); v = v / (np.linalg.norm(v)+1e-9)
        R = -np.eye(3, dtype=np.float32); R += 2*np.outer(v, v)
        return R
    K = np.array([[0,-v[2],v[1]],[v[2],0,-v[0]],[-v[1],v[0],0]], np.float32)
    R = np.eye(3, dtype=np.float32) + K + K@K * (1.0/(1.0+c+1e-9))
    return R

def anchor_with_vertical(pred3d_mm: np.ndarray, coco2d_px: np.ndarray, fps: float,
                         Hinv: np.ndarray, units: str="m", prefer_side: str="left",
                         leg_len_m: Optional[float]=0.42, z_weight: float=10.0,
                         use_anisotropic_z: bool=True) -> Tuple[np.ndarray, Dict]:
    scale_u = 1000.0 if units.lower()=="m" else 1.0  # convert ground XY meters→mm for matching Vp3D units
    ...
    ankles = np.concatenate([pred3d_mm[frames, COCO_IDX["left_ankle"], :], pred3d_mm[frames, COCO_IDX["right_ankle"], :]], axis=0)
    ...
    for t in frames:
        for a in [COCO_IDX["left_ankle"], COCO_IDX["right_ankle"]]:
            ...
            rows.append([p[0], 1, 0, 0]); rhs.append(gx)
            rows.append([p[1], 0, 1, 0]); rhs.append(gy)
            rows.append([z_weight*p[2], 0, 0, z_weight]); rhs.append(0.0)
    ...
    calib = {"s_xy": float(s_xy), "Rz": Rz, "T": Txyz, "gamma_z": 1.0, "anisotropic": False}
            ...
            if use_anisotropic_z:
                ...
                for t in frames:
                    for a in [COCO_IDX["left_ankle"], COCO_IDX["right_ankle"]]:
                        ...
                        g = image_to_ground_xy(Hinv, coco2d_px[t, a, :]) * scale_u
                        gx, gy = float(g[0,0]), float(g[0,1])
                        rows.append([1,0,0]); rhs.append(gx - p[0])
                        rows.append([0,1,0]); rhs.append(gy - p[1])
                        rows.append([0,0,1]); rhs.append(-p[2])
                A = np.asarray(rows, np.float64); b = np.asarray(rhs, np.float64)
...
# DLT3D
class DLT3D:
    def __init__(self, L: np.ndarray): self.L = L.astype(np.float64)
...
def refine_with_dlt3d(Pw: np.ndarray, uv_px: np.ndarray, dlt: DLT3D, init: Optional[Dict]=None, use_yaw_only: bool=True) -> Tuple[np.ndarray, Dict]:
    T,J,_ = Pw.shape
    W = Pw.copy()
    if init is None:
        s0 = 1.0; rz0=0.0; tx0=0.0; ty0=0.0; tz0=0.0; rx0=0.0; ry0=0.0
    else:
        ...
        rx0=0.0; ry0=0.0
    x0 = np.array([s0, rz0, tx0, ty0, tz0], dtype=np.float64) if use_yaw_only else np.array([s0, rx0, ry0, rz0, tx0, ty0, tz0], dtype=np.float64)
    uv = uv_px.reshape(-1,2).astype(np.float64); XYZ = W.reshape(-1,3).astype(np.float64)
    def residuals(x):
        if use_yaw_only:
            s, rz, tx, ty, tz = x; R = euler_to_Rxyz(0.0,0.0,rz)
        else:
            s, rx, ry, rz, tx, ty, tz = x; R = euler_to_Rxyz(rx,ry,rz)
    ...
    for t_i in range(T): out[t_i] = (s * (W[t_i] @ R.T)) + t
...
def extract_mediapipe_csv(video_path: str, out_csv: str) -> Tuple[int,int,float,int]:
    ...
    sys.stdout.flush()
    pose = mp.solutions.pose.Pose(static_image_mode=False, model_complexity=2, enable_segmentation=False, smooth_landmarks=True)
    rows = []
    i = 0
    t0 = time.time()
    t_last = t0
...
def read_toml_config(path: str) -> Dict:
    if _toml_reader is None:
        raise ImportError("tomllib/tomli is required to read TOML configs. Install 'tomli' for Python<3.11.")
...
def run_single_video(cfg: Dict, video_path: str, out_dir: str, base_dir: Optional[Path] = None):
    ...
    ckpt_resolved = _resolve(paths.get("ckpt", "../checkpoints/h36m_cpn_ft_h36m_dbb.pth"), expect_exists=False)
    ...
    if dlt3d_file and os.path.exists(dlt3d_file):
        ...
        Pw_mm, refine_params = refine_with_dlt3d(Pw_mm, coco2d_px, dlt3d, init=None, use_yaw_only=True)
    ...
    print(f"Saved: {out_base}.csv" + (" and .c3d" if bool(out.get("save_c3d", True)) and ezc3d is not None else ""))
...
if __name__ == '__main__':
    class App:
        def __init__(self, master: tk.Tk):
            ...
            self.cfg_path_var = tk.StringVar(value=str(Path("tests/markerless_3d_analysis/markerless3d_config.toml").resolve()))
            tk.Entry(row1, textvariable=self.cfg_path_var, width=80).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
            ...
            self.btn_default = tk.Button(row2, text="Create Default TOML", command=self.write_default_cfg)
        ...
        def browse_cfg(self):
            path = filedialog.askopenfilename(title="Select TOML", filetypes=(("TOML files","*.toml"), ("All files","*.*")))
            if path:
        ...
        def run_batch(self):
            try:
                ...
                patterns = [p.strip() for p in str(vid_cfg.get("pattern", "*.mp4;*_mp_pixel.csv")).split(';') if p.strip()]
                out_dir_cfg = cfg.get("output", {}).get("out_dir", str(input_dir / "mono3d_out"))
                out_dir = str((base_dir / out_dir_cfg).resolve()) if not os.path.isabs(out_dir_cfg) else out_dir_cfg
                files: List[str] = []
                for pat in patterns:
                    files.extend([str(p) for p in input_dir.glob(pat)])
                if not files:
                    messagebox.showwarning("No inputs", f"No files found in\n{input_dir}\nwith patterns: {', '.join(patterns)}")
                    return
                ...
                def _worker():
                    try:
                        for i, fp in enumerate(files, 1):
                            self.master.after(0, lambda i=i, fp=fp: self._set_status(f"[{i}/{len(files)}] {Path(fp).name}"))
