...
- CLI: python compress_videos_h264.py --dir /path/to/videos [--recursive] [--depth N] --preset medium --crf 23
...
## Linux:
1. **Install NVIDIA Drivers**:
   - Install the appropriate NVIDIA drivers for your GPU. For Ubuntu, you can add the graphics
     drivers PPA:
     ```bash
...
3. **Install or Compile FFmpeg with NVENC Support**:
   - Some Linux distributions provide FFmpeg packages with NVENC support. Check if `h264_nvenc`
     is available:
     ```bash
...
def is_nvidia_gpu_available():
    """Check if an NVIDIA GPU with working NVENC is available.

    Uses nvidia-smi (cross-platform) to detect NVIDIA GPUs, then verifies
    that h264_nvenc actually works by running a quick test encode.
    """
    try:
        # nvidia-smi works on Windows, Linux, and (older) macOS with NVIDIA drivers
        ...
        # testsrc at 1280x720 + yuv420p is more reliable than tiny color=black on modern GPUs.
        test_cmd = [
            FFMPEG,
            "-f",
            "lavfi",
        ...
        if test_result.returncode == 0:
            print("[bold green]NVIDIA NVENC H.264 encoder verified working.[/bold green]")
            return True
        else:
            print("[yellow]NVIDIA GPU found but h264_nvenc encoder not working.[/yellow]")
            return False
...
def find_videos_recursive(directory, max_depth=-1):
    ...
    Returns:
        list[tuple[str, str]]: List of (absolute_path, relative_subdir) where
        relative_subdir is the path relative to directory (e.g. '' or 'subdir1').
    """
...
def compress_video_worker_h264(video_info):
    ...
    try:
        ...
        try:
            ...
            _ = subprocess.check_output(cmd_probe).decode().strip()
        except Exception:
            pass
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            ...
            # Adaptive compression: if output is larger than input, delete unless user allowed it
...
def run_compress_videos_h264(
    ...
    Args:
        ...
        allow_larger_output: If True, keep output even when larger than input (e.g. H.265→H.264).
    ...
    print(f"  Allow larger output (e.g. H.265→H.264): {allow_larger_output}")
    ...
    print(
        "[yellow]This process might take several hours depending on your computer "
...
def get_compression_parameters():
    ...
    # 6. Allow output larger than input (e.g. H.265→H.264)
    allow_larger_var = tk.BooleanVar(value=False)
    tk.Checkbutton(
        main_frame,
        text="Allow output larger than input (e.g. H.265→H.264 compatibility)",
    ...
    def on_ok():
        try:
            ...
            if params["allow_larger_output"]:
                confirm_msg += (
                    "Output may be larger than input (e.g. H.265→H.264). Files will be kept.\n\n"
                )
...
ALLOW LARGER OUTPUT:
When converting e.g. H.265/HEVC to H.264, the output file may be larger than
the input. By default such files are not saved. Check this option to keep
them (useful for compatibility on PCs that don't support HEVC).
        """
...
def compress_videos_h264_gui():
    ...
    if compression_config["use_gpu"] and not use_gpu:
        ...
        messagebox.showwarning(
            "GPU Not Available",
            "GPU acceleration was requested but the h264_nvenc encoder is not available.\n\n"
            "This can happen if:\n"
            "• No NVIDIA GPU is present\n"
            "• Your FFmpeg was not compiled with NVENC support\n"
            "• NVIDIA drivers are outdated\n\n"
            "Compression will proceed using CPU (libx264) instead.",
        )

    # Find video files (GUI: optional recursion up to max_depth)
    ...
    if max_depth == 0:
        ...
        video_list = [
            (
                ...
                os.path.join(
                    output_directory,
                    os.path.splitext(os.path.basename(p))[0] + "_h264.mp4",
                ),
            )
        ...
        video_list = [
            (
                ...
                os.path.join(
                    output_directory,
                    rel_subdir,
                    os.path.splitext(os.path.basename(path))[0] + "_h264.mp4",
                ),
            )
    ...
    success_count, failure_count = run_compress_videos_h264(
        video_list,
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Compress videos to H.264 format using FFmpeg.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Compress with default settings (medium preset, CRF 23, original resolution, CPU)
  ...
  # Launch GUI mode
  python -m vaila.compress_videos_h264
        """,
    )
    parser.add_argument(
        "--dir",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Recurse into subdirectories and process all videos (output mirrors folder structure).",
    )
    parser.add_argument(
        "--depth",
        "-d",
        ...
        default=-1,
        metavar="N",
        help="Max subdirectory depth when using --recursive (0=root only, 1/2/3=levels, -1=unlimited, default: -1).",
    )
    parser.add_argument(
        "--preset",
        type=str,
    ...
    parser.add_argument(
        "--allow-larger-output",
        action="store_true",
        help="Keep output even when larger than input (e.g. H.265→H.264 for compatibility).",
    )
    return parser
...
def main():
    ...
    # If no CLI arguments (besides script name), launch GUI
    if len(sys.argv) == 1:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_directory = os.path.join(args.dir, f"compressed_h264_{timestamp}")
