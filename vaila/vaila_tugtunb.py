...
class TUGAnalyzer:
    ...
    def segment_tug_phases(self) -> dict:
        ...
        if 'Mid_Trunk_vel_y' in self.df.columns and 'Mid_Trunk_vel_x' in self.df.columns:
            ...
            mid_trunk_vy = self.df['Mid_Trunk_vel_y'].to_numpy() if 'Mid_Trunk_vel_y' in self.df.columns else com_vy
        ...
        if sit_start > sts_end + int(fs * 2.0):
            ...
            if window_len > 0:
                smoothed_speed = uniform_filter1d(trunk_speed_xy[search_start:search_end], size=window_len)
            ...
            while stop_start > sts_end and (mid_trunk_vy[stop_start] < speed_thresh and trunk_speed_xy[stop_start] < speed_thresh):
                stop_start -= 1
            ...
            for i in range(anchor_idx, turn_search_end):
                if mid_trunk_vy[i] < -0.2:
                    turn_search_end = min(turn_search_end, i + int(fs * 1.5)) # Allow a bit of overlap, but bounded
                    break
            ...
            if bck_start == sit_start: bck_start = turn_end
            ...
            fwd_stop = bck_start = turn_start = turn_end = stop_start = stop_end = (sts_end + sit_start) // 2
