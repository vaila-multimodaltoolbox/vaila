...
def save_info_file(marker_labels, marker_freq, analog_labels, analog_freq, dir_name, file_name):
...
def save_to_files(markers, marker_labels, marker_freq, analogs, points_residuals, analog_labels, analog_freq, file_name, output_dir, save_excel):
    print(f'Saving data to files for {file_name}')
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    dir_name = os.path.join(output_dir, "multimodal_c3d_to_csv", f"{file_name}_{timestamp}")
    ...
    marker_time_column = pd.Series([f"{i / marker_freq:.3f}" for i in range(num_marker_frames)], name='Time')
    ...
    # Prepare analog data
    analogs = analogs.squeeze(axis=0).T  # Remove the extra dimension and transpose to have time in rows
    analogs_df = pd.DataFrame(analogs, columns=analog_labels)

    # Add time column to analogs_df
    num_analog_frames = analogs_df.shape[0]
    analog_time_column = pd.Series([f"{i / analog_freq:.3f}" for i in range(num_analog_frames)], name='Time')
    ...
    points_time_column = pd.Series([f"{i / marker_freq:.3f}" for i in range(num_points_frames)], name='Time')
    ...
    # Save to CSV
    markers_ddf.to_csv(os.path.join(dir_name, f"{file_name}_markers.csv"), index=False, single_file=True)
    analogs_ddf.to_csv(os.path.join(dir_name, f"{file_name}_analogs.csv"), index=False, single_file=True)
    points_residuals_ddf.to_csv(os.path.join(dir_name, f"{file_name}_points_residuals.csv"), index=False, single_file=True)

    if save_excel:
        # Save to Excel (using pandas)
        print("Saving to Excel. This process can take a long time...")
        with pd.ExcelWriter(os.path.join(dir_name, f"{file_name}.xlsx"), engine='openpyxl') as writer:
            markers_ddf.compute().to_excel(writer, sheet_name='Markers', index=False, float_format="%.7f")
            analogs_ddf.compute().to_excel(writer, sheet_name='Analogs', index=False, float_format="%.7f")
            points_residuals_ddf.compute().to_excel(writer, sheet_name='Points Residuals', index=False, float_format="%.7f")

    # Save info file
    save_info_file(marker_labels, marker_freq, analog_labels, analog_freq, dir_name, file_name)
...
def convert_c3d_to_csv():
    ...
    save_excel = messagebox.askyesno("Save as Excel", "Do you want to save the data as Excel files? This process can be very slow.")
    ...
    if input_directory and output_directory:
        ...
        c3d_files = sorted([f for f in os.listdir(input_directory) if f.endswith('.c3d')])
        ...
        for c3d_file in tqdm(c3d_files, desc="Processing C3D files"):
            try:
                ...
                markers, marker_labels, marker_freq, analogs, points_residuals, analog_labels, analog_freq = importc3d(file_path)
                file_name = os.path.splitext(c3d_file)[0]
                save_to_files(markers, marker_labels, marker_freq, analogs, points_residuals, analog_labels, analog_freq, file_name, output_directory, save_excel)
