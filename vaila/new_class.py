class InterpolationConfigDialog:
    ...
    def create_dialog_content(self):
        ...
        self.title_label = ctk.CTkLabel(
            self.header_frame,
            text="Interpolation & Smoothing Config",
            font=ctk.CTkFont(size=24, weight="bold"),
        )
        self.title_label.pack()
        self.subtitle_label = ctk.CTkLabel(
            self.header_frame,
            text="Configure gap filling, smoothing and validate your data.",
        ...
        self.save_btn = ctk.CTkButton(
            self.buttons_frame,
            text="💾 Save Template",
            command=self.create_toml_template,
            width=140,
        )
        self.save_btn.grid(row=0, column=0, padx=5, pady=5)

        self.load_btn = ctk.CTkButton(
            self.buttons_frame, text="📂 Load TOML", command=self.load_toml_config, width=140
        )
        self.load_btn.grid(row=0, column=1, padx=5, pady=5)

        self.apply_btn = ctk.CTkButton(
            self.buttons_frame,
            text="✅ Apply & Run Batch",
            command=self.ok,
            fg_color="#2E7D32",
            hover_color="#1B5E20",
        )
    ...
    def build_interp_tab(self):
        ...
        lbl = ctk.CTkLabel(
            self.tab_interp, text="Select Interpolation Method:", font=ctk.CTkFont(weight="bold")
        )
        ...
        self.interp_combo = ctk.CTkOptionMenu(
            self.tab_interp, values=list(methods.values()), command=self.on_interp_change
        )
        self.interp_combo.pack(fill="x", pady=5)
        self.interp_combo.set("Linear (Straight lines)")

        lbl_gap = ctk.CTkLabel(
            self.tab_interp, text="Max Gap Size (frames):", font=ctk.CTkFont(weight="bold")
        )
        ...
        tip = ctk.CTkLabel(
            self.tab_interp,
            text="0 = Fill all gaps\n60 = Up to 2 seconds at 30 fps",
    ...
    def build_smooth_tab(self):
        ...
        lbl = ctk.CTkLabel(
            self.tab_smooth, text="Select Smoothing Method:", font=ctk.CTkFont(weight="bold")
        )
        lbl.pack(anchor="w", pady=(10, 5))
        ...
        self.smooth_combo = ctk.CTkOptionMenu(
            self.tab_smooth, values=list(methods.values()), command=self.on_smooth_change
        )
    ...
    def build_all_param_frames(self):
        ...
        ctk.CTkLabel(
            f5, text="Tip: 4-10 Hz for biomechanics", text_color="gray", font=("Arial", 10)
        ...
        ctk.CTkLabel(
            f5, text="Tip: fps of the video or capture freq", text_color="gray", font=("Arial", 10)
        ).pack(anchor="w")
    ...
    def build_general_tab(self):
        ...
        lbl_split = ctk.CTkLabel(
            self.tab_general, text="Split Dataset:", font=ctk.CTkFont(weight="bold")
        )
        lbl_split.pack(anchor="w", pady=(15, 5))
        ctk.CTkSwitch(self.tab_general, text="Enable Splitting", variable=self.split_var).pack(
            anchor="w", pady=5
        )

        lbl_rate = ctk.CTkLabel(
            self.tab_general,
            text="Sample Rate Override (optional):",
    ...
    def build_analysis_panel(self):
        ...
        self.load_test_btn = ctk.CTkButton(
            self.analysis_top, text="📊 Load Test CSV", command=self.load_test_data
        )
    ...
    def load_test_data(self):
        ...
        if file_path:
            try:
                ...
                self.col_combo = ctk.CTkOptionMenu(
                    self.col_frame,
                    values=numeric_cols,
                    variable=self.test_col_var,
                ...
                btn = ctk.CTkButton(
                    self.col_frame,
                    text="▶ Run Check",
                    command=lambda: self.run_analysis(self.test_col_var.get()),
                    width=100,
                )
                ...
                win_btn = ctk.CTkButton(
                    self.col_frame,
                    text="❄ Winter Check",
                    command=lambda: self.perform_winter_residual_analysis(self.test_col_var.get()),
                    width=100,
                    fg_color="#ea7317",
                    hover_color="#d3610b",
                )
    ...
    def run_analysis(self, column_name):
        ...
        ax1.set_title(
            f"Original vs Processed - {column_name}",
            fontweight="bold",
            color="white"
            if fig.patch.get_facecolor()
            == (0.16862745098039217, 0.16862745098039217, 0.16862745098039217, 1.0)
            else "black",
        )
        ax1.legend(loc="best")
    ...
    def ok(self):
        ...
        if self.validate():
            ...
            if messagebox.askokcancel(
                "Confirm Parameters", f"Apply the following processing?\n\n{conf_str}"
            ):
    ...
    def load_toml_config(self):
        ...
        if file_path:
            ...
            messagebox.showinfo(
                "Loaded", f"Loaded configuration from {os.path.basename(file_path)}"
            )
    ...
    def create_toml_template(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile="smooth_config.toml",
        )
        if file_path:
    ...
    def perform_winter_residual_analysis(self, column_name):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Invalid fs. Use a numeric value for Butterworth Sampling Freq."
            )
            return
        ...
        try:
            fc_arr, res_arr, opt_fc = winter_residual_analysis(
                data=data, fs=fs, fc_min=1.0, fc_max=15.0, n_fc=29, order=4
            )
        except Exception as e:
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Optimal Cutoff ≈ {opt_fc:.2f} Hz",
        )

        ax.set_title(
            f"Winter residual analysis - {column_name} (fs={fs} Hz)",
            fontweight="bold",
            color="white"
            if fig.patch.get_facecolor()
            == (0.16862745098039217, 0.16862745098039217, 0.16862745098039217, 1.0)
