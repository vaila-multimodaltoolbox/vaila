...
def plot_markers_plotly(markers, marker_labels, marker_freq, num_frames, num_markers, file_name):
    global_coordinate_system = np.eye(3)
    axis_length = 0.2  # Define the length of the coordinate system axes

    frames = []
    fig = make_subplots(rows=1, cols=1, specs=[[{'type': 'scatter3d'}]], subplot_titles=[f'Markers Animation - {file_name}'])
    ...
    try:
        ...
        if file_path:
            ...
            if selected_marker_indices:
                ...
                plot_markers_plotly(markers, selected_marker_labels, marker_freq, num_frames, num_markers, file_name)
            else:
