...
def extract_bbox_point(x_min, x_max, y_min, y_max, point):
    """Calcular o ponto desejado da bounding box."""
    if point == "center":
        return (x_min + x_max) / 2, (y_min + y_max) / 2
    elif point == "top-left":
        return x_min, y_min
    elif point == "top-middle":
        return (x_min + x_max) / 2, y_min
    elif point == "top-right":
        return x_max, y_min
    elif point == "middle-left":
        return x_min, (y_min + y_max) / 2
    elif point == "middle-right":
        return x_max, (y_min + y_max) / 2
    elif point == "bottom-left":
        return x_min, y_max
    elif point == "bottom-middle":
...
def convert_yolo_tracker_to_pixel_format(
    tracker_file, save_directory=None, chunk_size=10000, point="center"
):
    ...
    print(f"Converting YOLO tracker file: {tracker_file} using point: {point}")
    ...
    try:
        ...
        if is_long_format:
            ...
            print("Detected WIDE format (e.g. merged YOLO track csv)")
        else:
            # Fallback legacy format (e.g. X_1, Y_1)
            print("Detected LEGACY format (e.g. X_1, Y_1)")
            tracker_ids = sorted(
                [int(col.split("_")[1]) for col in columns if col.startswith("ID_")]
            )
        ...
        for chunk_number, chunk in enumerate(pd.read_csv(tracker_file, chunksize=chunk_size)):
            ...
            if is_long_format:
                ...
                pivot_df = chunk.pivot_table(
                    index="Frame", columns="Tracker ID", values=["_X", "_Y"], aggfunc="first"
                )
                ...
                for suffix in wide_suffixes:
                    ...
                    x, y = extract_bbox_point(
                        chunk[f"X_min_{suffix}"],
                        chunk[f"X_max_{suffix}"],
                        chunk[f"Y_min_{suffix}"],
                        chunk[f"Y_max_{suffix}"],
                        point,
                    )
...
def batch_convert_yolo_tracker(directory_path=None, point=None, chunk_size=None):
    """
    ...
    # Ask for point if not provided via CLI
    if point is None:
        try:
            ...
            point_menu = ttk.Combobox(
                point_dialog, textvariable=point_var, values=points, state="readonly"
            )
    ...
    if chunk_size is None:
        try:
            chunk_size = simpledialog.askinteger(
                "Chunk Size",
                "Enter chunk size for processing:\n"
                + "- Small files: 10000\n"
                + "- Medium: 5000\n"
                + "- Large: 1000\n"
                + "- Very large: 500",
    ...
    for idx, file_name in enumerate(potential_files, 1):
        try:
            ...
            result_path = convert_yolo_tracker_to_pixel_format(
                file_path, save_directory, chunk_size=chunk_size, point=point
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="vailá - Rearrange Data and convert tracking files"
    )
    parser.add_argument(
        "--yolo-tracker", type=str, help="Directory containing YOLO tracker CSVs to convert"
    )
    parser.add_argument(
        "--point",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Chunk size for reading large CSV files (default: 5000)",
    )
    ...
    if args.yolo_tracker:
        ...
        batch_convert_yolo_tracker(
            directory_path=args.yolo_tracker, point=args.point, chunk_size=args.chunk_size
        )
    else:
        rearrange_data_in_directory()
