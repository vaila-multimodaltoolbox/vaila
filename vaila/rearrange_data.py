...
def detect_precision_and_notation(file_path):
    ...
    for col in df.columns:
        col_max_decimal = df[col].apply(lambda x: len(x.split('.')[1]) if '.' in str(x) else 0).max()
...
class ColumnReorderGUI(tk.Tk):
    def __init__(self, original_headers, file_names, directory_path):
        ...
        self.max_decimal_places, self.scientific_notation = detect_precision_and_notation(os.path.join(directory_path, base_file_name))
    ...
    def delete_row_range(self, window, start, end):
        ...
        save_dataframe(deleted_df, new_file_path, deleted_df.columns, self.max_decimal_places)
    ...
    def save_intermediate(self, event=None):
        try:
            ...
            max_decimal_places = simpledialog.askinteger("Decimal Places", "Enter the number of decimal places for saving:", initialvalue=self.max_decimal_places)
            if max_decimal_places is None:
                max_decimal_places = self.max_decimal_places
            
            for file_name in self.file_names:
                file_path = os.path.join(self.directory_path, file_name)
                reshapedata(file_path, self.current_order, self.rearranged_path, "", max_decimal_places)
    ...
    def save_and_exit(self, event=None):
        try:
            ...
            for file_name in self.file_names:
                file_path = os.path.join(self.directory_path, file_name)
                reshapedata(file_path, self.current_order, self.rearranged_path, "_final", max_decimal_places)
    ...
    def merge_csv(self):
        ...
        if base_file and merge_file:
            insert_position = simpledialog.askinteger("Insert Position", "Enter the column position to insert the merge file (leave empty for last column):", initialvalue=None, minvalue=1)
            save_path = filedialog.asksaveasfilename(title="Save Merged File As", defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
            if save_path:
    ...
    def stack_csv(self):
        ...
        if base_file and stack_file:
            stack_position = simpledialog.askstring("Stack Position", "Enter 'start' to stack at the beginning, or leave empty to stack at the end:", initialvalue="end")
            save_path = filedialog.asksaveasfilename(title="Save Stacked File As", defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
            if save_path:
