...
def detect_gaps(x_values, y_values, min_gap_size=3):
    ...
    for i in range(len(x_values)):
        is_missing = pd.isna(x_values[i]) or pd.isna(y_values[i]) or (x_values[i] == "" and y_values[i] == "")
...
def merge_markers(df, source_id, target_id, frame_range=None):
    ...
    if (source_x_col not in df.columns or source_y_col not in df.columns or
        target_x_col not in df.columns or target_y_col not in df.columns):
    ...
    print(f"Marker {source_id} merged with marker {target_id} in frame range {start_frame}-{end_frame}")
    return df
...
def swap_markers(df, marker_id1, marker_id2, frame_range):
    ...
    if (x_col1 not in df.columns or y_col1 not in df.columns or
        x_col2 not in df.columns or y_col2 not in df.columns):
...
def visualize_markers(df, marker_ids=None):
    ...
    if 'frame' not in df.columns or len(df) == 0:
        messagebox.showerror("Error", "The data file appears to be empty or missing the 'frame' column.")
        return
...
def advanced_reid_gui():
    ...
    def get_selected_markers():
        checked = marker_selection.get_status()
        return [all_markers[i] for i, checked_state in enumerate(checked) if checked_state]
...
def fill_gaps_arima(df, marker_id, max_gap_size=30, order=(1,1,1)):
    ...
    for gap_start, gap_end in gaps:
        ...
        if len(before_segment) < 3 or len(after_segment) < 3:
            print(f"Not enough context around gap for marker {marker_id} (frames {gap_start}-{gap_end})")
            continue
        
        # Combine segments for ARIMA modeling
        full_segment_x = [p[0] for p in before_segment] + [np.nan] * gap_size + [p[0] for p in after_segment]
        full_segment_y = [p[1] for p in before_segment] + [np.nan] * gap_size + [p[1] for p in after_segment]
        ...
        try:
            ...
            print(f"Successfully filled gap for marker {marker_id} using ARIMA (frames {gap_start}-{gap_end})")
            
        except Exception as e:
            print(f"ARIMA failed for marker {marker_id}: {str(e)}. Falling back to linear interpolation.")
    ...
    if filled_gaps > 0:
        df[x_col] = x_filled
        df[y_col] = y_filled
        print(f"Filled {filled_gaps} gaps ({total_points_filled} points) for marker {marker_id} using ARIMA")
...
def auto_fill_gaps_arima():
    ...
    # Header label
    header_label = Label(arima_params_window, text="ARIMA Model Parameters", font=("Arial", 12, "bold"))
    ...
    def show_help_popup(param):
        ...
        help_window.geometry(f"+{arima_params_window.winfo_rootx() + 100}+{arima_params_window.winfo_rooty() + 100}")
    ...
    max_gap_help_btn = TkButton(max_gap_frame, text="?", width=2, command=lambda: show_help_popup("max_gap"))
    ...
    def start_processing():
        ...
        for marker_id in markers:
            ...
            if points_filled > 0:
                ...
                # Update the metadata to include information about filled gaps
                df.loc[0, 'processed_info'] += f"ARIMA filled {points_filled} points for marker {marker_id}; "
                
            # Update status to show progress
            status_var.set(f"Processing marker {marker_id}... ({markers.index(marker_id)+1}/{len(markers)})")
            arima_params_window.update_idletasks()  # Force GUI update
        
        # Add timestamp and processing info
        timestamp = pd.Timestamp.now().strftime("%Y-%m-%d_%H-%M-%S")
        df.loc[0, 'processed_info'] += f"ARIMA processed on {timestamp} with parameters (p={arima_params['p']},d={arima_params['d']},q={arima_params['q']})"
        ...
        arima_params_window.after(1000, lambda: [
            ...
            (lambda: visualize_markers(df) if messagebox.askyesno("View Results", "Would you like to visualize the results?") else None)()
        ])
    ...
    def show_all_help():
        ...
        help_window.geometry(f"+{arima_params_window.winfo_rootx() + 50}+{arima_params_window.winfo_rooty() + 50}")
        
        # Title
        Label(help_window, text="ARIMA Parameter Guide", font=("Arial", 12, "bold")).pack(pady=(10, 15))
        ...
        Label(scrollable_frame, text="p (AR) - Autoregressive Order:", font=("Arial", 10, "bold"), anchor="w").pack(fill="x", pady=(5, 0), padx=10)
        Label(scrollable_frame, text="• Controls how previous values influence current value\n• Higher values capture longer-term patterns\n• Recommended: 1-3 (start with 1)", justify="left").pack(fill="x", pady=(0, 10), padx=20)
        
        Label(scrollable_frame, text="d (Diff) - Differencing Order:", font=("Arial", 10, "bold"), anchor="w").pack(fill="x", pady=(5, 0), padx=10)
        Label(scrollable_frame, text="• Makes data stationary by removing trends\n• 0: No differencing, 1: Remove linear trend, 2: Remove quadratic trend\n• Recommended: 0-1 for marker trajectories", justify="left").pack(fill="x", pady=(0, 10), padx=20)
        
        Label(scrollable_frame, text="q (MA) - Moving Average Order:", font=("Arial", 10, "bold"), anchor="w").pack(fill="x", pady=(5, 0), padx=10)
        Label(scrollable_frame, text="• Incorporates error terms from previous predictions\n• Higher values smooth out irregular patterns\n• Recommended: 1-2 (start with 1)", justify="left").pack(fill="x", pady=(0, 10), padx=20)
        
        Label(scrollable_frame, text="Max Gap Size:", font=("Arial", 10, "bold"), anchor="w").pack(fill="x", pady=(5, 0), padx=10)
        Label(scrollable_frame, text="• Maximum number of consecutive frames to fill\n• Larger gaps are less reliable to fill\n• Recommended: 15-30 frames", justify="left").pack(fill="x", pady=(0, 10), padx=20)
        
        # Add practical advice
        Label(scrollable_frame, text="Practical Tips:", font=("Arial", 10, "bold"), anchor="w").pack(fill="x", pady=(10, 0), padx=10)
        Label(scrollable_frame, text="• Start with default values (p=1, d=1, q=1)\n• For smoother trajectories, increase q\n• For cyclical motion, increase p\n• For complex patterns, try p=2, d=1, q=2", justify="left").pack(fill="x", pady=(0, 10), padx=20)
        
        # OK button
        TkButton(help_window, text="OK", command=help_window.destroy).pack(pady=10)
    
    # Add help button for all parameters
    help_all_button = TkButton(arima_params_window, text="View Complete Parameter Guide", command=show_all_help)
