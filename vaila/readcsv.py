...
def show_csv_optimized(file_path=None, *, use_cache=True, turbo=False, fps=30, stride=1):
    ...
    if stride > 1:
        ...
        print(f"Applied stride {stride}: reduced frames from {len(index_vector)*stride} to {len(index_vector)}")
...
def show_csv_matplotlib_turbo(points, marker_names, fps=30):
    ...
    if len(valid_points) > 0:
        scatter = ax.scatter(valid_points[:, 0], valid_points[:, 1], valid_points[:, 2], 
                           c=colors[:len(valid_points)], s=30, alpha=0.8)
    else:
    ...
    def update(frame):
        ...
        if len(valid_points) > 0:
            ...
            scatter._offsets3d = (valid_points[:, 0], valid_points[:, 1], valid_points[:, 2])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1].startswith('--'):
        ...
        parser.add_argument("--turbo", action="store_true", help="Use turbo mode (faster visualization)")
        parser.add_argument("--fps", type=int, default=30, help="Frames per second")
        parser.add_argument("--stride", type=int, default=1, help="Skip frames (1=all, 2=every other)")
        
        args = parser.parse_args()
        
        if args.file:
            show_csv_optimized(args.file, 
