    ...
    def on_select():
        ...
        if len(selected_headers) % 3 != 0:
            tk.messagebox.showerror(
                "Error",
                "Please select columns in multiples of 3 (X, Y, Z for each marker).",
            )
    ...
    try:
        ...
        if file_path:
            ...
            if selected_headers:
                ...
                missing_headers = [
                    header for header in selected_headers if header not in headers
                ]
                if missing_headers:
                    print(
                        f"Error: The following selected headers were not found in the CSV file: {missing_headers}"
                    )
                else:
                ...
                for i in range(0, len(selected_headers), 3):
                    if i + 2 < len(selected_headers):
                        ...
                        if (
                            np.mean(np.abs(values_x)) > 100
                            or np.mean(np.abs(values_y)) > 100
                            or np.mean(np.abs(values_z)) > 100
                        ):
                        ...
                        marker = {
                            ...
                            "name": selected_headers[i].rsplit("_", 1)[
                                0
                            ],  # Group by marker name before last '_'
                        }
                ...
                app.layout.children[1].marks = {
                    i: str(i) for i in range(0, len(df), max(1, len(df) // 10))
                }
