...
def process_directory(directory_path=None, filter_config=None):
    ...
    if not csv_files:
        ...
        messagebox.showwarning(
            "No Files",
            f"No valid input CSV files found. Angle outputs (processed_*, *_rel, *_abs_*) are ignored.",
        )
        return None
...
def process_directory_videos(directory_path, filter_config=None):
    ...
    for video_file in sorted(videos):
        ...
        for f in os.listdir(directory_path):
            ...
            if csv_base == video_basename or csv_base.startswith(video_basename + ".") or f.startswith(video_basename):
...
def filter_landmarks(df, fps=30, cutoff=6.0, order=4):
    ...
    print(f"Applying Butterworth filter (fc={cutoff}Hz, order={order}) to {len(cols_to_filter)} columns...")
...
def process_angles(input_csv, output_csv, filter_config=None, video_dims=None):
    ...
    try:
        ...
        relative_angles_dict = {
            "frame": df.iloc[:, 0] if "frame" in df.columns[0].lower() else np.arange(total_frames), # Use existing or index
            "neck": neck_angles,
            "trunk": trunk_angles,
...
def draw_skeleton_enhanced(frame, landmarks, rel_angles, abs_angles, abs_angles_360=None):
    ...
    if 'right_mid_hand' not in landmarks:
        landmarks['right_mid_hand'] = compute_midpoint(landmarks['right_pinky'], landmarks['right_index'])
    if 'left_mid_hand' not in landmarks:
        landmarks['left_mid_hand'] = compute_midpoint(landmarks['left_pinky'], landmarks['left_index'])
    ...
    yl = y_top + step
    put_txt(f"L Shldr: {rel_angles.get('left_shoulder', 0):.1f}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Elbow: {rel_angles.get('left_elbow', 0):.1f}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Wrist: {rel_angles.get('left_wrist', 0):.1f}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Hip:   {rel_angles.get('left_hip', 0):.1f}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Knee:  {rel_angles.get('left_knee', 0):.1f}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Ankle: {rel_angles.get('left_ankle', 0):.1f}", x_left, yl, C_LEFT); yl += step
    ...
    yr = y_top + step
    put_txt(f"R Shldr: {rel_angles.get('right_shoulder', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Elbow: {rel_angles.get('right_elbow', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Wrist: {rel_angles.get('right_wrist', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Hip:   {rel_angles.get('right_hip', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Knee:  {rel_angles.get('right_knee', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Ankle: {rel_angles.get('right_ankle', 0):.1f}", x_right, yr, C_RIGHT); yr += step
    ...
    yl = y_bottom + step
    put_txt(f"L Trunk:  {fmt_abs(abs_left.get('trunk_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Neck:   {fmt_abs(abs_left.get('neck_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Thigh:  {fmt_abs(abs_left.get('left_thigh_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Shank:  {fmt_abs(abs_left.get('left_shank_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Foot:   {fmt_abs(abs_left.get('left_foot_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L UArm:   {fmt_abs(abs_left.get('left_upperarm_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Forearm:{fmt_abs(abs_left.get('left_forearm_abs'))}", x_left, yl, C_LEFT); yl += step
    put_txt(f"L Hand:   {fmt_abs(abs_left.get('left_hand_abs'))}", x_left, yl, C_LEFT); yl += step
    ...
    put_txt("ABSOLUTE (0..360)" if abs_angles_360 is not None else "ABSOLUTE (-180..180)", x_right, y_bottom, (0, 255, 255))
    yr = y_bottom + step
    put_txt(f"R Trunk:  {fmt_abs(abs_right.get('trunk_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Neck:   {fmt_abs(abs_right.get('neck_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Thigh:  {fmt_abs(abs_right.get('right_thigh_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Shank:  {fmt_abs(abs_right.get('right_shank_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Foot:   {fmt_abs(abs_right.get('right_foot_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R UArm:   {fmt_abs(abs_right.get('right_upperarm_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Forearm:{fmt_abs(abs_right.get('right_forearm_abs'))}", x_right, yr, C_RIGHT); yr += step
    put_txt(f"R Hand:   {fmt_abs(abs_right.get('right_hand_abs'))}", x_right, yr, C_RIGHT); yr += step

    return frame


def process_video_with_visualization(video_path, csv_path=None, output_dir=None, filter_config=None):
    """
    ...
    if csv_path is None:
        ...
        possible_suffixes = ['_pixel_vaila.csv', '_mp_norm.csv', '_mp_pixel.csv', '_mp_data.csv', '.csv']
    ...
    while cap.isOpened():
        ...
        if frame_idx >= len(df): break # Stop if no more data
        ...
        if not np.isnan(landmarks['left_shoulder']).any() and not np.isnan(landmarks['right_shoulder']).any():
             landmarks['mid_shoulder'] = (landmarks['left_shoulder'] + landmarks['right_shoulder']) / 2
        else: landmarks['mid_shoulder'] = np.array([np.nan, np.nan])
        ...
        else: landmarks['mid_hip'] = np.array([np.nan, np.nan])
        ...
        else: landmarks['mid_ear'] = np.array([np.nan, np.nan])
    ...
    generate_html_report(
        output_dir, video_path, csv_paths,
...
_JOINT_ANGLE_KEYS = {
    ...
    "mid_shoulder": "neck", "mid_hip": "trunk",
}
...
def plot_stick_sequence_mpangles(df, output_path, num_frames=8, rel_df=None, abs_df=None):
    ...
    if n == 0:
        return None
    start_col = 1 if (len(df.columns) > 0 and ('frame' in str(df.columns[0]).lower() or 'index' in str(df.columns[0]).lower())) else 0
    ...
    for i, fi in enumerate(frames_idx):
        ...
        if "left_ear" in pts and "right_ear" in pts:
            pts["mid_ear"] = ((pts["left_ear"][0] + pts["right_ear"][0]) / 2, (pts["left_ear"][1] + pts["right_ear"][1]) / 2)
        if "left_shoulder" in pts and "right_shoulder" in pts:
            pts["mid_shoulder"] = ((pts["left_shoulder"][0] + pts["right_shoulder"][0]) / 2, (pts["left_shoulder"][1] + pts["right_shoulder"][1]) / 2)
        if "left_hip" in pts and "right_hip" in pts:
            pts["mid_hip"] = ((pts["left_hip"][0] + pts["right_hip"][0]) / 2, (pts["left_hip"][1] + pts["right_hip"][1]) / 2)
        # Colored segments (like video)
        for start, end in _STICK_SEGMENTS:
            if start in pts and end in pts:
                color = _segment_color(start, end)
                ax.plot([pts[start][0], pts[end][0]], [pts[start][1], pts[end][1]], color=color, lw=2)
        # Neck: mid_ear to mid_shoulder (center)
        if "mid_ear" in pts and "mid_shoulder" in pts:
            ax.plot([pts["mid_ear"][0], pts["mid_shoulder"][0]], [pts["mid_ear"][1], pts["mid_shoulder"][1]], color=_segment_color("mid", "mid"), lw=2)
        ...
        for name, (x, y) in pts.items():
            ...
            if "eye" in name or name == "nose":
                continue
            ax.plot(x, y, "o", color="#16a34a", markersize=6, markeredgecolor="white", markeredgewidth=0.5)
        ...
        if rel_df is not None and fi < len(rel_df):
            ...
            for lm_name, key in _JOINT_ANGLE_KEYS.items():
                ...
                try:
                    ...
                    ax.text(x + 8, y - 8, f"{float(val):.0f}", fontsize=7, color="black", ha="left", va="bottom")
...
def generate_html_report(output_dir, video_path, csv_paths, output_video_basename=None, stick_figure_path=None):
    """
    ...
    try:
                ...
                for _, row in df_ang.iterrows():
                    ...
                    for c in cols:
                        ...
                        if isinstance(v, (int, float)) and np.isfinite(v):
                            angles_table_html += f"<td>{int(v) if c.lower() == 'frame' else f'{v:.1f}'}</td>"
                        else:
                            angles_table_html += f"<td>{v}</td>"
                    angles_table_html += "</tr>"
                angles_table_html += "</tbody></table></div>"
                if len(df_ang) >= 20:
                    angles_table_html += "<p><em>Showing first 21 rows. Download CSV above for full data.</em></p>"
        except Exception as e:
        ...
        if stick_figure_path and os.path.exists(stick_figure_path):
            rel_stick = os.path.basename(stick_figure_path) if os.path.dirname(stick_figure_path) == output_dir else os.path.relpath(stick_figure_path, output_dir)
            html_content += f"""
                <div class="section">
...
class MPAnglesApp:
    def __init__(self, root):
        ...
        ttk.Label(lf_opts, text="Calculates Rel, Abs(-180..180), Abs(0..360) automatically.").pack(anchor='w')
        ...
        try:
            if os.path.isdir(input_path):
                ...
                if results:
                    ...
                    messagebox.showinfo("Success", f"Processed {len(results)} video(s). Open batch_analysis_report.html in the directory for links.")
                else:
                    ...
                    if processed:
                        ...
                        messagebox.showinfo("Success", f"Processed {len(processed)} CSV(s) (angles only).")
                    else:
                        messagebox.showwarning("No Files", "No videos or valid landmark CSVs found in directory.")
...
def main():
    ...
    parser.add_argument("-i", "--input", help="Input: CSV file or Directory (with videos + landmark CSVs)")
    parser.add_argument("-v", "--video", help="Video path (required for CSV input if you want skeleton overlay and report)")
    ...
    if args.input:
        ...
        if os.path.isdir(path):
            ...
            if results:
                ...
                print(f"Processed {len(results)} video(s). See batch_analysis_report.html in the directory.")
            else:
            ...
            process_video_with_visualization(path, output_dir=args.output, filter_config=filter_config)
