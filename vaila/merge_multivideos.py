...
class VideoMergeApp:
    def __init__(self, root):
        ...
        self.video_list_frame = ttk.LabelFrame(self.main_frame, text="Selected Videos (in order of merging)")
        ...
        self.progress_bar = ttk.Progressbar(self.progress_frame, length=100, mode='indeterminate')
    ...
    def move_up(self):
        ...
        # Swap videos
        self.video_files[self.selected_index], self.video_files[self.selected_index-1] = \
            self.video_files[self.selected_index-1], self.video_files[self.selected_index]
    ...
    def start_merge(self):
        ...
        if not self.output_dir:
            messagebox.showerror("Error", "Output directory not set. Please select an output directory.")
            return
    ...
    def do_merge(self, output_video_path, output_subdir):
        ...
        try:
            ...
            # Update progress
            self.root.after(0, lambda: self.progress_label.config(text="Creating list of videos to merge..."))
            
            # Create FFmpeg command for merging
            ffmpeg_command = [
                "ffmpeg", "-y",  # Override output if exists
                "-f", "concat",
            ...
            # Update progress
            self.root.after(0, lambda: self.progress_label.config(text="Starting merge process..."))
            ...
            log_file_path = os.path.join(output_subdir, f"{os.path.basename(output_video_path)}_merge_info.txt")
            ...
            if process.returncode == 0:
                ...
                self.root.after(0, lambda: self.merge_complete(False, f"FFmpeg error: return code {process.returncode}"))
