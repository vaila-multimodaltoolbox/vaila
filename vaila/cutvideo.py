...
def play_video_with_cuts(video_path):
    ...
    def draw_controls():
        ...
        slider_y = 30
        slider_height = 10
        pygame.draw.rect(slider_surface, (60, 60, 60), 
                        (slider_x, slider_y, slider_width, slider_height))
        
        # Draw slider handle
        slider_pos = slider_x + int((frame_count / total_frames) * slider_width)
        pygame.draw.circle(slider_surface, (255, 255, 255),
                         (slider_pos, slider_y + slider_height // 2), 8)
        ...
        if current_start is not None:
            cut_text = font.render(f"Current Cut Start: {current_start}", True, (0, 255, 0))
    ...
    def save_and_generate_videos():
        ...
        if messagebox.askyesno("Generate Videos", 
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    print(f"Start frame marked: {frame_count}")  # Feedback apenas no terminal
                    ...
                    if frame_count > current_start:
                        ...
                        print(f"Cut marked: {current_start} to {frame_count}")  # Feedback apenas no terminal
                        current_start = None
                    else:
                        print("Error: End frame must be after start frame!")  # Feedback apenas no terminal
                    ...
                    if cuts:
                        cuts_info = "\n".join([f"Cut {i+1}: Frame {start} to {end}" 
                                             for i, (start, end) in enumerate(cuts)])
                        messagebox.showinfo("Cuts List", cuts_info)  # Mantém a janela apenas para listar
                    else:
