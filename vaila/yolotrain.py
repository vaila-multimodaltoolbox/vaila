...
class YOLOTrainApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.base_model = tk.StringVar(value="yolov8m.pt") # YOLOv8 medium model as default
    ...
    def create_widgets(self):
        ...
        # 1. Dataset Path
        tk.Label(self, text="Dataset Folder:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.dataset_path, width=60).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        tk.Button(self, text="Browse", command=self.browse_dataset).grid(row=0, column=2, padx=5, pady=5)

        # 2. YAML Path
        tk.Label(self, text="Dataset YAML File:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.yaml_path, width=60).grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        tk.Button(self, text="Browse", command=self.browse_yaml).grid(row=1, column=2, padx=5, pady=5)

        # 3. Base Model
        tk.Label(self, text="YOLO Base Model:").grid(row=2, column=0, sticky="e", padx=5, pady=5)
        tk.OptionMenu(self, self.base_model, "yolov8n.pt", "yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt").grid(row=2, column=1, padx=5, pady=5, sticky="w")

        # 4. Epochs
        tk.Label(self, text="Epochs:").grid(row=3, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.epochs, width=10, 
                 validate="key", validatecommand=(self.validate_numeric_cmd, '%P')).grid(row=3, column=1, padx=5, pady=5, sticky="w")

        # 5. Batch Size
        tk.Label(self, text="Batch Size:").grid(row=4, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.batch_size, width=10, 
                 validate="key", validatecommand=(self.validate_numeric_cmd, '%P')).grid(row=4, column=1, padx=5, pady=5, sticky="w")

        # 6. Image Size
        tk.Label(self, text="Image Size (px):").grid(row=5, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.img_size, width=10, 
                 validate="key", validatecommand=(self.validate_numeric_cmd, '%P')).grid(row=5, column=1, padx=5, pady=5, sticky="w")

        # 7. Device (GPU/CPU)
        tk.Label(self, text="Device:").grid(row=6, column=0, sticky="e", padx=5, pady=5)
        tk.OptionMenu(self, self.device, "cpu", "cuda").grid(row=6, column=1, padx=5, pady=5, sticky="w")

        # 8. Project Name (for the output folder in runs/train)
        tk.Label(self, text="Run Name:").grid(row=7, column=0, sticky="e", padx=5, pady=5)
        tk.Entry(self, textvariable=self.project_name, width=30).grid(row=7, column=1, padx=5, pady=5, sticky="w")
        ...
        self.start_button.grid(row=9, column=0, columnspan=3, pady=20, sticky="nsew") # Expand in all directions

        # Training Output Console (to display training logs)
        tk.Label(self, text="Training Output:").grid(row=10, column=0, columnspan=3, sticky="w", padx=5, pady=5)
        self.console = scrolledtext.ScrolledText(self, height=15, wrap="word") # 'wrap="word"' avoids word breaks at the end of the line
        self.console.grid(row=11, column=0, columnspan=3, padx=10, pady=5, sticky="nsew") # Expand in all directions
    ...
    def browse_dataset(self):
        ...
        path = filedialog.askdirectory(title="Select the Root Directory of your YOLO Dataset")
    ...
    def browse_yaml(self):
        ...
        self.attributes("-topmost", False) # Remove topmost before opening dialog
        file = filedialog.askopenfilename(title="Select your dataset.yaml file", filetypes=[("YAML files", "*.yaml *.yml")])
        self.attributes("-topmost", True) # Restore topmost after dialog is closed
        if file:
    ...
    def _run_training_logic(self):
        ...
        try:
            ...
            if not yaml_file or not os.path.exists(yaml_file):
                messagebox.showerror("Error", f"Dataset YAML file not found: {yaml_file}")
                return
            ...
            try:
                ...
                if epochs <= 0 or batch <= 0 or imgsz <= 0:
                    messagebox.showerror("Error", "Epochs, Batch Size and Image Size must be positive numbers.")
                    return
            except ValueError:
                messagebox.showerror("Error", "Please insert valid numeric values for Epochs, Batch Size and Image Size.")
                return

            if selected_device == "cuda" and not torch.cuda.is_available():
                messagebox.showwarning("Warning", "CUDA (GPU) selected, but not available. Training will be executed on CPU.")
                selected_device = "cpu" # Forces to CPU

            # Basic validation for run_name
            if not run_name or not re.match(r"^[a-zA-Z0-9_-]+$", run_name):
                 messagebox.showerror("Error", "Invalid run name. Use only letters, numbers, '-' or '_'.")
                 return
            ...
            best_model_path = os.path.join(output_project_dir, run_name, 'weights', 'best.pt')
