...
def calibrate_camera_from_images(calib_files, pattern_size, square_size, show_images=False):
    ...
    with open(os.path.join(output_dir, "camera_parameters.csv"), 'w') as f:
        ...
        f.write(f"{fx:.2f},{fy:.2f},{cx:.2f},{cy:.2f},{k1:.17f},{k2:.17f},{p1:.17f},{p2:.17f},{k3:.17f}\n")
...
def undistort_image(img_path, camera_matrix, dist_coeffs, alpha, output_dir=None):
    ...
    for a in alpha_values:
        ...
        cv2.putText(resized, f"Alpha: {a:.1f}", (10, 30), 
                 cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
        thumbs.append(resized)
    
    # Organizar em grid (2 linhas x 3 colunas)
    row1 = np.hstack(thumbs[:3])
    row2 = np.hstack([np.zeros_like(thumbs[0]) if len(thumbs) <= 3 else thumbs[3]] +
    ...
    # Ajustar à tela
    scale = min(screen_width / show_comparison.shape[1], 
        ...
        if found:
            ...
            for i in range(len(corners) - 1):
                if i % pattern_size[0] != pattern_size[0] - 1:  # Do not calculate between lines
                    dist = np.sqrt((corners[i+1][0][0] - corners[i][0][0])**2 + 
                                   (corners[i+1][0][1] - corners[i][0][1])**2)
                    pixel_sizes.append(dist)
            
            avg_size = np.mean(pixel_sizes)
            print(f"{os.path.basename(f)}: Found {len(corners)} corners, average size: {avg_size:.1f} pixels")
            
            # Show image with detected corners
            cv2.drawChessboardCorners(img, pattern_size, corners, found)
            cv2.putText(img, f"Corners: {len(corners)}, Size: {avg_size:.1f}px", 
...
def enhanced_chessboard_detection(calib_files, pattern_sizes=None):
    ...
    for f in calib_files:
        ...
        # Methods to try
        methods = [
            ("Normal", gray, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE),
            ("Enhanced", enhanced, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE),
            ("FILTER_QUADS", gray, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FILTER_QUADS),
            ("Enhanced+FILTER_QUADS", enhanced, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FILTER_QUADS),
            ("SimpleBLOB", gray, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE + cv2.CALIB_CB_FILTER_QUADS)
        ]
        ...
        if best_success and best_corners is not None and best_pattern is not None:
            ...
            try:
                pattern_size = (int(best_pattern[0]), int(best_pattern[1]))  # Ensure it is an integer tuple
...
class App(tk.Tk):
    def __init__(self):
        ...
        row = 0
        tk.Label(main_frame, text="Internal corners (cols, rows):").grid(row=row, column=0, sticky="w")
        ...
        tk.Checkbutton(main_frame, text="Show images during calibration", 
                      variable=self.show_images).grid(row=row, column=0, columnspan=2, sticky="w")

        # Preset parameters
        row += 1
        self.use_preset = tk.BooleanVar(value=False)
        tk.Checkbutton(main_frame, text="Use preset camera parameters", 
                      variable=self.use_preset).grid(row=row, column=0, columnspan=2, sticky="w")
        ...
        tk.Button(buttons_frame, text="Select calibration", command=self.load_calib,
                 width=15).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
        tk.Button(buttons_frame, text="Process image", command=self.run_undistort,
                 width=15).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
        ...
        tk.Button(buttons_frame2, text="Analyze board", command=self.analyze_board,
                 width=15).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
        tk.Button(buttons_frame2, text="Debug Detection", command=self.debug_detection,
                 bg="orange", width=15).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
