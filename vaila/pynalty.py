...
def rec2d(A, cc2d):
    ...
    for k in range(nlin):
        ...
        cc2d1 = np.matrix(
            [
                [A[0, 0] - x * A[6, 0], A[1, 0] - x * A[7, 0]],
                [A[3, 0] - y * A[6, 0], A[4, 0] - y * A[7, 0]],
            ]
        )
...
class PynaltyApp:
    ...
    def draw_content(self):
        ...
        if self.feedback_timer > 0:
            ...
            self.screen.blit(
                msg_surf, (cx - msg_surf.get_width() // 2, cy - msg_surf.get_height() // 2)
            )
    ...
    def draw_event_table(self):
        ...
        # List Events
        curr_y = y + 35
        for _i, idx in enumerate(range(len(self.events))):
            evt = self.events[idx]
            color = WHITE
            prefix = "  "
            ...
            if evt.is_done:
                status = "[x]"
            if idx == 0 and evt.frame_idx != -1:
                status = "[x]"  # Special case for simple events
            if idx == 3 and len(evt.points.get("points", [])) == 4:
                status = "[x]"
            if (
                (idx == 1 or idx == 2)
                and "Ball" in evt.points
                and "GK" in evt.points
                and evt.frame_idx != -1
            ):
                status = "[x]"
    ...
    def all_events_done(self):
        ...
        e2 = (
            self.events[1].frame_idx != -1
            and "Ball" in self.events[1].points
            and "GK" in self.events[1].points
        )
        e3 = (
            self.events[2].frame_idx != -1
            and "Ball" in self.events[2].points
            and "GK" in self.events[2].points
        )
    ...
    def calculate_and_show_results(self, y_start=100):
        try:
            ...
            dist, vel_ms, vel_kmh = distvelball_penalti(
                goal_ball_real[0, 0], goal_ball_real[0, 1], delta_frames, self.fps
            )
            ...
            gk_vel_ms = gk_dist / delta_jump_frames * self.fps if delta_jump_frames > 0 else 0
            ...
            self.last_results = {
                ...
                "vel_kmh": vel_kmh,
                "delta": delta_frames,
                "coord_x": goal_ball_real[0, 0],
                "coord_z": goal_ball_real[0, 1],
        ...
        if file_path:
            try:
                with open(file_path, "w", newline="") as f:
                    ...
                    writer.writerow(
                        ["BALL Velocity MS", f"{self.last_results['vel_ms']:.4f}", "m/s"]
                    )
                    writer.writerow(
                        ["BALL Velocity KMH", f"{self.last_results['vel_kmh']:.4f}", "km/h"]
                    )
                    writer.writerow(["BALL Delta Frames", self.last_results["delta"], "frames"])

                    writer.writerow(
                        ["GK Response Time", f"{self.last_results['gk_response_time']:.4f}", "s"]
                    )
                    writer.writerow(["GK Jump Dist", f"{self.last_results['gk_dist']:.4f}", "m"])
                    writer.writerow(
                        ["GK Velocity MS", f"{self.last_results['gk_vel_ms']:.4f}", "m/s"]
                    )
                    writer.writerow(
                        ["GK Velocity KMH", f"{self.last_results['gk_vel_kmh']:.4f}", "km/h"]
                    )
    ...
    def save_toml(self):
        ...
        data = {
            ...
            "events": [
                {"name": e.name, "frame_idx": e.frame_idx, "points": e.points} for e in self.events
            ],
                ...
                # Keyboard
                    ...
                    if event.key == pygame.K_RIGHT:
                        ...
                        self.current_frame_idx = min(
                            self.total_frames - 1, self.current_frame_idx + 10
                        )
                        self.update_frame()
                    ...
                    # State transitions
                        ...
                        self.current_event_idx = (self.current_event_idx + direction) % len(
                            self.events
                        )
                    ...
                    # Zoom Keys
                        ...
                        new_fps = simpledialog.askfloat(
                            "Input", "Enter Video FPS:", initialvalue=self.fps
                        )
                        if new_fps:
                            self.fps = new_fps
    ...
    def save_results_package(self):
        ...
        def draw_kick_overlay(img):
            ...
            if "GK" in e2.points:
                p = e2.points["GK"]
                cv2.circle(
                    img, (int(p[0]), int(p[1])), 8, (255, 255, 0), -1
                )  # BGR: Cyan is (255, 255, 0)
                cv2.putText(
                    img,
                    "Kick GK",
                    (int(p[0]) + 10, int(p[1])),
        ...
        self.save_snapshot(
            e3.frame_idx, "snapshot_calibration.png", out_dir, draw_calib_overlay
        )  # Use Goal Frame Context

        # 2. TOML
        toml_path = os.path.join(out_dir, "data.toml")
        with open(toml_path, "w") as f:
            # Construct data dict
            data = {"video": self.video_path, "fps": self.fps, "events": []}
            for e in self.events:
                data["events"].append(
                    {"name": e.name, "frame_idx": e.frame_idx, "points": e.points}
                )
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            if hasattr(self, "last_results") and self.last_results:
                ...
                if "Ball" in e2.points:
                    writer.writerow(["Kick Ball", e2.points["Ball"][0], e2.points["Ball"][1]])
                if "GK" in e2.points:
                    writer.writerow(["Kick GK", e2.points["GK"][0], e2.points["GK"][1]])
                if "Ball" in e3.points:
                    writer.writerow(["Goal Ball", e3.points["Ball"][0], e3.points["Ball"][1]])
                if "GK" in e3.points:
                    writer.writerow(["Goal GK", e3.points["GK"][0], e3.points["GK"][1]])
