...
def get_box_coordinates(image_path, video_path=None):
    ...
    selection_mode = {"mode": "inside", "shape": "rectangle", "color": (0, 0, 0)}  # Default black
    ...
    ax_color.text(0.5, -0.2, 'Color Preview', ha='center', fontsize=10, transform=ax_color.transAxes)
    ...
    def update_title():
        ...
        ax.set_title(
            f'Drawing Shape #{current_id} - Press Enter when done',
            fontsize=12
        )
        fig.canvas.draw_idle()
    ...
    def update_status():
        ...
        status_text.set_text(f"ID: {polygon_counter['current']}/{polygon_counter['total']} | {current_shape} | {current_mode}")
        fig.canvas.draw_idle()
    ...
    def on_save_config(event):
        if video_path and shapes:
            try:
                    ...
                    if len(selections) > len(current_selections):
                        ...
                        if current_selection[1] == 'free':
                            if free_index < len(free_polygon_sizes):
                                ...
                                box_points = [(points[j][0], points[j][1]) for j in range(i, i + n_points - 1)]
                ...
                config_path = save_config_toml(video_path, current_coords, current_selections, current_colors)
...
• Use "Help" button for more information
        ...
        if config_path and os.path.exists(config_path):
            try:
                ...
                if loaded_coords:
                    ...
                    # Load saved shapes
                    for coords, selection, color in zip(loaded_coords, loaded_selections, loaded_colors):
        ...
        if event.button == 1:  # Left button for definition
            if selection_mode["shape"] == "rectangle":
                ...
                if len(temp_points) == 1:
                    ...
                    ax.add_patch(rect)
                    shapes.append(rect)
                    selections.append((selection_mode["mode"], "rectangle", validate_color(selection_mode["color"])))
                    polygon_counter["current"] = len(shapes)
                    temp_points.clear()
                    clear_temp_shapes()
                    update_status()
    ...
    def on_motion(event):
        # Preview rectangle
        if not pick_mode["active"] and len(temp_points) == 1 and selection_mode["shape"] == "rectangle":
        ...
        if pick_mode["active"] and pick_rect["start"]:
            ...
            if x1 and y1 and x2 and y2:
                ...
                if region.size > 0:
                    ...
                    if len(avg_color) >= 3:
                        # RGB or RGBA
                        selection_mode["color"] = (float(avg_color[0]), float(avg_color[1]), float(avg_color[2]))
...
def run_drawboxe():
    ...
    coordinates, selections, colors = get_box_coordinates(first_frame_path, os.path.join(video_directory, first_video))
