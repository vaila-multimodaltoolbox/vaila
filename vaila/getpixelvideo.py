...
New in 0.4.2:
  ...
  Ctrl+Z undoes discrete marker edits; Restore jumps to last Save (or open).
...
# Product mark: always lowercase with acute accent; use italic=True when drawing with pygame.font.
VAILA_MARK = "vailá"
...
# In-memory undo depth for INSERT-mode marker edits (Ctrl+Z).
MARKER_UNDO_STACK_MAX = 50


def clone_marker_editor_state(
    *,
    coordinates: dict[int, list] | None,
    deleted_positions: dict[int, set[int]] | None,
    one_line_markers: list,
    deleted_markers: set[int],
    bboxes: dict,
    selected_marker_idx: int,
) -> dict[str, Any]:
    """Deep-copy marker editor state for undo / restore checkpoints."""
    coords_copy: dict[int, list] = {}
    if isinstance(coordinates, dict):
        for frame_idx, pts in coordinates.items():
            coords_copy[int(frame_idx)] = [tuple(p) if p is not None else (None, None) for p in pts]
    deleted_copy: dict[int, set[int]] = {}
    if isinstance(deleted_positions, dict):
        for frame_idx, idxs in deleted_positions.items():
            deleted_copy[int(frame_idx)] = set(idxs)
    bboxes_copy: dict[int, list] = {}
    if isinstance(bboxes, dict):
        for frame_idx, boxes in bboxes.items():
            bboxes_copy[int(frame_idx)] = [dict(box) for box in boxes]
    return {
        "coordinates": coords_copy,
        "deleted_positions": deleted_copy,
        "one_line_markers": [tuple(m) for m in one_line_markers],
        "deleted_markers": set(deleted_markers),
        "bboxes": bboxes_copy,
        "selected_marker_idx": int(selected_marker_idx),
    }


def push_marker_undo(stack: list[dict[str, Any]], state: dict[str, Any]) -> list[dict[str, Any]]:
    """Append ``state`` to undo stack, trimming to ``MARKER_UNDO_STACK_MAX``."""
    stack.append(state)
    if len(stack) > MARKER_UNDO_STACK_MAX:
        del stack[: len(stack) - MARKER_UNDO_STACK_MAX]
    return stack


def pop_marker_undo(stack: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Pop and return the last undo state, or None if empty."""
    if not stack:
        return None
    return stack.pop()
...
def compute_letterbox_pads(
    zoomed_width: int, zoomed_height: int, window_width: int, window_height: int
) -> tuple[int, int]:
    """Return (pad_x, pad_y) to center zoomed media inside the video viewport."""
    pad_x = max(0, (int(window_width) - int(zoomed_width)) // 2)
    pad_y = max(0, (int(window_height) - int(zoomed_height)) // 2)
    return pad_x, pad_y
...
def marked_count_hud_pos(
    ...
    *,
    ...
    clear_left_of: int | None = None,
) -> tuple[int, int]:
    """Top-right control-panel coords for the green ``M0:… · other:…`` strip hint.

    Prefer the empty margin right of the centered toolbar. When that margin is too
    narrow, still right-align but never sit left of ``clear_left_of`` (toolbar edge),
    so Go KP / Lock / Help stay uncovered.
    """
    x = int(window_width) - int(text_width) - int(margin_right)
    if clear_left_of is not None:
        x = max(x, int(clear_left_of) + 8)
    y = int(cluster_y_top) + max(0, (int(button_height) - int(text_height)) // 2)
    return x, y


def screen_to_video_coords(
    sx: float,
    sy: float,
    *,
    zoom_level: float,
    ...
    """Map screen (viewport) pixels to video/frame coordinates."""
    z = float(zoom_level) if zoom_level else 1.0
    return (
        (float(sx) - float(pad_x) + float(crop_x)) / z,
        (float(sy) - float(pad_y) + float(crop_y)) / z,
    )


def video_to_screen_coords(
    vx: float,
    vy: float,
    *,
    zoom_level: float,
    ...
    """Map video/frame coordinates to integer screen (viewport) pixels."""
    z = float(zoom_level) if zoom_level else 1.0
    return (
        int(float(vx) * z - float(crop_x) + float(pad_x)),
        int(float(vy) * z - float(crop_y) + float(pad_y)),
    )
...
def save_swap_toml(toml_path, swap_rules):
    ...
    try:
        ...
        with open(toml_path, "w") as f:
            ...
            f.write(f"# Generated by {VAILA_MARK} getpixelvideo.py\n\n")
...
def pygame_file_dialog(
    ...
    # Restore original window size and product caption (dialog sets "Select File").
    if restore_size:
...
def play_video_with_controls(
    ...
    window_height = max(
        min_initial_h - control_panel_height, min(original_height, screen_height - 170)
    )
    ...
    pad_x, pad_y = 0, 0  # letterbox; recomputed each frame from zoomed size
    ...
    timeline_drag_mf_snapshot: list[int] | None = None
    ...
    editor_mode = "visual"  # "visual" | "insert"
    ...
    track_ai_lost_streak: int = 0
    # Frames already holding a manually-placed point for the live-tracked marker
    # at the moment AI Track was armed -- live tracking must never overwrite
    # these (mirrors run_ai_batch_tracking's known_points protection).
    live_track_protected_frames: set[int] = set()
    # Shape drag (circle/box): define block_window size with mouse drag
    track_shape_dragging: bool = False
    track_shape_drag_origin: tuple[float, float] | None = None
    track_shape_drag_current: tuple[float, float] | None = None
    # Keyboard hold navigation
    nav_jump_frames: int = 60
    _nav_hold_key: int | None = None
    _nav_hold_started_ms: int = 0
    _nav_hold_last_ms: int = 0
    _NAV_HOLD_DELAY_MS = 250
    _NAV_HOLD_REPEAT_LR_MS = 45
    _NAV_HOLD_REPEAT_UD_MS = 90
    ...
    # Pitch Guide corner map: keep turf/lines translucent so the video stays readable underneath.
    _pg_fill = (14, 74, 35, 96)
    _pg_line = (245, 245, 245, 150)
    _pg_border = (230, 230, 230, 165)
    _pg_spot = (245, 245, 245, 195)
    # Marker labels / rings: avoid black-on-black when blended over video + green turf
    _pg_marker_ring = (28, 92, 42, 240)
    _pg_label_text = (255, 235, 72)
    _pg_label_text_highlight = (18, 82, 32)  # on bright yellow current-marker dot
    ...
    def _pitch_guide_reference_surface(current_idx: int | None = None) -> pygame.Surface | None:
        ...
        def draw_line(name_a, name_b, line_width=2):
            if name_a in point_by_name and name_b in point_by_name:
                pygame.draw.line(surface, _pg_line, pxy(name_a), pxy(name_b), line_width)
        ...
        def draw_rect(name_a, name_b, line_width=2):
            ...
            pygame.draw.rect(surface, _pg_line, rect, line_width)
        ...
        def draw_circle_at(name, radius_m, line_width=2, filled=False):
            ...
            if filled:
                pygame.draw.circle(surface, _pg_spot, center, radius_px)
            else:
                pygame.draw.circle(surface, _pg_line, center, radius_px, line_width)
        ...
        def draw_arc_from_points(center_name, lower_name, upper_name, side):
            ...
            if len(samples) > 1:
                pygame.draw.lines(surface, _pg_line, False, samples, 2)
        ...
        for idx, point in enumerate(pitch_guide_points):
            ...
            radius = 7 if is_current else 4
            fill = (255, 225, 35, 245) if is_current else (255, 255, 255, 235)
            pygame.draw.circle(surface, _pg_marker_ring, (px, py), radius + 1)
        ...
        pygame.draw.rect(surface, _pg_border, surface.get_rect(), 2)
        return surface
    ...
    def _pitch_guide_reference_surface_fifa(
        ...
        def rect_from_two(name_a, name_b, w=2):
            ...
            pygame.draw.rect(surface, _pg_line, r, w)
        ...
        if all(
            ...
            radius_px = max(2, int((radius_m / span_y) * (height - 2 * margin_y)))
            pygame.draw.circle(surface, _pg_line, center_px, radius_px, 2)
            pygame.draw.circle(surface, _pg_spot, center_px, 3)
        # Penalty spots
    ...
    def run_subspace_rts_tracking():
        ...
        for f in range(total_frames):
            if coordinates is not None and f in coordinates and target_marker < len(coordinates[f]):
        ...
        _flush_save_message(screen, f"Initializing AI Tracker for Marker {target_marker}...")
    ...
    def select_track_ai_weights() -> None:
        ...
        track_ai_params.resnet_variant = inferred_variant
    ...
    def _handle_track_ai_toml_dialog() -> None:
        ...
        try:
            ...
            total_h = window_height + control_panel_height
            dialog_w = min(560, window_width - 40)
            dialog_h = 280
            ...
            rect_weights = pygame.Rect(btn_start_x + 2 * (btn_w + btn_spacing), btn_y, btn_w, btn_h)
            rect_reset = pygame.Rect(btn_start_x + 3 * (btn_w + btn_spacing), btn_y, btn_w, btn_h)
            ...
            def _do_save_action() -> None:
                ...
                base_v = (
                    os.path.splitext(os.path.basename(video_path))[0] if video_path else "track_ai"
                )
                ...
                # Append nav_jump so Cfg round-trips the arrow-up step
                try:
                    with open(out_file, "a", encoding="utf-8") as tf:
                        tf.write(f"\n[navigation]\nnav_jump_frames = {int(nav_jump_frames)}\n")
                except OSError:
                    pass
                print(f">> AI Track: Parameters saved to {out_file}")
                save_message_text = f"AI Track: Saved {os.path.basename(out_file)}"
                showing_save_message = True
                save_message_timer = 90
                ...
                if in_file and os.path.isfile(in_file):
                    ...
                    try:
                        ...
                        with open(in_file, "rb") as tf:
                            raw = tomllib.load(tf)
                        nav_cfg = raw.get("navigation", {})
                        if "nav_jump_frames" in nav_cfg:
                            nav_jump_frames = max(1, int(nav_cfg["nav_jump_frames"]))
                    except Exception:
                        pass
                    print(f">> AI Track: Parameters loaded from {in_file}")
                    save_message_text = f"AI Track: Loaded {os.path.basename(in_file)}"
                    showing_save_message = True
                    save_message_timer = 90
            ...
            def _do_reset_action() -> None:
                ...
                if live_tracker is not None:
                    live_tracker.params = track_ai_params
                print(">> AI Track: Parameters reset to fast CPU defaults (NCC + Spatial Prior)")
                save_message_text = "AI Track: Reset to fast defaults (CPU)"
                showing_save_message = True
                save_message_timer = 90
            ...
            while dialog_open:
                ...
                t_title = font_title.render("AI Track – Parameters (.toml)", True, (255, 255, 255))
                ...
                t_jump = font_hint.render(
                    f"Nav jump (Up/Down arrows): {nav_jump_frames} frames   [-] / [+] or buttons:",
                    True,
                    (180, 190, 210),
                )
                screen.blit(t_jump, (dx + 110, dy + 128))
                pygame.draw.rect(screen, (70, 75, 90), rect_jump_down, border_radius=4)
                pygame.draw.rect(screen, (70, 75, 90), rect_jump_up, border_radius=4)
                screen.blit(
                    font_btn.render("-", True, (255, 255, 255)),
                    (rect_jump_down.x + 12, rect_jump_down.y + 2),
                )
                screen.blit(
                    font_btn.render("+", True, (255, 255, 255)),
                    (rect_jump_up.x + 11, rect_jump_up.y + 2),
                )
                ...
                screen.blit(t_hint, (dx + (dialog_w - t_hint.get_width()) // 2, dy + dialog_h - 18))
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key in (pygame.K_ESCAPE, pygame.K_c):
                            ...
                            nav_jump_frames = max(1, nav_jump_frames - 10)
                        elif event.key in (pygame.K_PLUS, pygame.K_EQUALS, pygame.K_KP_PLUS):
                            nav_jump_frames = min(600, nav_jump_frames + 10)
                        ...
                        if rect_close.collidepoint(event.pos):
                            dialog_open = False
                        elif rect_jump_down.collidepoint(event.pos):
                            nav_jump_frames = max(1, nav_jump_frames - 10)
                        elif rect_jump_up.collidepoint(event.pos):
                            nav_jump_frames = min(600, nav_jump_frames + 10)
            ...
            print(f">> AI Track TOML dialog error: {ex_dlg}")
    ...
    def toggle_track_ai_deep(explicit_state: bool | None = None) -> None:
        ...
        showing_save_message = True
        save_message_timer = 60
        print(f">> AI Track Deep checkbox: {state_str}")
    ...
    def toggle_track_ai(explicit_state: bool | None = None) -> None:
        ...
        if not new_state:
            ...
            track_ai_active = False
            live_tracker = None
            # Keep marker_selection_locked as-is. Unlock only via B key or Lock
            # button — turning Track AI OFF must not clear a deliberate (or
            # auto-on) lock.
        ...
        if anchor_pt is None or anchor_frame is None:
            ...
            live_tracker = None
            live_track_target_marker = target_marker
            save_message_text = "Track AI [ON]: click video to anchor. (Ctrl+W: backbone weights)"
            showing_save_message = True
            save_message_timer = 200
            print(
        ...
        # Snapshot frames already manually measured for this marker -- live
        # tracking must skip these instead of overwriting them (mirrors
        # run_ai_batch_tracking's known_points protection for the batch path).
        live_track_protected_frames = set()
        if coordinates is not None:
            for f_idx, pts in coordinates.items():
                if target_marker < len(pts):
                    pt_p = pts[target_marker]
                    if (
                        pt_p is not None
                        and pt_p[0] is not None
                        and pt_p[1] is not None
                        and target_marker not in deleted_positions.get(f_idx, set())
                    ):
                        live_track_protected_frames.add(f_idx)
    ...
    def _perform_live_ai_tracking(curr_frame_idx: int, curr_frame_img: np.ndarray) -> None:
        ...
        if not _can_edit_markers():
            return
        if (
            not track_ai_active
            or live_tracker is None
            or curr_frame_img is None
        ...
        # If user explicitly switched to another marker that has a point on current frame
            ...
            if (
                ...
                try:
                    if hasattr(live_tracker, "update_from_correction"):
                        live_tracker.update_from_correction(
                            curr_frame_img, (float(pt_sw[0]), float(pt_sw[1])), curr_frame_idx
                        )
                    else:
                        live_tracker.add_anchor(
                            curr_frame_img, (float(pt_sw[0]), float(pt_sw[1])), curr_frame_idx
                        )
                        live_tracker.retrain_online_model()
                        live_tracker.set_reference(
                            curr_frame_img,
                            (float(pt_sw[0]), float(pt_sw[1])),
                            reset_online=False,
                        )
        ...
        # Ground truth preservation: if this frame already has a coordinate, re-anchor without wiping
            ...
            if (
                ...
                if curr_frame_idx != live_track_last_frame:
                    try:
                        if hasattr(live_tracker, "update_from_correction"):
                            live_tracker.update_from_correction(
                                curr_frame_img, (float(pt[0]), float(pt[1])), curr_frame_idx
                            )
                        else:
                            live_tracker.add_anchor(
                                curr_frame_img, (float(pt[0]), float(pt[1])), curr_frame_idx
        ...
        if (
            0 < (curr_frame_idx - live_track_last_frame) <= max_allowed_gap
            and live_track_last_frame >= 0
        ):
            prev_pt = None
            if live_track_last_frame in coordinates and target_m < len(
                coordinates[live_track_last_frame]
            ):
            ...
            if prev_pt is not None:
                try:
                    ...
                    accepted = getattr(res, "accepted", True)
                    if accepted and live_track_protected_frames:
                        # Trajectory plausibility vs. a manually-measured frame
                        # ahead: reject a candidate that strays far off the
                        # straight-line path toward known ground truth instead
                        # of letting it run to a lateral false match through
                        # the unmeasured gap leading up to it.
                        lookahead_limit = curr_frame_idx + 60
                        anchor_ahead = min(
                            (
                                f
                                for f in live_track_protected_frames
                                if curr_frame_idx < f <= lookahead_limit
                            ),
                            default=None,
                        )
                        if anchor_ahead is not None:
                            pts_ahead = coordinates.get(anchor_ahead, [])
                            if (
                                target_m < len(pts_ahead)
                                and pts_ahead[target_m][0] is not None
                                and pts_ahead[target_m][1] is not None
                                and target_m not in deleted_positions.get(anchor_ahead, set())
                            ):
                                ax, ay = (
                                    float(pts_ahead[target_m][0]),
                                    float(pts_ahead[target_m][1]),
                                )
                                span = anchor_ahead - live_track_last_frame
                                frac = (
                                    (curr_frame_idx - live_track_last_frame) / span
                                    if span > 0
                                    else 1.0
                                )
                                exp_x = prev_pt[0] + (ax - prev_pt[0]) * frac
                                exp_y = prev_pt[1] + (ay - prev_pt[1]) * frac
                                dev = (
                                    (res.location[0] - exp_x) ** 2 + (res.location[1] - exp_y) ** 2
                                ) ** 0.5
                                search_w = getattr(
                                    getattr(live_tracker, "params", None),
                                    "search_window",
                                    (140, 140),
                                )
                                max_dev = max(search_w) * 0.9
                                if dev > max_dev:
                                    accepted = False
                    if not accepted:
                        track_ai_lost_streak += 1
                        if track_ai_lost_streak == 8:
                    ...
                    if not getattr(live_tracker, "_error_reported", False):
                        print(
                            f">> AI Track live tracking notice at frame {curr_frame_idx + 1}: {ex_tr}"
                        )
    ...
    def draw_controls():
        ...
        **Labeling**, **Guide** (field or skeleton overlay).
        ...
        def _modes_summary_text() -> str:
            ...
            parts: list[str] = [
                "INSERT" if editor_mode == "insert" else "VISUAL",
                core,
            ]
            if sequential_mode:
                parts.append("Seq")
        ...
        slider_height = 10
        scrub_bottom_pad = 5
        slider_y = control_surface_height - slider_height - scrub_bottom_pad
        ...
        # Marker timeline geometry (drawn after text bands so labels never overlap strip)
        heat_h = 8
        heat_gap_above_slider = 6
        heat_y = slider_y - heat_h - heat_gap_above_slider
        mf_timeline = sorted_frames_with_visible_markers(
        ...
        meta_gap = 7
        go_kp_h = 18
        primary_row_h = max(frame_info.get_height(), go_kp_h)
        ...
        meta_row_y = info_row_y - meta_gap - meta_row_h
        min_meta_y = buttons_band_bottom_y + 6
        if meta_row_y < min_meta_y:
            delta = min_meta_y - meta_row_y
            meta_row_y = min_meta_y
            info_row_y += delta

        info_x = slider_margin_left
        control_surface.blit(frame_info, (info_x, info_row_y))
        ...
        info_x += mouse_pix_txt.get_width() + 25
        if marker_info is not None:
            control_surface.blit(marker_info, (info_x, info_row_y))
            info_x += marker_info.get_width() + 25

        goto_marker_button_rect = pygame.Rect(
            info_x,
        ...
        # Geo Homog — standalone planar homography tracker (vaila/planar_geometry_tracker.py)
        geom_tracker_button_rect = pygame.Rect(
            info_x,
            info_row_y + (primary_row_h - go_kp_h) // 2,
            88,
            go_kp_h,
        )
        pygame.draw.rect(control_surface, (35, 125, 160), geom_tracker_button_rect)
        geom_tracker_text = font.render("Geo Homog", True, (255, 255, 255))
        control_surface.blit(
            geom_tracker_text, geom_tracker_text.get_rect(center=geom_tracker_button_rect.center)
        )
        info_x += geom_tracker_button_rect.width + 10
        ...
        if tracking_info is not None:
            ...
            mx_meta += tracking_info.get_width() + 14
        modes_font = pygame.font.SysFont("verdana", 11)
        speed_right_x = window_width - speed_text.get_width() - slider_margin_right
        max_modes_w = max(72, speed_right_x - mx_meta - 20)
        modes_line = "Mode: " + _modes_summary_text()
        while len(modes_line) > 14 and modes_font.size(modes_line)[0] > max_modes_w:
            modes_line = modes_line[:-4].rstrip(" ·") + "…"
        modes_surface = modes_font.render(modes_line, True, (228, 218, 130))
        dy_m = meta_row_y + (meta_row_h - modes_surface.get_height()) // 2
        control_surface.blit(modes_surface, (mx_meta, dy_m))
        dy_s = meta_row_y + (meta_row_h - speed_text.get_height()) // 2
        control_surface.blit(
            speed_text,
            (window_width - speed_text.get_width() - slider_margin_right, dy_s),
        )

        # Timeline strip + scrub slider (after labels so geometry stays clear)
        pygame.draw.rect(
            control_surface,
            (38, 38, 38),
            (slider_margin_left, heat_y, slider_width, heat_h),
        )
        if mf_timeline and slider_width > 0:
            for px in range(slider_width):
                ...
                pygame.draw.line(
                    control_surface,
                    col,
                    (slider_margin_left + px, heat_y),
                    (slider_margin_left + px, heat_y + heat_h - 1),
                )
        cur_strip_x = slider_margin_left + int((frame_count / denom_frames) * slider_width)
        pygame.draw.line(
            control_surface,
            (255, 220, 70),
            (cur_strip_x, heat_y),
            (cur_strip_x, heat_y + heat_h - 1),
            2,
        )
        ...
        # Deep option: checkbox + label glued to the right of AI Track (not a standalone button).
        track_deep_check_size = 12 if is_compact else 14
        track_deep_label_w = 30 if is_compact else 36
        ...
        row_start_top = max(10, (window_width - total_top_width) // 2)
        row_start_bottom = max(10, (window_width - total_bottom_width) // 2)
        ...
        while (
            len(_place_cap) > 4 and _top_btn_font.size(_place_cap)[0] > marker_mode_button_width - 8
        ):
        ...
        persist_label = (
            "Persist" if is_compact else ("Persist ON" if persistence_enabled else "Persist")
        )
        ...
        # 3. AI Track button, then Deep checkbox to its right — didactic grouping.
        ...
        current_x += track_rts_button_width + 4
        ...
        # Hit target covers checkbox + "Deep" label (option for AI Track).
        ...
        _deep_box = pygame.Rect(
            current_x,
            cluster_y_bottom + (button_height - track_deep_check_size) // 2,
            track_deep_check_size,
            track_deep_check_size,
        )
        _deep_fill = (30, 150, 80) if track_ai_use_deep else (40, 40, 48)
        _deep_border = (220, 220, 230) if track_ai_use_deep else (140, 140, 150)
        pygame.draw.rect(control_surface, _deep_fill, _deep_box, border_radius=2)
        pygame.draw.rect(control_surface, _deep_border, _deep_box, 1, border_radius=2)
        if track_ai_use_deep:
            # Simple check mark inside the box
            _cx, _cy = _deep_box.center
            pygame.draw.lines(
                control_surface,
                (255, 255, 255),
                False,
        ...
        _deep_lbl = btn_font.render(
            "Deep", True, (230, 230, 235) if track_ai_use_deep else (170, 170, 180)
        )
        control_surface.blit(
            _deep_lbl,
            (
                _deep_box.right + 4,
                cluster_y_bottom + (button_height - _deep_lbl.get_height()) // 2,
            ),
        )
        current_x += track_deep_group_width + button_gap
        ...
        shape_cap = (
            "Pt" if track_ai_shape == "point" else ("Cir" if track_ai_shape == "circle" else "Box")
        )
        ...
        control_surface.blit(shape_text, shape_text.get_rect(center=track_shape_button_rect.center))
        ...
        if labeling_mode:
            ...
            control_surface.blit(class_info, (_global_info_x, info_row_y))
            _global_info_x += class_info.get_width() + 25

        # Green M0/other strip hint: top-right margin of the control panel (not info row).
        _toolbar_right = max(
            row_start_top + total_top_width,
            row_start_bottom + total_bottom_width,
        )
        marked_hint_x, marked_hint_y = marked_count_hud_pos(
            window_width,
            marked_cnt_txt.get_width(),
            marked_cnt_txt.get_height(),
            margin_right=slider_margin_right,
            cluster_y_top=cluster_y_top,
            button_height=button_height,
            clear_left_of=_toolbar_right,
        )
        control_surface.blit(marked_cnt_txt, (marked_hint_x, marked_hint_y))
        ...
        return (
            ...
            track_rts_button_rect,  # AI Track on/off
            ...
            geom_tracker_button_rect,  # Geo Homog: standalone planar homography tracker
    ...
    def _write_fifa_dataset_template(
        ...
        *,
        ...
        if not os.path.exists(yaml_path):
            yaml_content = (
                f"# YOLO pose dataset - generated by {VAILA_MARK} getpixelvideo (FIFA template)\n"
    ...
    def _save_fifa_template_toml(path: str) -> str:
        ...
        text = (
            ...
            "# Load this file with key K (when Template:FIFA) or via the FIFA TOML flow.\n\n"
            "[fifa_template]\n"
    ...
    def _apply_fifa_toml_config(path: str, *, marker_load_only: bool = False) -> tuple[bool, str]:
        """Load TOML config, generate template CSV, and optionally dataset scaffold.

        When ``marker_load_only`` is True (Load CSV auto-sync): apply only in-memory FIFA
        settings (base_index, start_keypoint, slot count, coord format). No disk writes,
        no session reset — keeps coordinates about to be loaded intact.
        """
        ...
        try:
            ...
            if marker_load_only:
                msg = f"FIFA settings from TOML applied for marker load: {os.path.basename(path)}"
            else:
    ...
    def _quick_fifa_preset_disable() -> None:
        """Turn off FIFA slot mode and compact sparse rows to variable-length marker lists."""
        nonlocal pitch_guide_fifa_mode, fifa_fixed_keypoints, fifa_start_keypoint, fifa_index_base
        nonlocal current_label, coordinates, deleted_positions, selected_marker_idx
    ...
    def show_help_dialog():
        ...
        help_entries: list[tuple[str, str, str]] = [
            ...
            ("Right Arrow", "Next frame (when paused); hold = continuous step", "item"),
            ("Left Arrow", "Previous frame (when paused); hold = continuous step", "item"),
            ("Shift + Right / Left", "Jump to next / previous frame with visible markers", "item"),
            (
                "Up Arrow / Down Arrow",
                "Jump ±nav_jump frames (default 60); hold = keep jumping",
                "item",
            ),
            ("Cfg → Nav jump", "Change Up/Down step size (10–600, default 60)", "item"),
            ...
            (
                "Scrub Bar (bottom slider)",
                "Click or drag slider to jump directly to any frame",
                "item",
            ),
            ("", "", "blank"),
            ...
            (
                "Mode button",
                "Cycle Mode: Mark (normal) / Seq (sequential) / 1-line (outline)",
                "item",
            ),
            ("C", "Toggle 1-Line mode (creates sequence of points on single frame)", "item"),
            (
                "S  /  O",
                "Toggle Sequential mode (auto-advances marker index on each click)",
                "item",
            ),
            (
                "M  /  'MousePlay' button",
            ...
            (
                "ClickPass button",
                "Advance to next frame automatically after placing marker",
                "item",
            ),
            (
                "Guide (G key / button)",
            ...
            (
                "T  /  'AI Track' button",
                "Toggle interactive frame-by-frame AI tracking (ON/OFF)",
                "item",
            ),
            (
            ...
            (
                "  Online Adaptation",
                "Every click is an anchor: retrains online model without wiping history",
                "item",
            ),
            (
                "Shp: Cir/Box + drag",
                "Drag on video to set circle/box size; marker at region centroid",
                "item",
            ),
            (
                "Shift+T  /  Shift+Click",
                "Run full-video bidirectional batch tracking + RTS smoother",
                "item",
            ),
            (
                "Deep checkbox (right of AI Track)",
                "Optional deep-backbone features for AI Track (OFF = NCC/CPU; ON = ResNet etc.)",
                "item",
            ),
            (
                "'Cfg' button  /  Ctrl+T",
            ...
            (
                "Click / Drag on Strip",
                "Jump directly to frame (snaps to middle of marked frame cluster)",
                "item",
            ),
            (
                "Footer Status Row",
            ...
            (
                "I  /  'FPS' button",
                "Override or calibrate video sampling rate (Hz) for velocity/accel",
                "item",
            ),
            (
                "Left Click  /  Right Click",
                "Add measurement point / Undo last placed point",
                "item",
            ),
            (
                "1 / 2 / 3 / 4 / 5",
            ...
            (
                "L  /  'Labeling' button",
                "Toggle Bounding Box Labeling mode for YOLO object detection",
                "item",
            ),
            ...
            (
                "F5  /  Save button",
                "Export object detection dataset (train / val / test splits)",
                "item",
            ),
            (
                "F6  /  F7",
                "Load labeling JSON project / Load dataset folder to append (multi-video)",
                "item",
            ),
            ...
            (
                "BBox->Coords button",
                "Convert loaded tracking bounding boxes into marker coordinate CSVs",
                "item",
            ),
            (
                "Load  /  Save buttons",
        ...
        while waiting_for_input:
            ...
            pygame.draw.rect(
                overlay,
                (45, 115, 180) if btn_hover else (32, 75, 120),
                btn_open_html_rect,
                border_radius=4,
            )
            pygame.draw.rect(
                overlay,
                (70, 160, 235) if btn_hover else (50, 105, 165),
                btn_open_html_rect,
                width=1,
                border_radius=4,
            )
    ...
    def _delete_marker_range_dialog() -> tuple[bool, str]:
        ...
        if not _can_edit_markers():
            return False, "VISUAL — Ctrl+I to edit"
        if labeling_mode:
    ...
    def _run_geometric_tracker_action() -> tuple[bool, str]:
        ...
        print(f">>   {' '.join(cmd)}")
        _flush_save_message(screen, "Calculating Planar Geometry & Homography...")
        try:
        ...
        while browsing:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # Scroll wheel buttons (legacy pygame 1.x style)
                    ...
                    if event.button == 5:  # scroll down
                        ...
                        continue
                    # Path bar / file list: left-click only
                    if event.button != 1:
                        continue
                    # Click on path bar?
    ...
    def _resolve_neighbor_fifa_toml(marker_csv_path: str) -> str | None:
        """Resolve ``{stem}_fifa_template.toml`` beside the open video or beside the CSV."""
        candidates: list[str] = []
        if video_path and not os.path.isdir(video_path):
            vstem = os.path.splitext(os.path.basename(video_path))[0]
            candidates.append(
                os.path.join(os.path.dirname(video_path), f"{vstem}_fifa_template.toml")
            )
        csv_dir = os.path.dirname(marker_csv_path) or "."
        csv_fn = os.path.splitext(os.path.basename(marker_csv_path))[0]
        if csv_fn.endswith("_markers"):
            candidates.append(
                os.path.join(csv_dir, f"{csv_fn[: -len('_markers')]}_fifa_template.toml")
            )
        candidates.append(os.path.join(csv_dir, f"{csv_fn}_fifa_template.toml"))
        seen: set[str] = set()
        for c in candidates:
            if not c or c in seen:
                continue
            seen.add(os.path.normpath(c))
            if os.path.isfile(c):
                return c
        return None
    ...
    # ESC=quit | Space=play/pause | Left/Right/Up/Down=seek | Shift+Left/Right=marked frames
    # marker timeline strip: click/drag to jump (control panel)
    ...
    def _clone_marker_state() -> dict[str, Any]:
        return clone_marker_editor_state(
            coordinates=coordinates,
            deleted_positions=deleted_positions,
            one_line_markers=one_line_markers,
            deleted_markers=deleted_markers,
    ...
    def _apply_marker_state(state: dict[str, Any]) -> None:
        nonlocal coordinates, deleted_positions, one_line_markers, deleted_markers
        nonlocal bboxes, selected_marker_idx
        coordinates = {
    ...
    def _refresh_restore_snapshot() -> None:
        nonlocal restore_snapshot
        restore_snapshot = _clone_marker_state()

    def _push_undo() -> None:
        push_marker_undo(undo_stack, _clone_marker_state())

    def _can_edit_markers() -> bool:
        return editor_mode == "insert"

    def _require_insert() -> bool:
        """Return True if INSERT; else show banner and return False."""
        nonlocal showing_save_message, save_message_text, save_message_timer
        if editor_mode == "insert":
            return True
        save_message_text = "VISUAL — Ctrl+I to edit"
        showing_save_message = True
        save_message_timer = 45
        return False

    def _toggle_editor_mode() -> None:
        nonlocal editor_mode, showing_save_message, save_message_text, save_message_timer
        editor_mode = "insert" if editor_mode == "visual" else "visual"
        save_message_text = (
            "INSERT — markers editable (Ctrl+Z undo)"
            if editor_mode == "insert"
            else "VISUAL — browse only (Ctrl+I to edit)"
        )
        showing_save_message = True
        save_message_timer = 60

    def _undo_last_marker_edit() -> None:
        nonlocal showing_save_message, save_message_text, save_message_timer
        if editor_mode != "insert":
            save_message_text = "VISUAL — Ctrl+I to edit"
            showing_save_message = True
            save_message_timer = 45
            return
        prev = pop_marker_undo(undo_stack)
        if prev is None:
            save_message_text = "Nothing to undo"
            showing_save_message = True
            save_message_timer = 45
            return
        _apply_marker_state(prev)
        save_message_text = "Undo"
        showing_save_message = True
        save_message_timer = 30

    def _restore_marker_checkpoint() -> None:
        nonlocal undo_stack, showing_save_message, save_message_text, save_message_timer
        if restore_snapshot is None:
            save_message_text = "No restore checkpoint"
            showing_save_message = True
            save_message_timer = 45
            return
        _apply_marker_state(restore_snapshot)
        undo_stack.clear()
        save_message_text = "Restored to last Save (or open)"
        showing_save_message = True
        save_message_timer = 60
    ...
    while running:
        ...
        pad_x, pad_y = compute_letterbox_pads(
            zoomed_width, zoomed_height, window_width, window_height
        )
        cropped_frame = zoomed_frame[
            crop_y : crop_y + window_height, crop_x : crop_x + window_width
        ]

        # Clear video viewport each frame so zoom-out / letterboxing never leaves stale pixels.
        screen.fill((0, 0, 0), pygame.Rect(0, 0, window_width, window_height))
        ...
        if (
            ...
            for a, b in connections:
                ...
                if x1 is None or y1 is None or x2 is None or y2 is None:
                    continue
                sx1, sy1 = video_to_screen_coords(
                    x1,
                    y1,
                ...
                sx2, sy2 = video_to_screen_coords(
                    x2,
                    y2,
                    zoom_level=zoom_level,
                    crop_x=crop_x,
        ...
        if persistence_enabled:
            ...
            for marker_id, positions in marker_trails.items():
                ...
                for i in range(1, len(positions)):
                    ...
                    prev_screen_x, prev_screen_y = video_to_screen_coords(
                        prev_x,
                        prev_y,
                        zoom_level=zoom_level,
                        crop_x=crop_x,
                    ...
                    curr_screen_x, curr_screen_y = video_to_screen_coords(
                        curr_x,
                        curr_y,
                        zoom_level=zoom_level,
                        crop_x=crop_x,
                ...
                if (
                    ...
                    last_screen_x, last_screen_y = video_to_screen_coords(
                        last_x,
                        last_y,
                        zoom_level=zoom_level,
                        crop_x=crop_x,
        ...
        if one_line_mode:
            for idx, (f_num, x, y) in enumerate(one_line_markers):
                if f_num == frame_count:
                    ...
                    screen_x, screen_y = video_to_screen_coords(
                        x,
                        y,
                        zoom_level=zoom_level,
                        crop_x=crop_x,
                    ...
                    # Outline-only rings (no filled disc) so the click pixel stays visible.
                    if idx == selected_marker_idx:
                        pygame.draw.circle(
                            screen, (255, 165, 0), (screen_x, screen_y), 5, 1
                        )  # Orange ring
                        if marker_selection_locked:
                            pygame.draw.circle(screen, (40, 200, 255), (screen_x, screen_y), 7, 2)

                    _core_r = 2 if idx == selected_marker_idx else 3
            ...
            for i, (x, y) in enumerate(coordinates[frame_count]):
                ...
                # Selected marker: thin outline rings only (no filled disc) so the
                # underlying pixel stays visible while still marking the active slot.
                if i == selected_marker_idx:
                    pygame.draw.circle(
                        screen, (255, 165, 0), (screen_x, screen_y), 5, 1
                    )  # Orange ring
                    if marker_selection_locked:
                        # Compact cyan ring so Lock (B) is visible without covering detail
                        pygame.draw.circle(screen, (40, 200, 255), (screen_x, screen_y), 7, 2)
                        pygame.draw.circle(screen, (255, 255, 255), (screen_x, screen_y), 9, 1)

                # Shape-aware outline when AI tracking is active on this marker
                if track_ai_active and i == live_track_target_marker:
                    bw, bh = (
                        track_ai_params.block_window if track_ai_params is not None else (36, 36)
                    )
        ...
        # Live preview while dragging circle/box size for AI Track
        if (
            track_shape_dragging
            and track_shape_drag_origin is not None
            and track_shape_drag_current is not None
        ):
            ox, oy = track_shape_drag_origin
            cx, cy = track_shape_drag_current
            sox, soy = video_to_screen_coords(
                ox,
                oy,
                zoom_level=zoom_level,
                crop_x=crop_x,
            ...
            if track_ai_shape == "circle":
                r_px = max(4, int(np.hypot(cx - ox, cy - oy) * zoom_level))
                pygame.draw.circle(screen, (0, 255, 180), (sox, soy), r_px, 2)
                pygame.draw.circle(screen, (255, 255, 0), (sox, soy), 3)
            else:
                half_w = max(4, int(abs(cx - ox) * zoom_level))
                half_h = max(4, int(abs(cy - oy) * zoom_level))
                prev_box = pygame.Rect(sox - half_w, soy - half_h, half_w * 2, half_h * 2)
                pygame.draw.rect(screen, (0, 255, 180), prev_box, 2)
                pygame.draw.circle(screen, (255, 255, 0), (sox, soy), 3)
        ...
        if show_tracking and csv_loaded and frame_count in tracking_data:
            ...
            for box in boxes:
                ...
                screen_x1, screen_y1 = video_to_screen_coords(
                    box["x1"],
                    box["y1"],
                    zoom_level=zoom_level,
                    crop_x=crop_x,
                ...
                screen_x2, screen_y2 = video_to_screen_coords(
                    box["x2"],
                    box["y2"],
                    zoom_level=zoom_level,
                    crop_x=crop_x,
        ...
        if _my_scr < window_height:
            _vxf, _vyf = screen_to_video_coords(
                _mx_scr,
                _my_scr,
                zoom_level=zoom_level,
                crop_x=crop_x,
            ...
            hover_pixel_xy = (
                max(0.0, min(float(original_width - 1), float(_vxf))),
                max(0.0, min(float(original_height - 1), float(_vyf))),
            )
        else:
            hover_pixel_xy = None
        ...
        if pitch_guide_mode and template_mode != "free":
            ...
            _panel_y = 10
            if _rw > 0 and _rh > 0:
                _ref_left = window_width - _rw - 10
                if _panel_x + _panel_w > _ref_left - 12:
                    _panel_y = 10 + _rh + 12
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    _mods_nav = pygame.key.get_mods()
                    if _mods_nav & pygame.KMOD_SHIFT:
                        mf_nav = sorted_frames_with_visible_markers(
                            coordinates=coordinates,
                            deleted_positions=deleted_positions,
                            one_line_mode=one_line_mode,
                            one_line_markers=one_line_markers,
                        ...
                        if mf_nav:
                            _nxt = next((ff for ff in mf_nav if ff > frame_count), None)
                            frame_count = _nxt if _nxt is not None else mf_nav[0]
                            save_message_text = f"Next marked frame: {frame_count + 1}"
                        else:
                            save_message_text = "No frames with markers yet"
                        showing_save_message = True
                        save_message_timer = 45
                    else:
                        frame_count = min(frame_count + 1, total_frames - 1)
                        _nav_hold_key = pygame.K_RIGHT
                        _nav_hold_started_ms = pygame.time.get_ticks()
                        _nav_hold_last_ms = _nav_hold_started_ms
                    ...
                    if _mods_nav & pygame.KMOD_SHIFT:
                        ...
                        if mf_nav:
                            ...
                            for ff in mf_nav:
                                if ff < frame_count:
                                    ...
                                    break
                            frame_count = _prev if _prev is not None else mf_nav[-1]
                            save_message_text = f"Prev marked frame: {frame_count + 1}"
                        else:
                            save_message_text = "No frames with markers yet"
                        showing_save_message = True
                        save_message_timer = 45
                        ...
                        _nav_hold_key = pygame.K_LEFT
                        _nav_hold_started_ms = pygame.time.get_ticks()
                        _nav_hold_last_ms = _nav_hold_started_ms
                elif event.key == pygame.K_UP and paused:
                    frame_count = min(frame_count + nav_jump_frames, total_frames - 1)
                    _nav_hold_key = pygame.K_UP
                    _nav_hold_started_ms = pygame.time.get_ticks()
                    _nav_hold_last_ms = _nav_hold_started_ms
                elif event.key == pygame.K_DOWN and paused:
                    frame_count = max(frame_count - nav_jump_frames, 0)
                    _nav_hold_key = pygame.K_DOWN
                    _nav_hold_started_ms = pygame.time.get_ticks()
                    _nav_hold_last_ms = _nav_hold_started_ms
                ...
                if y >= window_height and event.button != 1:
                    ...
                    if load_button_rect.collidepoint(x, rel_y):
                        ...
                        # TOML config: use Cfg button or Ctrl+T (not right-click).
                    ...
                    elif geom_tracker_button_rect.collidepoint(x, rel_y):
                        ok_geom, msg_geom = _run_geometric_tracker_action()
                        save_message_text = msg_geom
                        showing_save_message = True
                        save_message_timer = 150 if ok_geom else 90
                        ...
                        timeline_drag_mf_snapshot = sorted_frames_with_visible_markers(
                            coordinates=coordinates,
                            deleted_positions=deleted_positions,
                            one_line_mode=one_line_mode,
                            one_line_markers=one_line_markers,
                        ...
                        frame_count = frame_index_from_marker_timeline_x(
                            ...
                            marked_sorted=timeline_drag_mf_snapshot,
                        )
                        if total_frames and total_frames > 0:
                            frame_count = max(0, min(frame_count, total_frames - 1))
                        save_message_text = f"Timeline → frame {frame_count + 1}"
                        showing_save_message = True
                    ...
                    if calib_mode and quick_measure_calibrating and quickmeasure_draft is not None:
                        ...
                        if not _require_insert():
                            ...
                            track_shape_drag_origin = (float(video_x), float(video_y))
                            ...
                            if track_ai_active and frame is not None:
                                if live_tracker is None:
                                    ...
                                    try:
                                        ...
                                        params.deep_weight = 0.25 if track_ai_use_deep else 0.0
                                        params.tracking_shape = track_ai_shape
                                        if track_ai_params is not None and getattr(
                                            track_ai_params, "deep_weights_path", ""
                                        ):
                                            params.deep_weights_path = (
                                                track_ai_params.deep_weights_path
                                            )
                                        tracker = AITracker(parameters=params)
                                        try:
                                        ...
                                        track_ai_lost_streak = 0
                                        print(
                                            f">> AI Track [ON]: Initial anchor set @ f{frame_count + 1} for m{placed_marker_idx}."
                                        )
                                        save_message_text = f"AI Track: Initial anchor set @ f{frame_count + 1} (m{placed_marker_idx})"
                                        showing_save_message = True
                                        save_message_timer = 120
                                    except Exception as ex_init:
                                        print(f"AI Track initial anchor error: {ex_init}")
                                elif placed_marker_idx == live_track_target_marker:
                                    try:
                                        if hasattr(live_tracker, "update_from_correction"):
                                            live_tracker.update_from_correction(
                                                frame, (video_x, video_y), frame_count
                                            )
                                        else:
                                            live_tracker.add_anchor(
                                                frame, (video_x, video_y), frame_count
                                        ...
                                        save_message_text = (
                                            f"AI Track: Added anchor @ f{frame_count + 1} "
                                            f"(m{placed_marker_idx}, {n_anch} anchors)"
                                        )
                                        showing_save_message = True
                                        save_message_timer = 90
                                    except Exception as ex_anch:
                                        print(f"AI Track online retraining error: {ex_anch}")

                            # Feature: ClickPass logic (advances frame on any marker placement)
                            if click_pass_mode:
                                # Visual Feedback: Redraw placed marker dot before advancing
                                screen_cx, screen_cy = video_to_screen_coords(
                                    video_x,
                                    video_y,
                                    zoom_level=zoom_level,
                                    crop_x=crop_x,
                ...
                if event.button == 2:
                    ...
                    if (
                        ...
                        if y < window_height and frame is not None:
                            ...
                            cx, cy = track_shape_drag_current
                            if track_ai_shape == "circle":
                                radius = max(6.0, float(np.hypot(cx - ox, cy - oy)))
                                bw = max(12, int(round(radius * 2)))
                                bh = bw
                            else:
                                bw = max(12, int(round(abs(cx - ox) * 2)))
                                bh = max(12, int(round(abs(cy - oy) * 2)))
                            center = (float(ox), float(oy))
                            from vaila.tracking import AITracker, AITrackerParameters

                            if track_ai_params is None:
                                track_ai_params = AITrackerParameters(
                                    search_window=(max(140, bw * 4), max(140, bh * 4)),
                                    block_window=(bw, bh),
                                    similarity_threshold=0.45,
                                    template_update_threshold=0.70,
                                    spatial_sigma=22.0,
                                ...
                                track_ai_params.block_window = (bw, bh)
                                sw0, sh0 = track_ai_params.search_window
                                track_ai_params.search_window = (
                                    max(sw0, bw * 3),
                                    max(sh0, bh * 3),
                                )
                                track_ai_params.tracking_shape = track_ai_shape
                            patch = None
                            try:
                                tmp = AITracker(parameters=copy.deepcopy(track_ai_params))
                                patch = tmp.extract_patch(frame, center, (bw, bh))
                                cx_c, cy_c = AITracker.compute_shape_centroid(
                                    patch, track_ai_shape, bw, bh
                                )
                                marker_xy = (
                                    center[0] - bw / 2.0 + cx_c,
                                    center[1] - bh / 2.0 + cy_c,
                                )
                            except Exception:
                                marker_xy = center
                            ...
                            while len(coordinates[frame_count]) <= target_idx:
                                coordinates[frame_count].append((None, None))
                            coordinates[frame_count][target_idx] = (
                                float(marker_xy[0]),
                                float(marker_xy[1]),
                            )
                            deleted_positions[frame_count].discard(target_idx)
                            ...
                            try:
                                ...
                                if live_tracker is None:
                                    ...
                                    live_tracker.params = params
                                    if hasattr(live_tracker, "_apply_shape_mask"):
                                        live_tracker._apply_shape_mask()
                                    live_tracker.update_from_correction(
                                        frame,
                                        (float(marker_xy[0]), float(marker_xy[1])),
                                        frame_count,
                                    )
                                live_track_target_marker = target_idx
                                live_track_last_frame = frame_count
                                track_ai_lost_streak = 0
                                save_message_text = (
                                    f"AI Track Shp:{track_ai_shape} size={bw}x{bh} "
                                    f"@ centroid ({marker_xy[0]:.0f},{marker_xy[1]:.0f})"
                                )
                                showing_save_message = True
                                save_message_timer = 120
                                print(f">> {save_message_text}")
                            except Exception as ex_shp:
                                print(f">> AI Track shape drag error: {ex_shp}")
                ...
                elif track_shape_dragging and track_shape_drag_origin is not None:
                    x, y = event.pos
                    if y < window_height:
                        video_x, video_y = screen_to_video_coords(
                            x,
                ...
                if my < window_height:  # Only if in video area
                    # Center zoom on mouse cursor (account for letterbox pads)
                    old_zw = int(original_width * old_zoom)
                    old_zh = int(original_height * old_zoom)
                    old_pad_x, old_pad_y = compute_letterbox_pads(
                        old_zw, old_zh, window_width, window_height
                    )
                    target_vx = (mx - old_pad_x + offset_x) / old_zoom
                    target_vy = (my - old_pad_y + offset_y) / old_zoom

                    zoomed_width = original_width * zoom_level
                    zoomed_height = original_height * zoom_level
                    new_pad_x, new_pad_y = compute_letterbox_pads(
                        int(zoomed_width), int(zoomed_height), window_width, window_height
                    )
                    offset_x = (target_vx * zoom_level) - (mx - new_pad_x)
                    offset_y = (target_vy * zoom_level) - (my - new_pad_y)
        ...
        if paused and _nav_hold_key is not None:
            ...
            if not pressed[_nav_hold_key]:
                ...
                if now_ms - _nav_hold_started_ms >= _NAV_HOLD_DELAY_MS:
                    is_ud = _nav_hold_key in (pygame.K_UP, pygame.K_DOWN)
                    repeat_ms = _NAV_HOLD_REPEAT_UD_MS if is_ud else _NAV_HOLD_REPEAT_LR_MS
                    if now_ms - _nav_hold_last_ms >= repeat_ms:
                        if _nav_hold_key == pygame.K_RIGHT:
                            frame_count = min(frame_count + 1, total_frames - 1)
                        elif _nav_hold_key == pygame.K_LEFT:
                            frame_count = max(frame_count - 1, 0)
                        elif _nav_hold_key == pygame.K_UP:
                            frame_count = min(frame_count + nav_jump_frames, total_frames - 1)
                        elif _nav_hold_key == pygame.K_DOWN:
                            frame_count = max(frame_count - nav_jump_frames, 0)
                        _nav_hold_last_ms = now_ms
                        if track_ai_active and live_tracker is not None:
                            # Seek+track so the marker follows hold-repeat nav (←→/↑↓)
                            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_count)
                            ret_nav, frame_nav = cap.read()
...
def _vaila_pitch_p_indices_from_columns(columns: Any) -> list[int]:
    """Return sorted keypoint indices ``N`` parsed from ``pN_x`` column names."""
    found: set[int] = set()
    for col in columns:
...
def load_vaila_p_xy_markers_df(
    df: Any, *, total_frames: int
) -> tuple[dict[int, list[Any]], list[str]]:
    """Parse ``frame`` + ``pN_x`` / ``pN_y`` into slot-aligned per-frame lists.

    Column index ``N`` maps to list slot ``N - min(indices)``, so FIFA **0-based**
    templates (``p0_x`` = ``top_left_corner`` / YOLO kp 0) round-trip correctly.
    Legacy **1-based** exports (``p1_x`` …) still map list slot 0 to ``p1``.
    """
    kp_ids = _vaila_pitch_p_indices_from_columns(df.columns)
    if not kp_ids:
        raise ValueError("no pN_x columns found")
    if "frame" not in df.columns:
        raise ValueError("no frame column")

    min_kp, max_kp = kp_ids[0], kp_ids[-1]
    n_slots = max_kp - min_kp + 1
    coordinates: dict[int, list[Any]] = {i: [] for i in range(total_frames)}

    for _, row in df.iterrows():
        try:
            frame_raw = row.get("frame", 0)
            frame_num = int(frame_raw) if pd.notna(frame_raw) else 0
        except (TypeError, ValueError):
            frame_num = 0

        pts: list[Any] = [(None, None)] * n_slots
        for k in kp_ids:
            slot = k - min_kp
            x_val = row.get(f"p{k}_x")
            y_val = row.get(f"p{k}_y")
            try:
                if pd.notna(x_val) and pd.notna(y_val):
                    pts[slot] = (float(x_val), float(y_val))
                else:
                    pts[slot] = (None, None)
            except (TypeError, ValueError):
                pts[slot] = (None, None)
    ...
    labels = [f"Pixel {i}" for i in range(min_kp, max_kp + 1)]
...
def sorted_frames_with_visible_markers(
    *,
    ...
    deleted_positions: dict[int, set[int]] | None,
    one_line_mode: bool,
    one_line_markers: list[Any],
    deleted_markers: set[int] | None,
    total_frames: int,
) -> list[int]:
    """Return sorted frame indices that have at least one visible marker coordinate."""
    del_pos = deleted_positions or {}
    del_one = deleted_markers or set()

    if one_line_mode:
        frames_hit: set[int] = set()
        for idx, m in enumerate(one_line_markers):
            if idx in del_one:
                continue
            try:
                f_num, x, y = int(m[0]), m[1], m[2]
            except (TypeError, ValueError, IndexError):
                continue
            if x is not None and y is not None and 0 <= f_num < total_frames:
                frames_hit.add(f_num)
        return sorted(frames_hit)
    ...
    for f_idx in range(max(0, total_frames)):
        ...
        if not pts:
            continue
        deleted_here = del_pos.get(f_idx, set())
        found = False
...
def frame_index_from_marker_timeline_x(
    *,
    mouse_x: int,
    strip_left: int,
    strip_width: int,
    denom_frames: int,
    marked_sorted: list[int],
) -> int:
    """Map a click (or drag) X position on the marker strip to a frame index.

    If the timeline column under the cursor contains marked frames, snap to the
    middle marked frame in that column; otherwise use the same proportional map
    as the main scrub slider.
    """
    if strip_width <= 0 or denom_frames <= 0:
        return 0
    rel_xx = float(mouse_x - strip_left)
    rel_xx = max(0.0, min(rel_xx, float(strip_width)))
    px_col = min(strip_width - 1, int(rel_xx))
    f0 = int(px_col * denom_frames / strip_width)
    f1 = int((px_col + 1) * denom_frames / strip_width)
    f1 = max(f0 + 1, min(f1, denom_frames))
    lo = bisect.bisect_left(marked_sorted, f0)
    hi = bisect.bisect_left(marked_sorted, f1)
    segment = marked_sorted[lo:hi]
    if segment:
        return segment[len(segment) // 2]
    target = int((rel_xx / strip_width) * denom_frames)
    return max(0, min(target, denom_frames - 1))
...
def load_coordinates_from_file(total_frames, video_width=None, video_height=None):
    ...
    if "frame" in df.columns and any(
        ...
        try:
            ...
            print(f"ERROR: failed to parse vailá CSV: {e}")
            return {i: [] for i in range(total_frames)}, []

        kp_ids = _vaila_pitch_p_indices_from_columns(df.columns)
        print(f"Keypoint column indices: {kp_ids[0]}..{kp_ids[-1]} ({len(kp_ids)} slots)")
    ...
    print(
        "  1. vailá format: 'frame', 'p0_x', 'p0_y', … (FIFA 0-based) "
        "or 'p1_x', 'p1_y', … (legacy 1-based)"
    )
...
def _write_data_yaml(dataset_dir):
    ...
    yaml_content = f"""# YOLO dataset - generated by {VAILA_MARK} getpixelvideo
path: .
train: {train_path}
...
def _write_pose_data_yaml(
    ...
    * ``"fifa"`` - relative ``images/{split}`` (matches
    ...
    if layout == "fifa":
        ...
        yaml_content = (
            f"# YOLO pose dataset - generated by {VAILA_MARK} getpixelvideo (FIFA layout)\n"
            f"path: {path_abs}\n"
            "train: images/train\n"
        ...
        yaml_content = (
            f"# YOLO pose dataset - generated by {VAILA_MARK} getpixelvideo\n"
            "path: .\n"
            f"train: {train_path}\n"
...
def run_getpixelvideo(
    ...
    print("\n" + "=" * 50)
    print(f"{VAILA_MARK} – Pixel Coordinate Tool")
    print("=" * 50)
