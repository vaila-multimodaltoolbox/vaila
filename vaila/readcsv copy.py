...
def select_headers_gui(headers):
    ...
    num_columns = 9 # Number of columns for the labels
...
def update_figure(selected_frame, relayoutData):
    fig = go.Figure()

    if 'markers' in globals():
        for marker in markers:
            fig.add_trace(go.Scatter3d(
                x=[marker['x'][selected_frame]],
                y=[marker['y'][selected_frame]],
                z=[marker['z'][selected_frame]],
                mode='markers',
                marker=dict(size=4),
                name=marker['name']
            ))
    ...
    frame_display = f'Current frame: {selected_frame}'
    ...
    try:
        ...
        if file_path:
            ...
            if selected_headers:
                ...
                # Verifique se todos os headers selecionados existem no DataFrame
                missing_headers = [header for header in selected_headers if header not in headers]
                if missing_headers:
                    print(f"Erro: Os seguintes headers selecionados não foram encontrados no arquivo CSV: {missing_headers}")
                else:
                    print("Todos os headers selecionados estão presentes no arquivo CSV.")
