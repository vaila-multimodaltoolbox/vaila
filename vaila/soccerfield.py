...
def plot_field(df):
    ...
    fig, ax = plt.subplots(figsize=(10.5 + 0.4, 6.8 + 0.4))  # Increasing slightly to accommodate the margin
    ax.set_xlim(-margin - 1, 105 + margin + 1)  # Expanding the limits to include the margin
    ...
    draw_rectangle(ax, points['canto_inf_esq'], 105, 68, 
                  edgecolor='none', 
    ...
    draw_line(ax, points['canto_inf_esq'], points['canto_sup_esq'], color='white', linewidth=2, zorder=1)
    
    # Right sideline (right sideline)
    draw_line(ax, points['canto_inf_dir'], points['canto_sup_dir'], color='white', linewidth=2, zorder=1)
    
    # Bottom goal line (bottom goal line)
    draw_line(ax, points['canto_inf_esq'], points['canto_inf_dir'], color='white', linewidth=2, zorder=1)
    
    # Top goal line (top goal line)
    draw_line(ax, points['canto_sup_esq'], points['canto_sup_dir'], color='white', linewidth=2, zorder=1)

    # Center line (center line)
    draw_line(ax, points['meio_campo_esq'], points['meio_campo_dir'], color='white', linewidth=2, zorder=1)

    # Center circle (center circle)
    draw_circle(ax, points['centro_campo'], 9.15, edgecolor='white', facecolor='none', linewidth=2, zorder=1)

    # Center spot (center spot)
    draw_circle(ax, points['centro_campo'], 0.2, edgecolor='white', facecolor='white', linewidth=2, zorder=1)

    # Left penalty area (left penalty area)
    draw_rectangle(ax, points['ga_esq_inf_esq'], 16.5, 40.3, edgecolor='white', facecolor='none', linewidth=2, zorder=1)

    # Left goal area (left goal area)
    draw_rectangle(ax, points['pa_esq_inf_esq'], 5.5, 18.32, edgecolor='white', facecolor='none', linewidth=2, zorder=1)

    # Left penalty spot (left penalty spot)
    draw_circle(ax, points['penalti_esq'], 0.2, edgecolor='white', facecolor='white', linewidth=2, zorder=1)

    # Left penalty arc - only the arc outside the large area
    draw_arc(ax, points['penalti_esq'], 9.15, 310, 50, edgecolor='white', linewidth=2, zorder=1)

    # Right penalty area (right penalty area)
    draw_rectangle(ax, (88.5, 13.85), 16.5, 40.3, edgecolor='white', facecolor='none', linewidth=2, zorder=1)

    # Right goal area (right goal area)
    draw_rectangle(ax, (99.5, 24.84), 5.5, 18.32, edgecolor='white', facecolor='none', linewidth=2, zorder=1)

    # Right penalty spot (right penalty spot)
    draw_circle(ax, points['penalti_dir'], 0.2, edgecolor='white', facecolor='white', linewidth=2, zorder=1)

    # Right penalty arc (right penalty arc)
    draw_arc(ax, points['penalti_dir'], 9.15, 130, 230, edgecolor='white', linewidth=2, zorder=1)

    # Left goal line (left goal line) - thicker than the other lines
    draw_line(ax, points['gol_esq_poste_esq'], points['gol_esq_poste_dir'], 
             color='white', linewidth=8, zorder=2)  

    # Right goal line (right goal line) - thicker than the other lines
    draw_line(ax, points['gol_dir_poste_esq'], points['gol_dir_poste_dir'], 
             color='white', linewidth=8, zorder=2)  
