...
- **Per-cut names from CSV (NEW)**: Load a CSV/TXT list of base names (one per cut) via **N** or the **Names CSV** button. Cut 1 → name 1, cut 2 → name 2, … and each segment is written as `<name>.mp4` (clean, no frame suffix). Accepts a single comma-separated line *or* one name per line; a 2-column `name,label` CSV also works (first field is used). Names are sanitized (letters/digits/`_`/`-`) and duplicates are de-duplicated with `_2`, `_3` suffixes. If fewer names than cuts are given, the remaining cuts fall back to the single base name / video stem; extra names are ignored. Per-cut names take priority over the single **B** base name.
...
- **Player startup fix (v0.3.47)**: `output_basename` is initialized before the pygame UI so the player opens reliably after metadata load.
...
- `prompt_output_basenames_csv()` — Tk file dialog to load a per-cut name list (**N** / **Names CSV** button).
...
- **Right (top to bottom):** **Loop**, **Help**, then a row with **Names CSV** (same as **N**; shows **Names ✓n** when a per-cut list is loaded) and **Base name** (same as **B**; shows **Base ✓** when a custom prefix is set).

### Keyboard and mouse
- **Playback/Navigation:** Space (play/pause), →/← (frame step), Shift+→/← (next/previous cut marker with wraparound), ↑/↓ (±60 frames), mouse on slider (jump), click/drag cut strip (jump with marker snap).
...
- **Markers/Cuts:** S (start), E (end), R (reset start), D or Delete (remove last cut), L (list cuts), C (load cut labels CSV).
...
- **File ops:** F (load cuts TOML or sync TXT), B (set single output base name), N (load per-cut base-name CSV/TXT), ESC (save TOML, optionally render cuts; batch prompt).
