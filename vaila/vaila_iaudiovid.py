    ...
    def process_video(self, video_file, output_file):
        ...
        try:
            ...
            if video_duration > audio_duration and audio_duration > 0:
                if self.status_callback:
                    self.status_callback(f"Video ({video_duration:.2f}s) is longer than audio ({audio_duration:.2f}s). Creating looped audio...")
                
                # Create a temporary file for the looped audio
                temp_dir = os.path.dirname(output_file)
                os.makedirs(temp_dir, exist_ok=True)
                temp_audio_file = os.path.join(temp_dir, f"temp_looped_audio_{datetime.now().strftime('%Y%m%d%H%M%S')}.wav")
                
                # Calculate how many loops we need
                loops_needed = int(math.ceil(video_duration / audio_duration))
                
                if self.status_callback:
                    self.status_callback(f"Creating looped audio with {loops_needed} repetitions...")
                ...
                loop_cmd = [
                    ...
                    '-map', '[aout]',
                    '-t', str(video_duration),  # Limit duration to video length
                    '-acodec', 'pcm_s16le',  # Use uncompressed audio to avoid encoding issues
                    temp_audio_file
                ]
                ...
                if loop_process.returncode == 0:
                    ...
                    if self.status_callback:
                        self.status_callback(f"Error creating looped audio: {loop_process.stderr}. Trying alternative method...")
                    
                    # Fallback to a simpler approach - concatenate the file with itself
                    temp_list_file = os.path.join(temp_dir, f"temp_list_{datetime.now().strftime('%Y%m%d%H%M%S')}.txt")
                    ...
                    if fallback_process.returncode == 0:
                        if self.status_callback:
                            self.status_callback("Looped audio created successfully with fallback method")
                        audio_input = temp_audio_file
                    else:
                        if self.status_callback:
                            self.status_callback(f"Fallback method also failed. Using original audio.")
            ...
            cmd = [
                ...
                '-i', video_file,  # First input: video file
                '-i', audio_input,  # Second input: audio file (original or looped)
    ...
    def process_all_videos(self):
        ...
        for i, video_file in enumerate(self.video_files, 1):
            try:
                ...
                self.processing_log.append(f"Error processing {os.path.basename(video_file)}: {str(e)}")
...
class AudioVideoGUI:
    ...
    def __init__(self, root):
        ...
        ttk.Label(video_frame, text="Directory containing video files:").pack(anchor=tk.W, pady=(0, 2))
        ...
        ttk.Label(output_frame, text="Directory to save processed files:").pack(anchor=tk.W, pady=(0, 2))
        ...
        ttk.Label(status_frame, textvariable=self.status_var, font=('Arial', 9, 'bold')).pack(side=tk.LEFT)
