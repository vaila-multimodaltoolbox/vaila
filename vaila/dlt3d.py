...
def create_ref3d_template(pixel_file):
    ...
    template = df.copy()
    template.iloc[:, 1:] = np.nan  # Limpa todos os dados de coordenadas, mas mantém os números dos frames
...
def main():
    ...
    create_ref = messagebox.askyesno("Create REF3D File", "Do you want to create a REF3D file based on the pixel file?")
    if create_ref:
        ...
        print("Please edit the REF3D file with real coordinates and run the DLT process again.")
        return
    else:
        real_file = filedialog.askopenfilename(title="Select Real 3D Coordinates File", filetypes=[("REF3D files", "*.ref3d")])
        if not real_file:
