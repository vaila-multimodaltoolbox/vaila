...
def load_skout_ascii(
    path: Path,
) -> Tuple[Dict[int, str], Dict[int, str], List[SkoutRecord]]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Skout bundle: create CSV + TOML from Skout ASCII export"
    )
    ap.add_argument(
        "input", type=str, help="Path to Skout ASCII export (e.g., braxing.txt)"
    )
    ap.add_argument(
        "--csv-out",
        type=str,
        default="",
        help="Output CSV path (default: input basename + .csv)",
    )
    ap.add_argument(
        "--toml-out",
        type=str,
        default="",
        help="Output TOML path (default: input basename + _config.toml)",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="HOME",
        help="Team name to write in CSV and TOML [HOME]",
    )
    ap.add_argument(
        "--time-mode",
        type=str,
        default="infer",
        choices=["infer", "sequence", "zero"],
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="Seconds per event when using sequence/implicit times [1.0]",
    )

    ap.add_argument(
        "--field-width", type=float, default=105.0, help="Field width in meters [105]"
    )
    ap.add_argument(
        "--field-height", type=float, default=68.0, help="Field height in meters [68]"
    )
    ap.add_argument(
        "--grid-width",
        type=float,
        default=None,
        help="Skout grid width (pixels/units); default: observed max X",
    )
    ap.add_argument(
        "--grid-height",
        type=float,
        default=None,
        help="Skout grid height (pixels/units); default: observed max Y",
    )
    ...
    toml_out = (
        Path(args.toml_out)
        if args.toml_out
        else in_path.with_name(in_path.stem + "_config.toml")
    )
