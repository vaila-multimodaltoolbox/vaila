...
def test_mhr70_pn_indices_map_left_right_body_landmarks():
    """C3D / rec3d pN = 0-based index into MHR70_NAMES.

    Guard against accidental left/right renumbering that would make mono or
    DLT C3D look mirrored relative to the green/orange overlay. The pN labels
    were 1-based until 0.4.3, when every DLT/REC point column in vaila was
    rebased to p0 to match getpixelvideo.py's pixel columns.
    """
    assert COLOR_LEFT_RGB == (0, 255, 0)
    assert COLOR_RIGHT_RGB == (255, 128, 0)
    assert MHR70_NAMES[0] == "nose"  # p0
    assert MHR70_NAMES[5] == "left-shoulder"  # p5
    assert MHR70_NAMES[6] == "right-shoulder"  # p6
    assert MHR70_NAMES[7] == "left-elbow"  # p7
    assert MHR70_NAMES[8] == "right-elbow"  # p8
    assert MHR70_NAMES[9] == "left-hip"  # p9
    assert MHR70_NAMES[10] == "right-hip"  # p10
    assert MHR70_NAMES[11] == "left-knee"  # p11
    assert MHR70_NAMES[12] == "right-knee"  # p12
    assert MHR70_NAMES[13] == "left-ankle"  # p13
    assert MHR70_NAMES[14] == "right-ankle"  # p14
    assert MHR70_NAMES[69] == "neck"  # p69
    left_idx = {i for i, n in enumerate(MHR70_NAMES) if n.startswith("left-")}
    right_idx = {i for i, n in enumerate(MHR70_NAMES) if n.startswith("right-")}
    assert left_idx.isdisjoint(right_idx)
    assert {5, 7, 9, 11, 13}.issubset(left_idx)
    assert {6, 8, 10, 12, 14}.issubset(right_idx)
