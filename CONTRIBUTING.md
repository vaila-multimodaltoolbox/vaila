...
- **Package version** — Source of truth is [`pyproject.toml`](pyproject.toml) under `[project].version` (PEP 621). This is what **`uv`** and **`pip`** report and what must match **wheels / sdist** metadata. Example form: `0.3.36` (often written as **v0.3.36** in prose).
...
- **Release notes** — Prefer stating **both** so installers and GitHub readers stay aligned, for example:
  - `Package version: v0.3.36`
  - `Release codename: rp23mar26 (Ribeirão Preto — 23 Mar 2026)`
- **Git tags** — Pick **one** convention and use it consistently:
  - **Option A:** Git tag = **`v0.3.36`** (semver); GitHub **release title** = **`rp23mar26`** or **`rp23mar26 — v0.3.36`**.
  - **Option B:** Git tag = **`rp23mar26`**; the release description **must** clearly state the **package version** (e.g. **v0.3.36**).
...
- **Versão do pacote:** definida em `pyproject.toml` (`[project].version`); é a versão que **`uv`** / **`pip`** mostram (ex.: `0.3.36`).
...
- **Tags Git:** ou tag semântica **`v0.3.36`** com título da release **`rp...`**, ou tag **`rp...`** com a versão do pacote explícita no texto — ver [Versioning and GitHub releases](#versioning-and-github-releases) (inglês).
