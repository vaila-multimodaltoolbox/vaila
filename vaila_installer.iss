...
;
; Post-install runs install_vaila_win.ps1 with -InstallLocation portable so the
; interactive location prompt is skipped. Files are already under {app}
; (Program Files\vaila); portable mode uses $PSScriptRoot == {app}.

#define MyAppName "vaila"
#define MyAppVersion "0.3.85"
...
; -InstallLocation portable: skip location prompt; $PSScriptRoot is already {app}
; GPU/SAM prompts remain interactive in the PowerShell window
Filename: "powershell.exe"; Parameters: "-ExecutionPolicy Bypass -File ""{app}\install_vaila_win.ps1"" -InstallLocation portable"; WorkingDir: "{app}"; Flags: shellexec waituntilterminated postinstall; Description: "Run vaila installation (uv) and install dependencies (rsync/scp)"
