...
def extract_button_info():
    ...
    with open(vaila_path, 'r', encoding='utf-8') as f:
        content = f.read()
...
def main():
    ...
    with open(readme_path, 'w', encoding='utf-8') as f:
        ...
        f.write("This directory contains documentation for all buttons in the vailá GUI (`vaila.py`).\n\n")
        f.write("## Structure\n\n")
        ...
        for filename, pos, text in sorted(created_files, key=lambda x: x[1]):
            f.write(f"| {pos} | {text} | `{filename.replace('-', '_')}` | [{filename}.md]({filename}.md) |\n")
        ...
        f.write("- Script-specific help: `vaila/help/` - Contains help for individual Python scripts\n")
    ...
    print(f"  2. Add HTML versions if needed")
    print(f"  3. Update docs/index.md and docs/help.html with links")
